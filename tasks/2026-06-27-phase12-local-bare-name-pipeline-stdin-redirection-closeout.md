# Phase 12 Local Bare-Name Pipeline Stdin Redirection Closeout

Task id: phase12-local-bare-name-pipeline-stdin-redirection-closeout-20260627
...
Reconcile the accepted fixed-/bin bare-name two-stage pipeline producer stdin
...
command input, accept consumer-stage redirection, accept broader shell grammar,
or accept a phase transition.

## Findings

- fixed: The accepted bare-name pipeline-stage stdin redirection frontier is
...
- fixed: The accepted evidence records producer canonical argv0=/bin/stdin,
  fd0 source-route=initramfs:/etc/banner.txt, fd1 as the pipe endpoint,
  inherited fd2, a closed loader temporary descriptor, a successful userspace
  stdin read, and a bounded process-table entry.
- fixed: The accepted evidence records consumer canonical argv0=/bin/stdin,
  fd0 as the pipe endpoint from the producer, inherited fd1/fd2, a closed
  loader temporary descriptor, successful launch/status, successful pipe read
  to EOF, and a bounded process-table entry.
...
- fixed: Existing direct path-form pipeline-stage stdin redirection,
  direct/bare-name stdin redirection, direct/bare pipeline argv, command argv,
...
- not-an-issue: No implementation change is required for this closeout; the
  bare-name pipeline stdin redirection core task already accepted the source
...
- Bare-name pipeline stdin redirection core:
  tasks/2026-06-27-phase12-local-bare-name-pipeline-stdin-redirection-core.md.
- Bare-name pipeline stdin redirection classification and evidence:
...
- Bare-name pipeline stdin redirection QEMU/substitute transcript:
...
can redirect only the producer fd0 from one read-only initramfs file while
preserving producer fd1 as the pipe endpoint:

~~~text
stdin </etc/banner.txt | stdin
~~~

Both stages resolve through bounded /bin lookup to /bin/stdin before
descriptor-backed VFS open/read, the accepted loader, userspace
launch/status, and bounded process-table observation. The producer records
fd0 source-route=initramfs:/etc/banner.txt, fd1 as the pipe endpoint,
inherited fd2, loader-temp-open=false, and a regular-file EOF after read. The
consumer records fd0 as the pipe endpoint, inherited fd1/fd2,
...
The accepted direct path-form and bare-name producer-stage pipeline stdin
redirection surfaces now give a mechanically objective follow-up for a local
pipeline stdin redirection frontier checkpoint.
...
selected_next_task: phase12-local-pipeline-stdin-redirection-frontier-checkpoint-20260627.

The pipeline-stage stdin redirection frontier checkpoint is mechanically
