# Phase 10 Arbitrary /tmp Output Redirection Frontier Closeout

Task: phase10-arbitrary-tmp-output-redirection-frontier-closeout-20260604
Status: accepted

## Scope

Close out the accepted stdout and stderr arbitrary volatile '/tmp' basename
output redirection frontier without adding code.

The accepted shell-visible output-path forms are:
...
The basename grammar remains the conservative Phase 10 output-path policy: a
non-empty basename under '/tmp/' made only of ASCII letters, digits, '.', '_',
and '-'. The route is volatile VFS only and does not imply persistence,
directories, arbitrary input paths, descriptor moves, process
accounting/concurrency, or broad writable filesystem mutation.

This closeout does not implement code, run Pi 5 hardware, acquire
hardwareTestLock, add arbitrary input paths, persistent storage, broad writable
filesystem mutation, descriptor moves, process accounting/concurrency,
networking, SSH, or a phase transition.

## Findings

- fixed: Consolidated stdout truncate/create and append/create arbitrary
  volatile '/tmp' output evidence for '/tmp/alpha.log' and '/tmp/beta.out',
  including volatile VFS target routes, userspace TalosWrite provenance,
  descriptor-backed readbacks, waitpid, laststatus, and shell fd1 restoration.
- fixed: Consolidated explicit fd1 arbitrary-path aliases for
  '/tmp/gamma.log' and '/tmp/delta.out', preserving the same child-only fd1
  rebinding and restoration behavior as implicit stdout redirection.
- fixed: Consolidated stderr truncate/create and append/create arbitrary
  volatile '/tmp' output evidence for '/tmp/omega.err' and '/tmp/theta.log',
  including volatile VFS target routes, userspace TalosWrite provenance,
  descriptor-backed readbacks, waitpid, laststatus, shell fd2 restoration, and
  distinct stdout routing.
- fixed: Confirmed the shared path policy rejects outside-/tmp paths,
  nested/traversal paths, empty basenames, unsupported descriptor numbers,
  reserved opposite-stream scratch names, and unsupported fd2 shorthand output
  grammar without shrinking exact-path output redirection behavior.
...
- not-an-issue: The stderr slice reused the already accepted conservative
  volatile '/tmp' output basename policy; no new ADR or policy change is
  required for this frontier closeout.
...
- deferred: arbitrary input paths, persistence, recursive directories, path
  traversal, broad writable filesystem mutation, arbitrary descriptor syntax
  beyond accepted fd1/fd2 output forms, descriptor moves, here-docs, wider
  shell grammar, multi-stage/concurrent pipelines, pipefail, jobs, async
  execution, fork, signals, process accounting/concurrency, Pi 5 proof,
...
- stdout arbitrary-/tmp primary evidence:
  ...
  records 'exec stdout >/tmp/alpha.log',
  'exec stdout >>/tmp/beta.out', 'exec stdout 1>/tmp/gamma.log', and
  'exec stdout 1>>/tmp/delta.out'.
- stdout truncate/create and append/create readback evidence:
  the stdout primary log records target paths '/tmp/alpha.log' and
  '/tmp/beta.out', matching 'target-route=volatile-vfs:/tmp/<basename>',
  'source=userspace-talos-write', waitpid, laststatus, and descriptor-backed
  'cat /tmp/<basename>' readbacks with
  'source=volatile-vfs-descriptor-read'.
- explicit fd1 arbitrary-path evidence:
  the stdout primary log records 'source-fd=0x0000000000000001', target paths
  '/tmp/gamma.log' and '/tmp/delta.out', matching volatile VFS routes,
  descriptor-backed readbacks, waitpid, laststatus, and shell fd1 restoration
  through a later normal 'exec stdout' routed to 'runtime-console0/stdout'.
...
- stderr truncate/create and append/create readback evidence:
  the stderr primary log records target paths '/tmp/omega.err' and
  '/tmp/theta.log', matching 'target-route=volatile-vfs:/tmp/<basename>',
  'source=userspace-talos-write', waitpid, laststatus, and descriptor-backed
  'cat /tmp/<basename>' readbacks with
  'source=volatile-vfs-descriptor-read'.
- restoration and stream-separation evidence:
  the stdout primary log records shell fd1 restoration through a later normal
  'exec stdout' routed to 'runtime-console0/stdout'. The stderr primary log
  records shell fd2 restoration through a later normal 'exec stderr' routed to
  'runtime-console0/stderr' and a distinct normal 'exec stdout' routed to
  'runtime-console0/stdout'.
- path-policy negative evidence:
  the stdout primary log records deterministic rejection for
  'exec stdout >/var/out.txt', 'exec stdout >/tmp/nested/out.txt',
  'exec stdout >/tmp/', 'exec stdout 3>/tmp/alpha.log',
  'exec stdout >/tmp/../bad.txt', and 'exec stdout >/tmp/stderr.txt'. The
  stderr primary log records deterministic rejection for
  ...
  'exec stderr >/tmp/misbound.err'. Both primary logs end with their final
  classifications, errors=0, and PASS.
...
- VFS-backed '/bin/stdout' output through child fd1 and '/bin/stderr' output
  through child fd2;
- child-only fd1/fd2 rebinding and shell descriptor restoration after child
  exit;
- truncate/create and append/create behavior for accepted stdout and stderr
  output targets;
...
- arbitrary descriptor syntax beyond accepted fd1/fd2 output forms,
  descriptor moves, and here-docs;
...
No explicit follow-up task is mechanically unblocked by this closeout. The
supervisor must plan the next bounded Phase 10 objective before any broader
capability is promoted.

## Validation

- static inspection: accepted stdout and stderr arbitrary-/tmp QEMU/substitute
  evidence was inspected, including truncate/create, append/create, explicit
  fd1 aliases, matching target paths/routes, userspace TalosWrite provenance,
  descriptor-backed readbacks, fd1/fd2 restoration, distinct stdout/stderr
  behavior, deterministic negatives, waitpid, laststatus, completion markers,
  errors=0, and PASS lines.
...
Commit: final arbitrary-/tmp output redirection frontier closeout commit
