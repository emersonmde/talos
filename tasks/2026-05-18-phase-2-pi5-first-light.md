...
- 2026-05-19 UEFI diagnostic local/substitute validation passed: `file` identified the app as `PE32+ executable (EFI application) Aarch64`, FAT staging contained `EFI/BOOT/BOOTAA64.EFI` and `startup.nsh`, QEMU/AAVMF printed `Talos EFI first-light PASS`, and the normal Talos gates still passed: `cargo fmt --check`, `cargo -Zjson-target-spec test`, Pi 5 target build, `./scripts/rpi5-image.sh`, and `./scripts/qemu-smoke.sh`. No hardware test was run because the lab boot tree does not yet include a U-Boot/UEFI stage.
...
- To hardware-test the UEFI diagnostic, stage a known UEFI-capable loader first, most likely U-Boot for Pi 5, then boot `EFI/BOOT/BOOTAA64.EFI` from the Talos FAT image and capture serial output.
