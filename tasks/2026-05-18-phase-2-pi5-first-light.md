...
- Combined prefix-plus-armstub diagnostic completed and stopped at the same boundary. The remaining productive path is lab-side TFTP request/file-load visibility, EEPROM boot diagnostics, or a known-good Pi OS Lite source-tree snapshot for a byte-level comparison.
