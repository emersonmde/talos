...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=TALOS_RPI5_SECONDARY_SCHEDULER_SERVICE_LOOP_PROOF");
    ...
    println!("cargo:rustc-check-cfg=cfg(talos_rpi5_secondary_scheduler_service_loop_proof)");
...
fn assemble_aarch64(source: &str, output: &PathBuf, target: &str) {
    ...
    if env::var_os("TALOS_RPI5_SECONDARY_SCHEDULER_SERVICE_LOOP_PROOF").is_some() {
        command.arg("-DTALOS_RPI5_SECONDARY_SCHEDULER_SERVICE_LOOP_PROOF");
        println!("cargo:rustc-cfg=talos_rpi5_secondary_scheduler_service_loop_proof");
    }
