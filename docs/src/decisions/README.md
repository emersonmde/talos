...
- Decision: Add staging helpers for a U-Boot bridge: one preflight script checks whether the local environment can configure U-Boot `rpi_arm64_defconfig`, and one boot-tree script stages a supplied `u-boot.bin` as `kernel_2712.img` with `EFI/BOOT/BOOTAA64.EFI`, `boot.scr`, PXE fallback config, and a serial-prefixed mirror.
- Required validation: Before hardware, the U-Boot binary must be non-empty and sourced from a trusted build, `boot.scr` must be generated by `mkimage`, archive review must pass in `uboot-efi` mode, and the pre-hardware review must confirm this is one controlled Pi 5 test under `hardwareTestLock`.
- Risks: The current container is missing `bison`, `flex`, `dtc`, `mkimage`, and an AArch64 GCC cross compiler, and cannot install packages without elevated privileges. A third-party prebuilt U-Boot binary would require provenance review before lab use. U-Boot may also need environment tuning if its default DHCP/PXE path does not load the EFI app from this TFTP lab shape.
- Alternatives considered: keep direct firmware diagnostics, ask for EEPROM/vclog data, or build a Linux-loaded payload first. U-Boot is the nearest public intermediate-loader path that aligns with the existing Pi 5 firmware handoff and the already validated EFI payload.
