...
- Status: accepted as the documentation-only Milestone 8.3 QEMU/substitute
  smoke plan for the live translation-register activation frontier. No Rust
  ...
  MAIR_EL1/SCTLR_EL1 mutation, active-root descriptor copy, ASID allocation,
  live TLB invalidation, activation DSB/ISB, lower-EL ERET, scheduler runnable
...
- Context: The accepted live translation-register activation contract selected
  a model/substitute-only activation-commit boundary below architectural
  register mutation and explicitly named this QEMU/substitute smoke-plan task
  as the mechanically next documentation boundary.
- Decision: Accept
  phase8-qemu-live-translation-register-activation-smoke-plan-20260531. The
  plan requires scenario qemu_live_translation_register_activation_smoke,
  retained evidence at
  tasks/evidence/2026-05-31-qemu-live-translation-register-activation-smoke-core/qemu-live-translation-register-activation-smoke.log,
  classification qemu-live-translation-register-activation-smoke-complete, and
  PASS vocabulary. It proves copied Phase 8 lineage, TTBR0 materialized-root
  provenance, TTBR1 descriptor-image kernel-root provenance, TCR/MAIR
  compatibility records, blocked SCTLR/ASID/TLB/barrier states, active-root
  nonmutation, kernel diagnostic reachability, model-only activation-commit
  intent, rollback/teardown, deterministic rejection, and zero live side
  effects.
- Evidence level: static documentation/source inspection of the accepted live
  translation-register activation contract and source inventory plus adjacent
  Phase 8 QEMU/substitute smoke-plan patterns.
...
- Consequences: The next objective task is the queued
  phase8-live-translation-register-activation-core-20260531 if dependencies
  remain satisfied. Live register mutation, active-root descriptor copy,
  ASID/TLB/barrier execution, lower-EL ERET, scheduler runnable publication,
  process lifecycle, startup ABI expansion, descriptor-backed filesystem
  syscalls, Pi 5 hardware proof, boot archive publication, shell, networking,
  SSH, RP1/PCIe, UART interrupt ownership, and DMA/cache-driver policy remain
  blocked until later explicit tasks accept their contracts and gates.
...
- Status: accepted as the documentation-only Milestone 8.3 source inventory
  for the live translation-register activation frontier. No Rust behavior,
  ...
  syscalls, writable filesystem, networking, SSH, RP1/PCIe, UART interrupt
  ownership, or DMA/cache-driver policy was added.
- Context: The accepted live descriptor-image installation closeout
  established a model-only installation-ready binding between
  KernelHalfDescriptorImage and LiveAddressSpaceActivationPlan below any live
  translation-register sequence. Supervisor planning selected a bounded
  inventory before any contract or implementation can admit register
  activation.
- Decision: Accept
  phase8-live-translation-register-activation-source-inventory-20260531. The
  inventory distinguishes accepted model-only TTBR0 materialized-root
  provenance, TTBR1 descriptor-image kernel-root provenance, compatibility-only
  TCR/MAIR records, blocked SCTLR/ASID/TLB states, planned-only no-live
  DSB/ISB, kernel-owned fault-reporting reachability, and zero live side
  effects from unaccepted live TTBR0_EL1/TTBR1_EL1/TCR_EL1/MAIR_EL1/
  SCTLR_EL1 mutation, active-root copy, live ASID/TLB/barrier sequencing,
  lower-EL launch, scheduler publication, process lifecycle, filesystem
  syscalls, and Pi 5 proof.
- Evidence level: static documentation/source inspection of accepted live
  activation, kernel-half reachability, kernel-half descriptor-image, and live
  descriptor-image installation docs/task records plus retained
  QEMU/substitute installation evidence at
  tasks/evidence/2026-05-31-qemu-live-descriptor-image-installation-smoke-core/qemu-live-descriptor-image-installation-smoke.log.
- Validation: git status --short before edits was clean except durable
  ...
  mdbook build passed; git diff --cached --check passed before commit.
- Consequences: The next objective task is the queued
  phase8-live-translation-register-activation-contract-20260531 if
  dependencies remain satisfied. Live register mutation, active-root
...
- Status: accepted as the documentation-only Milestone 8.3 QEMU/substitute
  smoke plan for the live descriptor-image installation frontier. No Rust
  behavior, assembly behavior, QEMU execution, Pi 5 hardware run, boot archive
  publication, hardware-lock acquisition, live TTBR0_EL1/TTBR1_EL1/TCR_EL1/
  ...
  networking, SSH, RP1/PCIe, UART interrupt ownership, or DMA/cache-driver
  policy was added.
- Context: The accepted live descriptor-image installation contract selected a
  model-level installation-ready activation binding between the accepted
  non-installed KernelHalfDescriptorImage and the accepted
  LiveAddressSpaceActivationPlan, below any live translation-register
  mutation.
- Decision: Accept
  phase8-qemu-live-descriptor-image-installation-smoke-plan-20260531. The
  plan defines qemu_live_descriptor_image_installation_smoke, retained
  evidence path
  tasks/evidence/2026-05-31-qemu-live-descriptor-image-installation-smoke-core/qemu-live-descriptor-image-installation-smoke.log,
  classification qemu-live-descriptor-image-installation-smoke-complete, and
  PASS vocabulary. It requires copied descriptor-image/activation lineage,
  TTBR0/TTBR1 provenance without register writes, preserved
  coverage/permissions/diagnostics, installation-ready binding observations,
  deterministic no-partial-install rejection, idempotent teardown, and zero
  live side effects.
- Evidence level: static documentation inspection of the accepted live
  descriptor-image installation contract/source inventory, adjacent Phase 8
  contracts and QEMU/substitute smoke-plan patterns, roadmap, SUMMARY, and
  ADR index.
- Validation: git status --short before edits was clean except durable
  supervisor state promotion outside the Talos repo; git diff --check passed;
  mdbook build passed; git diff --cached --check passed before commit.
- Consequences: The next objective task is the queued
  phase8-live-descriptor-image-installation-core-20260531 if dependencies
  remain satisfied. Live register mutation, active-root descriptor copy,
  ASID/TLB/barrier activation, lower-EL ERET, scheduler runnable publication,
  process lifecycle, startup ABI expansion, descriptor-backed filesystem
  syscalls, Pi 5 hardware proof, shell, networking, SSH, RP1/PCIe, UART
...
- Status: accepted as the Milestone 8.3 QEMU/substitute evidence for the
  non-installed KernelHalfDescriptorImage boundary. No Pi 5 hardware run, boot
  archive publication, hardware-lock acquisition, live TTBR0_EL1/TTBR1_EL1/
  TCR_EL1/MAIR_EL1/SCTLR_EL1 mutation, ASID allocation, live TLB mutation,
  ...
  DMA/cache-driver policy was added.
- Context: The accepted smoke plan selected
  qemu_kernel_half_descriptor_image_smoke and required retained evidence for
  copied input lineage, non-installed TTBR1 descriptor-image records,
  deterministic rejection, teardown, and zero live side effects after the core
  implementation commit 3e0e836.
- Decision: Accept
  phase8-qemu-kernel-half-descriptor-image-smoke-core-20260531. The task adds
  the QEMU-only scenario wiring, retained script, and evidence log proving
  phase8-kernel-half-descriptor-image-v1 with policy
  ttbr1-shared-privileged-kernel-root-descriptor-image-v1. The evidence
  records TTBR0 materialized-root provenance, model-owned TTBR1 kernel-root
  image intent, required kernel coverage, privileged-only normal/device
  descriptor attributes, root/table ownership, no-partial-image errors,
  idempotent teardown, and zero live activation side effects.
- Evidence level: QEMU/substitute boot smoke, unit tests, static inspection,
  and documentation validation. Retained evidence path:
  tasks/evidence/2026-05-31-qemu-kernel-half-descriptor-image-smoke-core/qemu-kernel-half-descriptor-image-smoke.log.
- Validation: cargo fmt --all -- --check passed; cargo -Zjson-target-spec
  test passed with 324 no_std tests;
  scripts/qemu-kernel-half-descriptor-image-smoke.sh passed and retained the
  required final qemu-kernel-half-descriptor-image-smoke-complete/PASS lines;
  git diff --check passed; mdbook build passed; git diff --cached --check
  passed before commit.
- Consequences: The next bounded task should be
  phase8-kernel-half-descriptor-image-closeout-checkpoint-20260531. Live
...
- Status: accepted as the documentation-only Milestone 8.3 QEMU/substitute
  kernel-half descriptor-image smoke plan. No Rust behavior, assembly
  behavior, QEMU execution, Pi 5 hardware run, boot archive publication,
  hardware-lock acquisition, live TTBR0_EL1/TTBR1_EL1/TCR_EL1/MAIR_EL1/
  ...
  networking, SSH, RP1/PCIe, UART interrupt ownership, or DMA/cache-driver
  policy was added.
- Context: The accepted descriptor-image contract selected a non-installed
  KernelHalfDescriptorImage boundary and explicitly named this QEMU/substitute
  smoke plan as the next bounded evidence boundary before implementation.
- Decision: Accept
  phase8-qemu-kernel-half-descriptor-image-smoke-plan-20260531. The plan
  selects qemu_kernel_half_descriptor_image_smoke, retained evidence path
  tasks/evidence/2026-05-31-qemu-kernel-half-descriptor-image-smoke-core/qemu-kernel-half-descriptor-image-smoke.log,
  classification qemu-kernel-half-descriptor-image-smoke-complete, and PASS
  vocabulary. It requires success observations for copied input lineage,
  non-installed TTBR1 shared privileged kernel-root descriptor-image records,
  kernel coverage, privileged-only normal/device descriptor attributes,
  model-owned root/table leases, no-partial-image rollback, idempotent
  teardown, deterministic rejection cases, and zero live activation side
  effects.
- Evidence level: static documentation/source inspection. Reviewed the
  accepted descriptor-image contract commit a3bc161, descriptor-image source
  inventory, kernel-half reachability smoke-plan pattern,
...
- Validation: git status --short before edits was clean except durable
  ...
  mdbook build passed; git diff --cached --check passed before commit.
- Consequences: The next bounded task should be
  phase8-kernel-half-descriptor-image-core-20260531. Live register mutation,
  ASID/TLB/barrier activation, lower-EL ERET, scheduler runnable publication,
  process lifecycle, startup ABI expansion, descriptor-backed filesystem
  syscalls, Pi 5 hardware proof, shell, networking, SSH, RP1/PCIe, UART
  interrupt ownership, and DMA/cache-driver policy remain blocked until later
...
- Status: accepted as the documentation-only Milestone 8.3 kernel-half
  reachability closeout checkpoint. No Rust behavior, assembly behavior, QEMU
  ...
  filesystem syscalls, writable filesystem, networking, SSH, RP1/PCIe, UART
  interrupt ownership, or DMA/cache-driver policy was added.
- Context: The kernel-half reachability source inventory, contract,
  QEMU/substitute smoke plan, target-independent core, and retained
  QEMU/substitute smoke evidence were accepted. A closeout checkpoint was
  needed to reconcile the accepted frontier and deferred surfaces before any
  later Phase 8.3 task is selected.
- Decision: Accept
  phase8-kernel-half-reachability-closeout-checkpoint-20260531. The checkpoint
  records the accepted frontier as target-independent
  KernelHalfReachabilityPlan construction plus retained QEMU/substitute
  evidence for copied input lineage through LiveAddressSpaceActivationPlan,
  TTBR0 materialized-root provenance, selected TTBR1 shared privileged
  kernel-root policy, blocked descriptor-image construction, required
  privileged kernel reachability entries, compatibility-only TCR/MAIR records,
  blocked SCTLR/ASID/TLB/barrier/live-register state, deterministic no-partial
  rejection, idempotent plan-local teardown, zero live side effects,
  qemu-kernel-half-reachability-smoke-complete, and PASS.
- Evidence level: static documentation inspection of accepted commits
  1eda202, 7d95e0a, c1645d5, a4294f5, and edda81d plus retained
...
- Consequences: No explicit queued follow-up task remains. Supervisor planning
  ...
  Kernel-half descriptor-image construction, live translation-register
...
- Status: accepted as the documentation-only Milestone 8.3 kernel-half
  reachability contract. No Rust behavior, assembly behavior, QEMU execution,
  ...
  filesystem syscalls, writable filesystem, networking, SSH, RP1/PCIe, UART
  interrupt ownership, or DMA/cache-driver policy was added.
- Context: The accepted kernel-half reachability source inventory mapped the
  remaining blocked-no-accepted-kernel-half-map gap after model-only live
  address-space activation. A bounded contract was needed to select policy
  identity and evidence vocabulary before implementation or smoke evidence.
- Decision: Accept
  phase8-kernel-half-reachability-contract-20260531. The contract selects a
  preflight-only KernelHalfReachabilityPlan with identity
  phase8-kernel-half-reachability-plan-v1 and policy
  preflight-ttbr1-shared-kernel-root-reachability-v1. The chosen future
  direction keeps process-user mappings in TTBR0_EL1 and reserves TTBR1_EL1
  for a shared privileged kernel root, while descriptor-image construction and
  live register mutation remain blocked. The plan requires kernel
  text/rodata/data/bss, vectors, active stack, heap/page-frame allocator,
  UART/MMIO diagnostics, scheduler state, and panic/fault reporting
  reachability records; compatibility-only TCR/MAIR/TTBR/ASID/TLB/barrier
  vocabulary; deterministic blocker/error behavior; no-partial construction;
  idempotent plan-local teardown; and zero live side effects.
- Evidence level: static documentation/source inspection. Reviewed accepted
  kernel-half inventory, live activation docs, adjacent Phase 8 contracts,
  translation/vector/UART/MMIO/scheduler/fault-reporting source owners,
  architecture notes, roadmap, SUMMARY, and ADR index.
- Validation: git status --short before edits was clean; git diff --check
  passed; mdbook build passed; git diff --cached --check passed before
  commit.
- Consequences: The next bounded task should be
  phase8-qemu-kernel-half-reachability-smoke-plan-20260531. Kernel-half
...
- Status: accepted as the documentation-only Milestone 8.3 process-install
  ...
  hardware run, boot archive publication, hardware-lock acquisition,
  address-space mutation, user-frame allocation, page-table installation,
  lower-EL launch, argv/envp implementation, exec/spawn/wait, shell,
  descriptor-backed filesystem syscall, writable filesystem, persistent
  storage, networking, SSH, RP1/PCIe, UART interrupt ownership, or
  DMA/cache-driver policy was added.
- Context: The accepted process-install source inventory found that the
  program-loader frontier produces a ProgramImagePlan, but no source owner yet
  accepts process address-space lifetime, frame leases, page-table roots,
  install rollback, initial lower-EL frames, user stacks, descriptor
  inheritance, or scheduler handoff.
- Decision: Accept phase8-process-install-contract-20260530. The first
  process-install boundary is target-independent and metadata-only: a
  ProcessImageInstallPlan derived from a validated ProgramImagePlan, with
  ordered page records, exact UserText/UserData permissions, file-copy and
  zero-fill ranges, all-or-nothing semantics, deterministic errors, and
  explicit lower-EL launch and descriptor deferrals.
- Evidence level: static documentation/source inspection. Reviewed the
  accepted process-install source inventory, loader format contract, loader
  closeout, Phase 7 EL0/address-space and descriptor-table contracts,
  src/program_loader.rs, src/posix.rs, src/scheduler.rs, roadmap, SUMMARY, and
  ADR index.
- Validation: git status --short before edits was clean; git diff --check
  passed; mdbook build passed; git diff --cached --check passed before
  commit.
- Consequences: The next bounded task should be
  phase8-qemu-process-install-smoke-plan-20260530, documentation-only, to
  specify QEMU/substitute evidence for the metadata-only install-plan
  boundary. Process-install implementation, physical page allocation,
  page-table mutation, lower-EL launch of the loaded image, argv/envp
  construction, exec/spawn/wait, shell, descriptor-backed filesystem syscalls,
  writable filesystems, persistent storage, networking, SSH, RP1/PCIe, UART
  interrupt ownership, and DMA/cache-driver policy remain blocked.
...
- Status: accepted as the documentation-only Milestone 8.1 read-only
  initramfs/VFS QEMU/substitute smoke plan. No Rust behavior, assembly
  behavior, QEMU run, Pi 5 hardware run, boot archive publication,
  hardware-lock acquisition, VFS implementation, initramfs parser,
  descriptor-backed filesystem read, ELF/program loader, argv/envp setup,
  process creation, shell behavior, networking, SSH, RP1/PCIe, UART interrupt
  ownership, or DMA/cache-driver policy was added.
- Context: The accepted read-only initramfs/VFS contract required a
  deterministic QEMU/substitute smoke plan before target-independent core
  implementation and before any runtime evidence claim.
- Decision: Accept phase8-readonly-initramfs-vfs-smoke-plan-20260530. The
  plan defines the qemu_readonly_initramfs_vfs_smoke scenario, deterministic
  fixture contents, exact lookup/read/offset/EOF observations, required
  ENOENT/ENOTDIR/EISDIR/ENAMETOOLONG/EBADF/EFAULT/EINVAL/ENOTSUP cases, exact
  PASS/classification lines, retained evidence path, failure classification,
  and regression gates.
- Evidence level: static documentation inspection. Reviewed the accepted
  read-only initramfs/VFS contract, Phase 8 source inventory, roadmap, ADR
  index, and prior QEMU smoke plan patterns for exact output, evidence
  retention, and deferred-surface language.
- Validation: git status --short before edits was clean; git diff --check
  passed; mdbook build passed; git diff --cached --check passed before
  commit.
- Consequences: The next mechanically unblocked task is
  phase8-readonly-initramfs-vfs-core-20260530. The QEMU/substitute evidence
  task remains dependency-gated behind the accepted core. Pi 5 hardware proof,
  boot archive publication, ELF/program loading, argv/envp setup, process
  creation, shell, networking, SSH, RP1/PCIe, UART interrupt ownership,
  DMA/cache-driver policy, writable filesystems, and persistent storage remain
  blocked until later explicit tasks accept their contracts and gates.
...
## 2026-05-29 - Phase 7.4 Process Descriptor Table Closeout Accepted

- Status: accepted as the documentation-only Milestone 7.4 process descriptor
  table closeout. No Rust behavior, assembly behavior, QEMU rerun, Pi 5
  hardware run, archive publishing, hardware-lock acquisition, stdin/read,
  close/dup/read syscall behavior, process loading, VFS/filesystem behavior,
  shell behavior, networking, SSH, RP1/PCIe, UART interrupt ownership, or
  DMA/cache-driver policy was added.
- Context: The accepted process descriptor table contract, core, smoke plan,
  and QEMU/substitute smoke core established the first ProcessOwnerId-backed
  inherited-stdio descriptor-table path and retained QEMU evidence for
  lower-AArch64 talos_write fd 1/fd 2 through runtime-console0.
- Decision: Accept
  phase7-process-descriptor-table-closeout-checkpoint-20260529. The closeout
  records the accepted task/commit matrix, retained evidence path,
  classification=qemu-process-descriptor-stdio-smoke-complete, PASS line,
  validation gates, residual risks, and deferred surfaces.
- Evidence level: static evidence and documentation inspection. No new
  QEMU/substitute or physical Pi 5 evidence was produced because this task
  changes only Markdown documentation and durable worker state.
- Validation: git status --short before edits was clean; git diff --check
  passed; mdbook build passed.
- Consequences: The process-owned inherited-stdio descriptor-table slice is
  accepted only at the QEMU/substitute evidence level. Pi 5 physical
  descriptor-table proof, stdin/read, close/dup/read, descriptor lifetime and
  close semantics, process loading, VFS/filesystem, shell, networking, SSH,
  RP1/PCIe, UART interrupt ownership, DMA/cache-driver policy, and full POSIX
  ...
  close-semantics source inventory.
...
- Decision: Add `scripts/rpi5-tftp-cursor.sh` and require it, or an equivalent `/tftp/logs?max_bytes=1048576&limit=1` call with `cursor` omitted, to capture the pre-run TFTP EOF cursor before controlled hardware tests. Also use `scripts/rpi5-wait-tftp-delta.sh <cursor>` after power-cycle because `/serial/observe` can return after the first serial burst before the Pi reaches the network/TFTP phase. Treat prior stale or empty deltas as an evidence-collection flaw, not as proof the hardware tests did or did not fetch the current archive. This entry supersedes the earlier `cursor=0` wording; the lab-controller tail-mode contract is omission of `cursor`, not a synthetic cursor value.
...
- Risks: If the TFTP log grows past 1 MiB and omitted-cursor tail mode ever stops returning the real EOF cursor, add an explicit lab API cursor endpoint rather than relying on any synthetic cursor workaround.
...
## 2026-05-30 - Phase 8 Initial Process Launch Contract Accepted

- Status: accepted as the documentation-only Milestone 8.3 initial process
  launch contract. No Rust behavior, assembly behavior, QEMU execution, Pi 5
  hardware run, boot archive publication, hardware-lock acquisition,
  TTBR/TCR/MAIR/SCTLR write, ASID allocation, live TLB invalidation,
  lower-EL ERET, initial user stack implementation, argv/envp, process
  lifecycle, scheduler runnable publication, shell behavior,
  descriptor-backed filesystem syscalls, writable filesystem, networking, SSH,
  RP1/PCIe, UART interrupt ownership, or DMA/cache-driver policy was added.
- Context: The accepted source inventory showed that the next launch boundary
  must make entry/SP provenance, activation, saved-frame intent, scheduler
  publication, and QEMU/substitute evidence explicit before any implementation
  or lower-EL launch attempt.
- Decision: Accept phase8-initial-process-launch-contract-20260530. The
  contract selects phase8-initial-process-launch-plan-v1 as a
  target-independent launch-preparation record. It consumes the accepted
  ...
  UserText descriptor coverage, records
  user_sp_state=blocked-missing-initial-user-stack, records
  activation_state=blocked-no-ttbr-activation, defines saved-frame intent
  without register writes, and requires no-partial-launch/no-runnable
  publication behavior for commit requests.
- Evidence level: static documentation/source inspection. Reviewed the
  accepted launch inventory, process page-table materialization contract and
  closeout, process address-space and process-install contracts, loader,
  install, address-space, materialization, POSIX, AArch64 lower-EL and
  exception helpers, translation register vocabulary, scheduler owner
  placeholders, roadmap, SUMMARY, and ADR index.
- Validation: git status --short before edits was clean; git diff --check
  passed; mdbook build passed; git diff --cached --check passed before
  commit.
- Consequences: The next bounded task should be
  phase8-qemu-initial-process-launch-smoke-plan-20260530. Implementation,
  QEMU execution, TTBR activation, lower-EL ERET to /bin/init, initial user
