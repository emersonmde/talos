...
- Decision: Add `scripts/rpi5-tftp-cursor.sh` and require it, or an equivalent `/tftp/logs?cursor=0&max_bytes=1048576&limit=1` call, to capture the pre-run TFTP EOF cursor before controlled hardware tests. Treat prior stale deltas as an evidence-collection flaw, not as proof the hardware tests did or did not fetch the current archive.
- Required validation: The helper must pass shell syntax validation and return the same current EOF cursor as the expanded TFTP log query before it is used as a hardware-test gate.
