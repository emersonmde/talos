...
- src/arch/aarch64/gicv2.rs currently exposes PPI/SPI enable, acknowledge,
  EOI, enable/pending/active inspection, highest-pending reads, CPU-interface
  enablement, distributor enablement, SGI priority setup, and a minimal
  GICD_SGIR target-list write.

GICv2 SGIs occupy INTIDs 0 through 15. Talos exposes only the GICD_SGIR offset
0xf00 surface needed by the diagnostic. The accepted QEMU proof uses SGI INTID
1 and target-list filter 0, with CPU target-list bits matching logical CPU IDs:
logical CPU 1 uses bit 0x02, CPU 2 uses bit 0x04, and CPU 3 uses bit 0x08.
The sender records the selected SGI ID, target-list bit, and SGIR value. The
receiver acknowledges through GICC_IAR, classifies the observed INTID as the
diagnostic SGI, writes the exact IAR value to GICC_EOIR, and publishes bounded
per-core receive and EOI counters after the handler returns.

The first proof is QEMU-only and is accepted by
`scripts/qemu-cross-core-ipi-delivery-smoke.sh`. It starts secondary cores
through the accepted PSCI path, sends one diagnostic SGI from CPU 0 to each
