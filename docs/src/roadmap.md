...
129. Pi 5 generated-root command-input direct-read harness core: accepted in
    ...
    drivers, networking, SSH, Phase 11/12 expansion, and phase transition
    remain deferred to explicit tasks.
130. Pi 5 generated-root command-input direct-read Pi 5 proof: accepted in
    'phase10-pi5-generated-root-command-input-direct-read-pi5-proof-20260617'
    with terminal classification 'command-input-command0-prelude-blocked'. The
    selected candidate reached same-boot firmware-initramfs valid-artifact
    source evidence, ready command=0, a visible talos> prompt, selected-tree
    identity, stable same-power-cycle TFTP evidence for
    da591740/kernel_2712.img and da591740/initramfs_2712, final pre-restore
    identity, and restore proof. The first failing invariant is command 0:
    after a fresh pre-write read and successful /serial/write of rootinfo, the
    ...
    Generated-root command-input acceptance, persistence, storage drivers,
    networking, SSH, Phase 11/12 expansion, and phase transition remain
...
141. Pi 5 serial capture readiness source contract: accepted in
    'phase10-pi5-serial-capture-readiness-source-contract-20260617' with
    classification
    'serial-capture-readiness-contract-guard-core-selected'. The accepted
    command0 source-response retention closeout left command0 non-evaluable
    because serial readiness/capture did not retain a usable generated-root
    ready command=0 boundary: the first candidate retained stale later-command
    readiness with fresh_after_prompt=false and zero command0 bytes, while the
    known-good control and candidate rerun retained only early firmware/RP1
    bytes under the long-settle direct-read strategy. The selected follow-up is
    'phase10-pi5-serial-capture-readiness-guard-core-20260617', a local/static
    guard over readiness/capture evidence before any hardware retry. It must
    preserve command0-source-response-retention-guard-v2 as the later command0
    transaction gate. Generated-root command-input success, command0
    response-generation failure, persistence, storage-driver work, networking,
    SSH, Phase 11/12 expansion, and phase transition remain rejected.
142. Pi 5 serial capture readiness guard core: accepted in
    'phase10-pi5-serial-capture-readiness-guard-core-20260617' with
    classification 'serial-capture-readiness-guard-core-local-static'. The
    proof-review helper now enforces serial-capture-readiness-guard-v1 before
    the retained command0-source-response-retention-guard-v2 transaction gate:
    same-boot firmware-initramfs valid-artifact ready command=0 text, a visible
    prompt, and a fresh command 0 pre-write boundary are required. Task-owned
    fixtures prove the positive evaluable window and reject early-firmware-only
    capture, stale later-command readiness, dispatch-only metadata, and
    tail-only source response. The selected follow-up is
    'phase10-pi5-serial-capture-readiness-pi5-proof-20260617'. Hardware
...
154. Pi 5 serial command 0 post-write observe contract: accepted in
    'phase10-pi5-serial-command0-post-write-observe-contract-20260617' with
    classification
    'command0-post-write-observe-contract-helper-core-selected'. The contract
    keeps the feature boundary at command0 serial write delivery and separates
    /serial/write byte acceptance from command-loop delivery. The v2 proof
    already accepted selected-kernel/TFTP agreement, same-boot readiness,
    visible prompt, fresh pre-write boundary, and a 9-byte rootinfo write, but
    its post-write direct-read window retained zero bytes. Because the
    lab-controller contract says /serial/observe with a saved cursor is the
    preferred endpoint after write, the selected discriminator is a
    cursor-bound post-write observe helper/core task:
...
157. Pi 5 serial command 0 post-write observe closeout: accepted in
    ...
    storage, networking, SSH, Phase 11/12 expansion, or phase transition.
158. Pi 5 serial command 0 saturated-capture source contract: accepted in
    'phase10-pi5-serial-command0-saturated-capture-source-contract-20260617'
    with classification
    'command0-saturated-capture-source-contract-guard-core-selected'. The first
    failing invariant remains command0 write delivery after selected-kernel/TFTP
    agreement: /serial/write accepted 9 bytes for rootinfo, but cursor-bound
    /serial/observe started from saturated cursor 4194304 and retained zero
    readiness or post-write command0 bytes. The task compares endpoint/capture
    repair, command-indexed saturated direct-read fallback, and explicit pause.
    The selected follow-up is
    'phase10-pi5-serial-command0-saturated-capture-guard-core-20260617', a
    local/static guard for deadline-loop-direct-read-after-saturated-cursor
    evidence. That guard must reject empty saturated observe windows,
    write-accepted-only evidence, stale pre-write output,
    stale later-command-only output, unordered command0 fragments, and
    source-response-only evidence. Command0 write-delivery success, command0
    source-response retention success, generated-root command-input success,
...
180. Pi 5 command0 readiness timeout-boundary closeout: accepted in
    'phase10-pi5-command0-readiness-timeout-boundary-closeout-20260618'
    with classification
    'command0-readiness-timeout-boundary-closeout-command0-delivery-blocked-planning-needed'.
    The closeout reconciles that selected-kernel/TFTP and immediate/final
    selected identity passed in the accepted rerun, but command0 input delivery
    remains unaccepted because the fresh command=0 write boundary was missing
    or stale before the write. The retained readiness/pre-write output had
    advanced through empty input timeouts to command=3, and post-write evidence
    reached command=4 without ordered rootinfo, dispatch command=0
    status=handled, responses=1, or ready command=1 from a saved command=0
    boundary. selected_next_task is null and planningNeeded=true;
    source-response retention v3, generated-root command-input success,
    storage, networking, SSH, Phase 11/12 expansion, and phase transition
    remain unaccepted.
...
183. Pi 5 command0 live write-window closeout: accepted in
    'phase10-pi5-command0-live-write-window-closeout-20260618' with
    classification
    'command0-live-write-window-closeout-command0-delivery-blocked-planning-needed'.
    The closeout reconciles the accepted Pi 5 proof: selected-kernel/TFTP,
    fresh live command=0 boundary, immediate rootinfo write, stable
    immediate/final pre-restore selected identity, and restore proof passed,
    but ordered post-write rootinfo, dispatch command=0 status=handled,
    responses=1, and ready command=1 evidence was not retained. Command0 input
    delivery remains unaccepted. selected_next_task is null and
...
188. Pi 5 serial command0 source-response retention v3 after input delivery:
    accepted as a blocked proof in
    'phase10-pi5-serial-command0-source-response-retention-v3-after-input-delivery-20260617'
    with classification 'command0-source-response-retention-v3-blocked'. The
    serialized hardware run preserved the accepted command0 input-delivery
    boundary again: selected 208984-byte kernel_2712.img identity, stable
    same-power-cycle TFTP serves, firmware-initramfs valid-artifact readiness,
    9-byte rootinfo write, ordered command0 line/dispatch/responses=1 evidence,
    ready command=1, final selected identity, and baseline restore proof. It
    did not retain the required source-response fragments for the same command0
    boundary: the retained command0 output starts mid-source-response at
    'path=/generated/manifest.txt exec-path=/generated/status7' before line,
    dispatch, and ready markers. Source-response retention, generated-root
    command-input success, storage, networking, SSH, Phase 11/12 expansion, and
...
211. Phase 12.3 ARP request emission core: accepted in
    'phase12-network-arp-request-emission-core-20260619' with classification
    'phase12-network-arp-request-emission-core-accepted'. The accepted
    src/network.rs frontier adds build_outbound_arp_request, a pure
    caller-buffered helper that writes a complete Ethernet/IPv4 ARP request
    frame for a local endpoint and target IPv4. Tests cover deterministic
    broadcast destination MAC, local source MAC, ARP EtherType, Ethernet/IPv4
    ARP fields, sender MAC/IP, zero target MAC, target IPv4, exact frame
    length, too-small output rejection before partial construction, and
    composition with unresolved outbound neighbor resolution without ARP cache
    mutation. Retry timers, neighbor-discovery state, packet queues, routing,
    driver transmit, live packet I/O, sockets, SSH, ping/network reachability,
...
236. Phase 12.3 userspace ping operation substitute smoke closeout: accepted
    in
    'phase12-network-userspace-ping-operation-substitute-smoke-closeout-20260619'
    with classification
    'phase12-network-userspace-ping-operation-substitute-smoke-closeout-accepted'.
    The closeout reconciles the retained substitute smoke transcript with the
    accepted UserspacePingOperation contract, SinglePingPacketService, fake
    NetworkDevice behavior, and caller-owned buffers. The accepted evidence
    remains host-only: one operation can drive unresolved ARP to echo-reply
    completion and demonstrate status observation, retry exhaustion, explicit
    timeout, duplicate/active busy, and receive/transmit IO error mapping
    through retained QEMU/substitute evidence. The boundary is still not bound
    to a real descriptor object or syscall ABI. Shell ping, public sockets,
    live driver adapters, live packet I/O, hardware reachability, SSH,
...
243. Phase 12.3 network runtime device pump core: accepted in
    'phase12-network-runtime-device-pump-core-20260620' with classification
    'phase12-network-runtime-device-pump-core-accepted'. src/network.rs now
    exposes NetworkRuntimeDevicePump, a host-only caller-driven runtime/service
    boundary over NetworkDevice. It owns fixed-capacity local ARP and ping
    operation state, receives one frame into caller-owned storage per pump call,
    prioritizes local ARP/ICMP response generation, and offers non-reply frames
    to one selected active NetworkPingOperationDescriptor. Tests cover no-frame,
    nonlocal/no-reply, local ARP reply transmit, local ICMP echo reply
    transmit, active ping ARP-to-ICMP advancement, echo-reply completion,
    receive-buffer pressure, receive error, local and active transmit errors,
    retry, timeout, terminal status observation, and responder-vs-active
    ordering. Shell ping, public sockets, stable syscall ABI acceptance, socket
    syscall ABI, live driver adapters, live packet I/O, hardware reachability,
    SSH, smoltcp, UDP/TCP, lab mutation, boot publication, Phase 12.1 retry,
    Phase 12.4 socket expansion, and phase transition remain rejected.
    selected_next_task is
...
248. Phase 12.3 network runtime ping syscall substitute smoke closeout:
    accepted in
    'phase12-network-runtime-ping-syscall-substitute-smoke-closeout-20260620'
    with classification
    'phase12-network-runtime-ping-syscall-substitute-smoke-closeout-accepted'.
    The closeout reconciles the retained smoke transcript, smoke script,
    accepted RuntimePingOperationSyscallSubstitute source boundary, task
    record, docs, durable state, and rejected claims. The accepted evidence
    level remains host/QEMU-substitute only over
    ...
    UserspacePingOperation, SinglePingPacketService, fake/trait-level
    NetworkDevice behavior, caller-owned receive/transmit/status buffers, and
    fixed-capacity state. It accepts only retained fake-device smoke evidence:
    ...
    completed status, close, local ARP and ICMP reply dispatch while a
    descriptor is open, retry exhaustion, explicit timeout, invalid descriptor,
    closed descriptor, zero-capacity, busy-open, receive IO error, local
    transmit IO error, and active-ping transmit IO error. Shell ping, public
    sockets, stable syscall ABI acceptance, socket syscall ABI, live driver
...
- 'phase10-pi5-command0-selected-kernel-precondition-closeout-20260617'
  ...
  unaccepted.
- 'phase10-pi5-serial-command0-lab-boundary-after-precondition-recovery-closeout-20260617'
  accepts the after-precondition-recovery lab-boundary closeout with
  classification
  command0-lab-boundary-after-precondition-closed-final-identity-regressed-planning-needed.
  The Pi 5 proof retained selected-kernel/TFTP precondition evidence and
  ordered prearmed command0 serial delivery in the same run, but command0 input
  delivery remains unaccepted because the final pre-restore identity regressed
  to the baseline tree before explicit restore. selected_next_task is null and
  planningNeeded=true; source-response retention v3, generated-root
  command-input success, storage, networking, SSH, Phase 11/12 expansion, and
  phase transition remain unaccepted.
...
- 'phase10-pi5-command0-final-identity-regression-pi5-proof-20260618'
  ...
  unaccepted.
- 'phase10-pi5-command0-final-identity-regression-closeout-20260618'
  accepts the final-identity regression closeout with classification
  command0-final-identity-closeout-command0-delivery-blocked-planning-needed.
  The closeout reconciles that selected-kernel/TFTP and immediate/final
  selected identity passed in the retained proof, but command0 input delivery
  remains unaccepted because ordered command0 serial delivery failed:
  rootinfo reached command=4 rather than command=0. selected_next_task is null
...
- 'phase10-pi5-command0-readiness-timeout-boundary-pi5-proof-20260618'
  ...
  phase10-pi5-command0-readiness-timeout-boundary-closeout-20260618.
- 'phase10-pi5-command0-readiness-timeout-boundary-closeout-20260618'
  accepts the readiness timeout-boundary closeout with classification
  command0-readiness-timeout-boundary-closeout-command0-delivery-blocked-planning-needed.
  The closeout keeps selected-kernel/TFTP and selected identity as passing
  evidence for the retained rerun, but command0 input delivery remains
  unaccepted because the fresh command=0 write boundary was missing or stale
  before write and ordered command0 delivery did not follow the write.
  selected_next_task is null and planningNeeded=true; source-response
  retention v3, generated-root command-input success, storage, networking,
  SSH, Phase 11/12 expansion, and phase transition remain unaccepted.
- 'phase10-pi5-command0-live-write-window-core-20260618' accepts the
  local/static live write-window discriminator. It rejects retained output
  already advanced past command=0 before rootinfo is written, delayed writes,
  unordered delivery, and the retained readiness-timeout-boundary rerun. The
  selected follow-up is
  phase10-pi5-command0-live-write-window-pi5-proof-20260618; command0 input
  ...
  unaccepted.
- 'phase10-pi5-command0-live-write-window-pi5-proof-20260618' accepts the
  serialized Pi 5 live write-window proof as blocked. Selected-kernel/TFTP,
  fresh command=0 boundary, immediate rootinfo write, immediate/final
  pre-restore selected identity, and restore proof passed, but post-write
  serial retained no ordered command0 delivery evidence. The selected follow-up
  is phase10-pi5-command0-live-write-window-closeout-20260618.
- 'phase10-pi5-command0-live-write-window-closeout-20260618' accepts the
  closeout with command0 input delivery still unaccepted. selected_next_task is
  null and planningNeeded=true; source-response retention v3, generated-root
...
- 'phase10-pi5-command0-timeout-stable-command-index-core-20260618' accepts
  the local/source timeout-stable command-index core. Empty no-data timeout at
  command=0 now holds the Pi 5 proof command index instead of terminally
  advancing command0, emits bounded proof-visible `timeout-hold` markers, and
  fails through `timeout-hold-exhausted` rather than waiting silently. Normal
  completed command input still dispatches through descriptor-backed
  stdin/stdout and advances once. The selected follow-up is
  phase10-pi5-command0-timeout-stable-command-index-pi5-proof-20260618;
  command0 input delivery, source-response retention, generated-root
  command-input success, storage, networking, SSH, Phase 11/12 expansion, and
  phase transition remain unaccepted.
...
- 'phase10-pi5-serial-command0-source-response-retention-v3-after-input-delivery-20260617'
  accepts a blocked serialized Pi 5 proof. The run retained selected-kernel/TFTP
  identity, firmware-initramfs readiness, the accepted rootinfo command0
  delivery shape, final selected identity, and restore proof, but it did not
  retain source=firmware-initramfs reason=valid-artifact for the same command0
...
the next bounded source/static write-target contract. It selects exactly one
Ethernet-private clock-manager register: CLK_ETH_TSU_CTRL for
RP1_CLK_ETH_TSU / rp1_eth tsu_clk. The Talos register address is
0x1c00018134, derived from observed RP1 base 0x1c00000000, clock-manager base
offset 0x018000, and retained Linux CLK_ETH_TSU_CTRL offset 0x00134. The only
future real-candidate write is idempotent: pre-read the 32-bit raw value,
write that same pre_raw back, post-read, restore-write pre_raw, and
restore-read. The contract preserves the full raw value, enable bit, auxsource
bits, source bits, and reserved bits; rejects shared RP1_CLK_SYS pclk/hclk
writes, CLK_ETH_CTRL, div/sel/PLL/frequency-counter and GPCLK output-enable
writes, reset-controller, GPIO32/PHY reset, MDIO/PHY, DMA, descriptors,
interrupts, packet I/O, networking, sockets, SSH, Phase 12.2, and phase
...
phase12-rp1-ethernet-clk-eth-ctrl-write-restore-core-20260610 and
phase12-rp1-ethernet-clk-eth-ctrl-write-restore-closeout-20260610 accept the
local/static CLK_ETH_CTRL candidate/control report surface only. The core
records the exact 0x1c00018064 target, pre-read-raw-only write rule,
write/readback/restore/readback sequence, preserved-field inventory, safety
invariants, retained risks, paired no-clock-write/no-Ethernet control, and
deterministic rejections for shared-clock/runtime/downstream claims. The
closeout closes same-shaped local/static retries and selects the serialized
Pi 5 candidate/control write/restore proof as the next bounded task, with
hardwareTestLock, selected-tree identity, expected TFTP fetches, fresh serial
markers, final pre-restore identity, restore verification, archive reviews,
classification JSON, evidence map, and capture summary required. Runtime
clock/reset ownership, shared-clock ownership, reset-controller,
GPIO32/PHY, MDIO, DMA, descriptors, interrupts, packet I/O, networking,
sockets, SSH, Phase 12.2, and phase transition remain unaccepted.
...
that proof frontier as
rp1-ethernet-clk-eth-ctrl-write-restore-frontier-closed. Same-shaped
CLK_ETH_CTRL idempotent write/restore hardware retries are closed for this
candidate/control pair. The proof does not make a next ownership slice
mechanically objective by itself, so supervisor planning is required for any
different Phase 12.1 reset, GPIO32/PHY, MDIO/PHY, interrupt, DMA, descriptor,
packet, or functional-clock dependency. Broad clock/reset ownership,
shared-clock ownership, reset-controller, GPIO32/PHY, MDIO, DMA, descriptors,
interrupts, packet I/O, networking, sockets, SSH, Phase 12.2, and phase
transition remain unaccepted.
...
phase12-rp1-ethernet-mdio-mpe-enable-proof-closeout-20260611 closes this
frontier as rp1-ethernet-mdio-mpe-enable-ownership-frontier-closed.
Same-shaped NCR.MPE set/readback/restore hardware retries are closed for the
accepted candidate/control pair. Future MAN transaction, PHY-ID retry, PHY
reset, broad MDIO/PHY ownership, Ethernet driver behavior, packet I/O,
networking, sockets, SSH, Phase 12.2, or phase-transition work requires
supervisor planning with a qualitatively different discriminator and explicit
acceptance criteria.

phase12-rp1-ethernet-mdio-phy-id-after-mpe-source-contract-20260611 accepts a
source/docs/evidence contract for a corrected-target after-MPE MDIO PHY-ID
discriminator. The earlier PHY-ID no-write proof remains only a closed
wrong-target blocker: NCR/NSR/MAN at
0x1c00000000/0x1c00000008/0x1c00000034 did not accept a runtime MDIO
transaction, MAN write, or PHY-ID read. The corrected future targets are
observed-window NCR 0x1c00100000, NSR 0x1c00100008, and MAN 0x1c00100034 with
MACB_MID context 0x1c001000fc. A future candidate may only read corrected NCR,
require MPE bit 4 already set, poll NSR.IDLE bit 2, write MAN frames
0x600a0000 and 0x600e0000 for MII_PHYSID1/2, and extract MAN.DATA bits 15:0.
If corrected NCR.MPE is clear, it must perform no NCR or MAN write and
classify a precise precondition blocker. Runtime MDIO success, visible PHY-ID
...
identity mismatch for the autoneg candidate and records the zero-event
known-good baseline TFTP triage as an unresolved freshness issue. Earlier
minimal-sentinel and v4 register-vector proofs remain accepted comparators, but
they do not prove the current live capture path after the later recurrence. No
static helper repair was selected; the next bounded discriminator is
...
phase12-rp1-ethernet-post-physical-precondition-link-status-pi5-proof-20260614
is blocked with committed post-physical-link-status-source-precondition-blocker
evidence. Candidate/control capture identity, TFTP byte agreement, serial
nonce freshness, final pre-restore identity, and restore proof passed, and the
...
closes the hardware-proof frontier as
bcm54213pe-readonly-preflight-hw-proof-frontier-closed-candidate-fetch-blocker.
Accepted evidence is limited to the local/static proof-core target/control
shape and the serialized Pi 5 no-MDIO/no-Ethernet control proof. The candidate
hardware path remains blocked by no fresh TFTP events or serial output after
selected candidate publication and power-cycle, so MII_CTRL1000/MII_STAT1000
raw/decoded values, link readiness, PHY/MAC behavior, packet I/O, networking,
SSH, Phase 12.2, and phase transition remain unaccepted. Same-shaped
BCM54213PE read-only preflight hardware retries are closed for this
...
phase12-rp1-ethernet-bcm54213pe-autoneg-convergence-closeout-20260616 closes
that frontier as
bcm54213pe-autoneg-convergence-frontier-closed-timeout-link-not-ready. The
closed evidence accepts only one corrected-target PHY1 BMCR restart write frame
...
BMSR autoneg-complete false, and MACB_NSR_LINK false. The only mechanically
unblocked follow-up is the timeout source checkpoint
phase12-rp1-ethernet-bcm54213pe-post-convergence-timeout-source-checkpoint-20260616;
the link-ready packet-readiness checkpoint remains blocked. GPIO32/PHY reset
ownership, Broadcom selector/config writes, interrupt ownership, PHY/MAC
...
accepts tx-selected-register-read-visible. The serialized Pi 5 proof retained
control and candidate selected-tree identity, same-power-cycle TFTP byte
agreement, cursor-nonce serial freshness, final pre-restore identity, and
restore proof. The no-MDIO/no-Ethernet control constructed no target facts. The
candidate reached NCR 0x10 before/after, wrote the TX selector value 0x0c00
once, read the selected TX shadow register as raw 0x0e00, and stopped with
rx-delay-write-count=0x0, tx-delay-write-count=0x0, and bmcr-write-count=0x0.
This only closes the selected-register read visibility blocker; TX delay
...
reconciles the accepted local/static discriminator core and serialized Pi 5
proof without a new hardware run. The accepted frontier is limited to decisive
selected-tree/TFTP/serial/restore evidence, the paired no-MDIO/no-Ethernet
control, TX selector write completion, and selected TX shadow register read
visibility with raw 0x0e00. The queued post-TX selected-read source checkpoint
is the next mechanically dependency-satisfied boundary after this closeout is
committed. TX delay write/readback, BMCR restart, convergence polling, link
...
- The network runtime device pump core accepts a host-only caller-driven
  NetworkDevice pump that owns fixed-capacity local ARP and ping-operation
  state, receives exactly one frame into caller-owned storage, prioritizes
  local ARP/ICMP responder traffic, and advances one selected active ping
  descriptor through ARP resolution, ICMP transmit, echo-reply completion,
  retry, timeout, and terminal status observation. Shell ping, public sockets,
...
- The runtime ping syscall-substitute closeout reconciles the accepted
  RuntimePingOperationSyscallSubstitute implementation, source/unit
  QEMU-substitute validation, task record, docs, durable state, and rejected
  claims. The accepted evidence level remains host-only over
...
- The runtime ping syscall-substitute smoke closeout reconciles the retained
  smoke transcript, smoke script, accepted RuntimePingOperationSyscallSubstitute
  source boundary, task record, docs, durable state, and rejected claims. The
  accepted evidence level remains host/QEMU-substitute only over
  RuntimePingOperationSyscallSubstitute, NetworkRuntimeDevicePump, local
  ARP/ICMP responder behavior, active ping descriptor dispatch,
  UserspacePingOperation, SinglePingPacketService, fake/trait-level
...
- The process-local ping descriptor closeout reconciles the accepted contract,
  core implementation, source/unit evidence, task records, docs, durable state,
  and rejected claims. The accepted evidence level remains
  host/QEMU-substitute source/unit evidence over fake/trait-level NetworkDevice
  behavior, process-local descriptor ownership, DescriptorShapedPingControl,
  RuntimePingOperationSyscallSubstitute, NetworkRuntimeDevicePump,
  caller-owned receive/transmit/status storage, and fixed-capacity state. It
  covers open/start/pump-or-read-result/status, retry_arp, timeout, close,
  inherited-stdio descriptor allocation, invalid and closed descriptors,
  missing current owner, process descriptor capacity unwind, duplicate active
  operation, wrong-kind stdio descriptors, retry exhaustion, explicit timeout,
  receive IO error, and local transmit IO error. Shell ping, public sockets,
  stable syscall ABI acceptance, socket syscall ABI acceptance, live driver
  adapters, live packet I/O, hardware reachability, SSH, smoltcp, UDP/TCP, lab
...
- The process-local ping descriptor smoke closeout reconciles the accepted
  contract, core implementation, retained smoke evidence, task records, docs,
  durable state, and rejected claims. The accepted evidence level remains
  host/QEMU-substitute smoke evidence over fake/trait-level NetworkDevice
  behavior, process-local descriptor ownership, DescriptorShapedPingControl,
  RuntimePingOperationSyscallSubstitute, NetworkRuntimeDevicePump,
  caller-owned buffers, and fixed-capacity state. It covers one accepted
  ping-control lifecycle through the process-local descriptor path plus
  deterministic descriptor lifecycle and error controls: missing owner, full
  process descriptor table with backing-descriptor unwind, duplicate active
  open, wrong-kind stdio descriptor, closed descriptor, retry exhaustion,
...
- The process-local ping SVC dispatch smoke accepts retained
  host/QEMU-substitute evidence for
  ...
  state. The transcript covers open, start, pump_or_read_result, status,
  retry_arp, timeout, and close through the dispatch facade, including
  unresolved ARP, ARP-to-ICMP advancement, echo-reply completion, terminal
  status observation, invalid and closed descriptors, missing owner, process
  descriptor table capacity unwind, duplicate active operation, retry
  exhaustion, explicit timeout, receive-buffer pressure, receive IO error,
  local transmit IO error, and active transmit IO error. Shell ping, public
...
- The process-local ping SVC user-argument smoke closeout accepts the retained
  host/QEMU-substitute smoke evidence after reconciling the accepted contract,
  core implementation, source/unit evidence, retained transcript, task records,
  durable state, and rejected claims. The accepted evidence level remains
  host/QEMU-substitute smoke evidence over fake/trait-level NetworkDevice
  behavior, experimental user-argument decoding, UserMapping copy-in/copy-out,
  process-local descriptor ownership, internal dispatch-shaped control,
  ProcessLocalPingDescriptorControl, DescriptorShapedPingControl,
  RuntimePingOperationSyscallSubstitute, NetworkRuntimeDevicePump,
  caller-owned buffers, task-owned result/status slots, and fixed-capacity
  state. It accepts only retained smoke coverage for one process-local ping
  dispatch lifecycle plus deterministic selector, reserved-field, owner,
  descriptor, capacity, user-memory, buffer-pressure, scratch-pressure, TTL,
...
- The VFS-backed userspace ping diagnostic packet queue core accepts the
  crate-internal source implementation behind that contract. src/network.rs now
  provides PacketQueueNetworkDevice over fixed-capacity receive/transmit queues
  of bounded PacketQueueFrame records; VfsPingDiagnosticSvcFixture source/unit
  tests use it to record outbound ARP and IPv4/ICMP echo request frames, inject
  matching ARP/ICMP replies, complete result/status copy-out, and close the
  process-local descriptor. Deterministic controls cover transmit queue
  capacity, oversized injected frames, output-buffer pressure, malformed
  injected frames, explicit retry, timeout, receive/transmit IO errors, invalid
  descriptor, and unchanged SyscallNumber/TALOS_* vocabulary. The evidence is
  host/QEMU-substitute only over fake/trait-level NetworkDevice behavior,
  UserMapping copy-in/copy-out, caller-owned buffers, and task-owned diagnostic
  state. Shell ping, public sockets, stable/socket ABI acceptance, live driver
  adapters, live packet I/O, hardware reachability, SSH, lab mutation, boot
  publication, Phase 12.1 link-hardware retry, broad socket expansion, and phase
  transition remain rejected. The selected next bounded task is
...
- The shell-visible ping diagnostic smoke accepts retained host/QEMU-substitute
  evidence for `exec /bin/pingdiag` through VFS executable lookup, VFS
  ...
  queues, `PacketQueueNetworkDevice::pump_driver`, completed status/result
  copy-out, close, `waitpid`, and `laststatus`. The retained transcript
  covers successful ARP/ICMP progression plus malformed arguments, missing
  executable identity, owner/descriptor failures, invalid and closed
  ...
  close/drop behavior, and unchanged syscall vocabulary. Kernel fake commands,
  public sockets, stable/socket ABI acceptance, live driver adapters, live
  packet I/O, hardware reachability, SSH, smoltcp, UDP/TCP, lab mutation, boot
...
- The shell sockdiag open/close closeout accepts the socket-open frontier only
  at source/unit plus host/QEMU-substitute smoke level. The accepted boundary
  is shell-visible /bin/sockdiag execution through VFS executable
  lookup/open/read, startup ABI, TALOS_SOCKET_SYSCALL = 6 for
  AF_INET/SOCK_STREAM/protocol 0, process DescriptorObjectKind::Socket
  ownership, bounded socket backing state, TALOS_CLOSE_SYSCALL = 2 close/drop,
  waitpid, laststatus, deterministic negative controls, and unchanged
  /bin/pingdiag behavior. Send/recv, bind/connect/listen/accept,
  poll/blocking network I/O, UDP/TCP payload transport, live driver adapters,
