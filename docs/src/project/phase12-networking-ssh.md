...
The pipeline-output append regular-file redirection frontier checkpoint freezes
the accepted local-only direct path-form and fixed-/bin bare-name append
frontier without runtime feature changes. The accepted witnesses remain exactly
'/bin/stdout | /bin/stdin >/tmp/pipeline-report.txt' followed by
'/bin/stdout | /bin/stdin >>/tmp/pipeline-report.txt', and
'stdout | stdin >/tmp/pipeline-report.txt' followed by
'stdout | stdin >>/tmp/pipeline-report.txt'. Direct forms keep explicit program
paths; bare-name forms resolve only through bounded /bin lookup. Both forms keep
descriptor-backed VFS loading, accepted userspace launch/status, producer fd1 to
the pipe endpoint, consumer fd0 from that pipe endpoint, child-only consumer fd1
to 'volatile-vfs:/tmp/pipeline-report.txt', first-command truncate/sink
semantics, second-command append-at-EOF semantics, descriptor-backed readback of
two reports, shell fd1 restoration, and coherent process status observations.
Stderr pipeline redirection and append, input/combined pipeline redirections,
arbitrary paths, persistent storage, PATH/current-directory lookup, command
lookup beyond bounded /bin, generated-root retry, live network/SSH, Pi 5
hardware proof, and phase transition remain deferred. No later queued same-lane
...
The direct pipeline stderr append regular-file redirection core accepts exactly
'/bin/stdout | /bin/stderr 2>/tmp/pipeline-stderr.txt' followed by
'/bin/stdout | /bin/stderr 2>>/tmp/pipeline-stderr.txt'. Both stages load
through descriptor-backed VFS open/read and the accepted userspace
launch/status path. The producer records fd1 as the pipe endpoint; the
final-stage consumer records fd0 as that pipe endpoint and fd2 child-only as
'volatile-vfs:/tmp/pipeline-stderr.txt'. The first pipeline uses
truncate/sink semantics and the second records op=append at regular-file EOF.
Descriptor-backed 'cat /tmp/pipeline-stderr.txt' reads both stderr fixture
writes in order with bytes=0x3e, and a later normal '/bin/stderr' proves shell
fd2 restoration. Fixed-/bin bare-name pipeline stderr append, input/combined
pipeline redirections, stdout final-stage redirection for this pipeline shape,
arbitrary paths, persistent storage, generated-root retry, live networking/SSH,
Pi 5 hardware proof, and phase transition remain deferred. The next local
POSIX/VFS task is the queued direct pipeline stderr append closeout.
...
append semantics, descriptor-backed readback bytes=0x3e, shell fd2
restoration, and coherent process-status observations. Fixed-/bin bare-name
pipeline stderr append remains separate queued implementation work;
input/combined pipeline redirections, stdout final-stage redirection for this
pipeline shape, arbitrary paths, persistent storage, generated-root retry, live
networking/SSH, Pi 5 hardware proof, and phase transition remain deferred. The
next local POSIX/VFS task is the queued fixed-/bin bare-name pipeline stderr
append core.

The fixed-/bin bare-name pipeline stderr append regular-file redirection core
accepts exactly 'stdout | stderr 2>/tmp/pipeline-stderr.txt' followed by
'stdout | stderr 2>>/tmp/pipeline-stderr.txt'. Both stages resolve only
through the accepted bounded /bin lookup to /bin/stdout and /bin/stderr, then
...
truncate/sink semantics and the second records op=append at regular-file EOF.
Descriptor-backed 'cat /tmp/pipeline-stderr.txt' reads both stderr fixture
writes in order with bytes=0x3e, and a later normal 'stderr' proves shell fd2
restoration. Direct path-form pipeline stderr append remains a retained
regression control. Input/combined pipeline redirections, stdout final-stage
redirection for this pipeline shape, arbitrary paths, persistent storage,
environment-backed PATH, current-directory search, command lookup beyond
bounded /bin, generated-root retry, live networking/SSH, Pi 5 hardware proof,
...
task accepts the corresponding fixed-/bin bare-name append extension while
keeping live networking/SSH paused. The accepted stdout sequence is
'stdout >/tmp/talos-output-alpha.txt' followed by
'stdout >>/tmp/talos-output-alpha.txt'; the accepted stderr sequence is
'stderr 2>/tmp/talos-error-beta.log' followed by
'stderr 2>>/tmp/talos-error-beta.log'. Each command resolves only through the
bounded /bin lookup, launches descriptor-backed VFS/userspace, rebinds only
child fd1 or fd2, and uses descriptor-backed cat readback to prove two fixture
writes in truncate-then-append order. Direct path-form append remains retained
as a regression/control surface. PATH/current-directory lookup, command lookup
...
The
phase12-local-bare-name-combined-pipeline-stderr-append-redirection-core-20260628
task accepts the exact fixed-/bin bare-name truncate-then-append sequence
'stdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr-append.txt'
followed by
'stdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt'.
Both stages resolve only through bounded /bin lookup to /bin/stdin and
/bin/stderr before descriptor-backed VFS/userspace execution. The producer
keeps fd0 from initramfs:/etc/banner.txt and fd1 to the pipe endpoint; the
final-stage consumer keeps fd0 from that pipe endpoint, inherited fd1, and
child-only fd2 to volatile-vfs:/tmp/pipeline-combined-stderr-append.txt. The
...
fixture writes in order. Mixed direct/bare stage names, command lookup beyond
bounded /bin, arbitrary input/output paths, persistent storage, broad shell
...
The combined pipeline stdout append redirection frontier checkpoint freezes the
accepted local-only direct path-form and fixed-/bin bare-name witnesses without
adding runtime behavior. The accepted witnesses remain exactly
'/bin/stdin </etc/banner.txt | /bin/stdin >/tmp/pipeline-combined-append.txt',
'/bin/stdin </etc/banner.txt | /bin/stdin >>/tmp/pipeline-combined-append.txt',
...
persistent storage, environment-backed PATH, current-directory search, command
lookup beyond bounded /bin, arbitrary shell grammar, unbounded/concurrent
...
task keeps live networking/SSH paused while accepting the direct path-form
combined pipeline extension to safe caller-chosen volatile /tmp leaf output
paths. The accepted stdout sequence is
...
followed by
'/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/talos-pipeline-error-beta.log'.
Both preserve descriptor-backed VFS/userspace execution, serialized pipe
handoff, final-stage fd0 from the pipe, child-only fd1/fd2 volatile-vfs /tmp
leaf rebinding, append-at-EOF semantics, descriptor-backed readback,
lifecycle/status/process observations, and later descriptor restoration
controls. Fixed-/bin bare-name coverage is accepted by the following bounded
/bin lookup slice; persistence, broad path support, live networking/SSH, Pi 5
...
task accepts the corresponding fixed-/bin bare-name combined pipeline extension
while keeping live networking/SSH paused. The accepted stdout sequence is
...
stages resolve only through bounded /bin lookup before using the accepted
descriptor-backed VFS/userspace, pipe, volatile-file, process-status, and
restoration paths. Direct path-form bounded /tmp combined-pipeline output-path
append remains a retained control. Mixed direct/bare broadening, command lookup
beyond bounded /bin, paths outside volatile /tmp, nested/traversal paths,
persistence, broad shell grammar, live networking/SSH, Pi 5 hardware proof,
generated-root retry, boot publication, and phase transition remain deferred.

The
phase12-local-combined-pipeline-bounded-tmp-output-path-append-redirection-frontier-checkpoint-20260628
checkpoint freezes the accepted local-only direct path-form and fixed-/bin
bare-name combined pipeline bounded volatile /tmp output-path append frontier
without adding runtime behavior. The accepted direct and bare-name witnesses
remain the exact stdout/stderr truncate-then-append sequences targeting
volatile-vfs:/tmp/talos-pipeline-output-alpha.txt and
volatile-vfs:/tmp/talos-pipeline-error-beta.log. Both forms keep the accepted
descriptor-backed VFS/userspace execution path, producer fd0 from
initramfs:/etc/banner.txt, producer fd1 as the pipe endpoint, final-stage fd0
from that pipe endpoint, child-only final-stage fd1/fd2 volatile-vfs /tmp leaf
rebinding, append-at-EOF semantics, descriptor-backed readback of two fixture
writes, lifecycle/status/process observations, and descriptor restoration
controls. Persistent writable filesystem behavior, nested/traversal paths,
...
NETWORK output but no TALOS: selected-image-handoff-sentinel-v16 or later
Talos marker. The selected next task is
...
The accepted
phase12-local-combined-pipeline-bounded-tmp-output-path-append-redirection-frontier-checkpoint-20260628
task freezes that local-only bounded volatile /tmp combined-pipeline
output-path append frontier without adding runtime behavior. The accepted
direct path-form and fixed-/bin bare-name witnesses remain the exact
stdout/stderr truncate-then-append sequences targeting
volatile-vfs:/tmp/talos-pipeline-output-alpha.txt and
volatile-vfs:/tmp/talos-pipeline-error-beta.log. Both forms preserve
descriptor-backed VFS/userspace execution, serialized pipe handoff,
final-stage fd0 from the pipe endpoint, child-only final-stage fd1/fd2
volatile-vfs /tmp leaf rebinding, append-at-EOF semantics,
descriptor-backed readback of two fixture writes, lifecycle/status/process
observations, and later descriptor restoration controls. Persistent writable
filesystem behavior, nested/traversal paths, paths outside volatile /tmp,
separated redirection-token grammar, explicit alternate fd syntax, mixed
direct/bare broadening, PATH/current-directory lookup, command lookup beyond
bounded /bin, arbitrary shell grammar, live networking/SSH, Pi 5 hardware
proof, generated-root retry, boot publication, and phase transition remain
...
phase12-ssh-live-tcp-pi5-candidate-runtime-marker-preflight-v11-20260630
accepts blocked-candidate-tftp-capture. The serialized Pi 5 run published the
v11 runtime-marker candidate from source commit
55c6920fbe86b233ea82b7d764e18f595b1e50cb, with selected
da591740/kernel_2712.img at 152,176 bytes and SHA-256
d16c7d8dc44d0f8009a92e61f871be7713c360c162170167f5f3a8da379cd4c5. Lab
identity stayed stable at tree
d0419aa69f7e6dc7600122eeebdb94aa287eab49f863b677252f2ad1e8e3c42e before
restore. The repaired readiness helper observed both nonce-bearing
route-start and runtime-ready markers with kernel_main optional, but the
stable same-cursor TFTP delta contained zero events, so selected fetch identity
was not proved in the same helper-owned pre-restore TFTP window.
candidate-capture-ready remains rejected. The lab was restored to
phase12-ssh-v10-openssh-clean-pre-20260624T074100Z /
...
tree 61811ab93aafa897cf5a0c937bf5485b0abdd4f7c14d770d399393d98f3866c9,
selected da591740/kernel_2712.img at 87,432 bytes, SHA-256 fb501f73..., and
two stable same-cursor selected kernel serves. The fresh serial window
retained firmware NETWORK output but no TALOS: boot-contract-v18 marker,
kernel_main, or later Talos marker. Because identity, TFTP, final
pre-restore identity, serial freshness, and restore proof were decisive, no
inconclusive known-good control branch was required. The selected next task is
...
service success claim, ssh-ready=true, fake/kernel-backed command expansion,
broad shell work, or phase transition was performed. The closeout preserves
v24's selected-byte service for the v23 _start discriminator image, final
selected identity on tree dbe73980..., fresh serial capture with firmware
output, and restore proof to tree a0452458.... The first missing fact is
selected-kernel image entry itself: the fresh post-power serial window had zero
...
phase12-ssh-live-tcp-pi5-selected-normal-runtime-pre-entry-preflight-v32-20260701
accepts blocked-selected-normal-runtime-rust-entry-missing. The worker
published the v31-selected pre-entry archive under hardwareTestLock, then
restored the lab to phase12-ssh-v10-openssh-clean-pre-20260624T074100Z. The
primary run served da591740/kernel_2712.img twice at 152,144 bytes, retained
firmware NETWORK serial output, but retained zero occurrences of asm_start,
asm_pre_rust_entry, rust_entry, boot info parsed, target init, exceptions
ready, kernel_main, route-start, or runtime-ready markers. Required triage then
proved the serial path with a known-good production-timer control retaining
rpi5-production-timer-preemption: PASS, followed by an unchanged candidate
rerun that again served the selected image twice at 152,144 bytes and retained
no ordered pre-entry or normal-runtime marker family member. The first missing
fact is now: selected rpi5_ssh_service_smoltcp_runtime_ready Image is served
by TFTP, but Pi 5 serial does not retain even TALOS: asm_start from that
...
service success claim, ssh-ready=true, fake/kernel-backed command expansion,
broad shell work, or phase transition was performed. The closeout reconciles
v32 against the v26 rust_entry/UART10 selected-entry proof, the v30 selected
normal-runtime marker-missing boundary, and the v31 assembly pre-entry
discriminator contract. The accepted frontier is now precise: the earlier
45,400-byte selected-entry marker-loop image reaches rust_entry/UART10, while
the 152,144-byte selected normal-runtime pre-entry image is served by TFTP but
retains no TALOS: asm_start, TALOS: asm_pre_rust_entry, TALOS: rust_entry, or
later ordered marker. selected_next_task is null and planningNeeded=true
pending supervisor planning for the next bounded repair or discriminator;
packet-I/O/OpenSSH/generated-root retry, remote receipt,
compatibility/service readiness, ssh-ready=true, fake command expansion, broad
shell work, and phase transition remain blocked.
...
phase12-ssh-live-tcp-selected-normal-runtime-pre-rust-continuation-closeout-v36-20260701
accepts selected-normal-runtime-pre-rust-frontier-proved-supervisor-planning.
No hardware action, lab publication, boot snapshot mutation, Pi 5 power cycle,
packet-I/O, OpenSSH/generated-root retry, remote receipt, compatibility claim,
service success claim, ssh-ready=true, fake/kernel-backed command expansion,
broad shell work, or phase transition was performed. The closeout reconciles
v36 against v34 and v35: v34 proves the 152,144-byte selected normal-runtime
archive class reaches TALOS: asm_start on Pi 5, v35 defines the selected
pre-rust loop after CPACR setup, BSS clear, and stack setup, and v36 proves
that selected archive reaches TALOS: asm_pre_rust_entry 542 times with
selected-byte TFTP service and restore proof. The next unresolved continuation
fact is whether the selected 152,144-byte normal-runtime feature archive enters
rust_entry and later normal-runtime milestones. selected_next_task is null and
planningNeeded=true pending supervisor planning for the next bounded
...
a0452458391d0e398b7e17e0f068bb652235f666bf277d004e0e214626128d10. This proves
the selected normal-runtime archive can enter rust_entry on Pi 5; it does not
prove BootInfo parsing, target init, exceptions, kernel_main, packet-I/O,
OpenSSH/generated-root retry, remote receipt, compatibility/service readiness,
ssh-ready=true, fake command expansion, broad shell work, or phase transition.
selected_next_task is
...
phase12-ssh-live-tcp-selected-normal-runtime-rust-entry-continuation-closeout-v38-20260701
accepts selected-normal-runtime-rust-entry-frontier-proved-supervisor-planning.
No hardware action, lab publication, boot snapshot mutation, Pi 5 power cycle,
packet-I/O, OpenSSH/generated-root retry, remote receipt, compatibility claim,
service success claim, ssh-ready=true, fake/kernel-backed command expansion,
broad shell work, or phase transition was performed. The closeout reconciles
v38 against v34, v36, and v37: v34 proves the selected normal-runtime archive
class reaches TALOS: asm_start on Pi 5, v36 proves the selected pre-rust
handoff at TALOS: asm_pre_rust_entry, v37 defines the selected rust_entry
marker-loop contract, and v38 proves that selected 152,816-byte archive
reaches TALOS: rust_entry 208 times with selected-byte TFTP service and
restore proof. The next unresolved continuation fact is whether the selected
normal-runtime feature archive proceeds through BootInfo parsing, target init,
exceptions, kernel_main, and later runtime/service milestones.
selected_next_task is null and planningNeeded=true pending supervisor planning
for the next bounded continuation step; packet-I/O/OpenSSH/generated-root
...
phase12-ssh-live-tcp-selected-normal-runtime-bootinfo-continuation-reconciliation-v39-20260701
accepts selected-normal-runtime-bootinfo-discriminator-ready. No hardware
action, lab publication, boot snapshot mutation, Pi 5 power cycle, packet-I/O,
OpenSSH/generated-root retry, remote receipt, compatibility claim, service
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The reconciliation preserves
the v38 fact that selected-byte Pi 5 evidence reaches TALOS: rust_entry, then
adds a separate selected normal-runtime BootInfo marker-loop archive. The new
artifact emits TALOS: boot info parsed only after
BootInfo::from_aarch64_x0(dtb_pa) returns, keeps the selected runtime route
linked in the artifact, and explicitly withholds target init, exceptions,
kernel_main, route-start, runtime-ready, packet-I/O, service success,
ssh-ready, and phase-transition claims. The non-published v39 archive serves
da591740/kernel_2712.img at 152,880 bytes with SHA-256
62c285790a87b3ab8395aa9dcbd8167318506c940fe5a4f61e07371c0806486b and archive
SHA-256 23ba0d4dee7cde85e6b6ef914528f209c20cebf0edc022723a4bd1c84ea4cec5.
selected_next_task is
...
phase12-ssh-live-tcp-selected-normal-runtime-bootinfo-continuation-closeout-v40-20260701
accepts selected-normal-runtime-bootinfo-frontier-proved. No hardware action,
lab publication, boot snapshot mutation, Pi 5 power cycle, packet-I/O,
OpenSSH/generated-root retry, remote receipt, compatibility claim, service
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The closeout reconciles v40
against v34, v36, v38, and v39: v34 proves selected normal-runtime assembly
entry at TALOS: asm_start, v36 proves selected TALOS: asm_pre_rust_entry
handoff, v38 proves TALOS: rust_entry, v39 defines the selected BootInfo
marker-loop contract, and v40 proves the selected 152,880-byte archive reaches
TALOS: boot info parsed 1,826 times with selected-byte TFTP service and
restore proof. The next unresolved continuation facts are target init,
exceptions, kernel_main, route-start, runtime-ready, packet-I/O, remote
receipt, compatibility/service readiness, OpenSSH, ssh-ready=true, fake
command expansion, broad shell work, and phase transition. selected_next_task
is
...
OpenSSH/generated-root retry, remote receipt, compatibility claim, service
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The closeout reconciles v42
against v34, v36, v38, v40, and v41: v34 proves selected normal-runtime
...
parsed, v41 defines the selected target-init marker-loop contract, and v42
proves the selected 152,880-byte archive reaches TALOS: target init 3,006
times with selected-byte TFTP service and restore proof. The next unresolved
continuation facts are exceptions ready, kernel_main, route-start,
...
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The reconciliation preserves
the v42 selected-byte target-init fact, then adds a selected normal-runtime
exceptions-ready marker-loop archive. The artifact emits TALOS: exceptions
ready only after arch::aarch64::exceptions::init() returns and before
kernel_main, route-start, runtime-ready, packet-I/O, service success,
ssh-ready, or phase transition. The non-published v43 archive serves
da591740/kernel_2712.img at 152,880 bytes with SHA-256
...
exceptions-ready Pi 5 hardware preflight task exists in taskQueue; packet-I/O,
OpenSSH/generated-root retry, remote receipt, compatibility/service readiness,
ssh-ready=true, fake command expansion, broad shell work, hardware action, and
phase transition remain blocked until supervisor planning adds that task.
...
remote receipt, compatibility claim, service success claim, ssh-ready=true,
fake/kernel-backed command expansion, broad shell work, or phase transition was
performed. The worker recomputed archive SHA-256
72c28bafe9bedd1474fd1dfb19db101e9078122506db1b6f13bbbebdad383f19 and exact tar
member ./da591740/kernel_2712.img SHA-256
2f89f98cf9403ccee58c20f8014f3c5fd83accb2f99da2f35b4b1eec6928cdc5 directly from
target/tmp/selected-normal-runtime-kernel-main-v45.tar.gz. The archive review
retained the kernel_main marker-loop contract with required marker
TALOS: kernel_main capture-nonce=runtime-marker-route-static, and the successor
capture helper dry-run targets da591740/kernel_2712.img at 152,896 bytes with
...
and planningNeeded=false. Later facts remain unproved: selected Pi 5
kernel_main marker retention, route-start, runtime-ready, packet-I/O, OpenSSH
...
capture-nonce=runtime-marker-route-static 1,794 times. The lab was restored to
a0452458391d0e398b7e17e0f068bb652235f666bf277d004e0e214626128d10. This proves
the selected normal-runtime archive reaches boot::rpi5::kernel_main on Pi 5;
it does not prove route-start, runtime-ready, packet-I/O, OpenSSH
compatibility, remote receipt, service readiness, ssh-ready=true, fake command
expansion, broad shell work, or phase transition. selected_next_task is
...
normal-runtime frontier remains route-start-retained because the v52 candidate
rerun served da591740/kernel_2712.img at the 104,136-byte baseline size, final
pre-restore identity was the accepted baseline tree, and serial retained zero
occurrences of TALOS: ssh-service-smoltcp-runtime-ready
...
OpenSSH/generated-root retry, remote receipt, compatibility claim, service
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The closeout reconciles v55
against the accepted v54 discriminator and v55 Pi 5 evidence: selected
post-power identity, selected same-window TFTP byte service, and selected final
pre-restore identity are no longer the missing facts. Runtime-ready remains
blocked because the selected candidate serial window retained zero occurrences
of TALOS: ssh-service-smoltcp-runtime-ready
capture-nonce=runtime-ready-static. The selected normal-runtime frontier
therefore remains route-start-retained; packet-I/O, OpenSSH compatibility,
...
OpenSSH/generated-root retry, remote receipt, compatibility claim, service
success claim, ssh-ready=true, fake/kernel-backed command expansion, broad
shell work, or phase transition was performed. The worker refreshed the
current-source selected normal-runtime pre-rust marker-loop archive from commit
dc7f065f3fe6df3abfa874ddc65c47a5d371e2c6. The archive is
target/tmp/selected-normal-runtime-pre-rust-v61.tar.gz with SHA-256
6e7a35f4d875a510719ca8fbdb256f6513d8d0b1eb6c5e321e198b75f8878cd9; the
selected da591740/kernel_2712.img is 152,144 bytes with SHA-256
90c72361bc67be8933436ddc5e6807dc127a8cb329a3fcab49404c10f8086d59 and Image
header text_offset=0, header_image_size=152144, flags=12, magic=ARMd. Static
disassembly shows TALOS: asm_start emitted once, then CPACR setup, BSS clear,
stack setup, and a loop on TALOS: asm_pre_rust_entry before the later
rust_entry branch. selected_next_task is
...
phase12-ssh-live-tcp-selected-normal-runtime-rust-entry-after-pre-rust-closeout-v64-20260702
accepts selected-normal-runtime-rust-entry-frontier-proved. No hardware action,
lab publication, boot snapshot mutation, Pi 5 power cycle, BootInfo parsing
proof, target-init proof, exceptions proof, kernel_main proof, route-start
proof, runtime-ready proof, packet-I/O, OpenSSH/generated-root retry, remote
receipt, compatibility claim, service success claim, ssh-ready=true,
fake/kernel-backed command expansion, broad shell work, or phase transition was
performed by the closeout. The closeout reconciles v64 against the accepted v63
discriminator and v64 Pi 5 evidence: selected post-power identity, selected
same-window TFTP byte service, selected final pre-restore identity,
marker-family serial observation, and restore proof are no longer missing
facts. The selected candidate reached TALOS: rust_entry 208 times. BootInfo
parsing, target init, exceptions, kernel_main, route-start, runtime-blocked,
and runtime-ready remain blocked because the same window retained zero
occurrences of those later markers. The first missing fact is now after TALOS:
rust_entry and before the next explicit post-rust-entry normal-runtime
...
phase12-ssh-live-tcp-selected-normal-runtime-post-rust-entry-continuation-reconciliation-v65-20260702
accepts selected-normal-runtime-post-rust-entry-discriminator-ready. No hardware
action, lab publication, boot snapshot mutation, Pi 5 power cycle, target-init
...
reconciled the v64 rust_entry frontier against current source and helper state,
then materialized a fresh selected BootInfo marker-loop archive from the current
tree. The archive is target/tmp/selected-normal-runtime-bootinfo-v65.tar.gz with
SHA-256 68a3e9356753c66b646477880f786fc10a01b021bd8758d19484f409df81ad9d;
selected da591740/kernel_2712.img is 152,880 bytes with SHA-256
87bbaab6842cbd83c1dff548d81151af6f9ff5309236b7ba65481174560987a8. Static
review proves TALOS: boot info parsed is embedded with negative claims for
...
hardwareTestLock, the worker published the v65 selected BootInfo archive and
ran the serialized Pi 5 preflight. Static review identified archive SHA-256
68a3e9356753c66b646477880f786fc10a01b021bd8758d19484f409df81ad9d and selected
da591740/kernel_2712.img as 152,880 bytes with SHA-256
87bbaab6842cbd83c1dff548d81151af6f9ff5309236b7ba65481174560987a8. The first
candidate served the selected kernel twice and retained TALOS: boot info parsed
184 times, but serial freshness rejected the identity join. The required
triage then captured a known-good control with 13 TFTP events and a candidate
rerun with decisive selected identity, two selected TFTP serves, final
pre-restore selected identity, restore proof, and 192 TALOS: boot info parsed
occurrences. The same selected serial window did not retain a separate TALOS:
rust_entry line, so this remains an inconclusive BootInfo preflight rather than
a selected-normal-runtime-bootinfo-marker-retained proof. selected_next_task is
...
compatibility claim, service success claim, ssh-ready=true,
fake/kernel-backed command expansion, broad shell work, or phase transition was
performed. The worker reconciled v64 rust_entry proof, the v65 selected
BootInfo archive/source contract, and v66 selected TFTP plus BootInfo marker
evidence. Static source review shows the selected
...
TALOS: boot info parsed. The v66 candidate rerun retained that marker 192
times with selected TFTP byte service, so the selected normal-runtime BootInfo
frontier is proved by lineage even though the v66 same-window serial window did
not retain a separate TALOS: rust_entry line. selected_next_task is
...
compatibility claim, service success claim, ssh-ready=true,
fake/kernel-backed command expansion, broad shell work, or phase transition was
performed. The contract reconciles the accepted local/static smoltcp
descriptor-delivery frontier with the accepted v71 selected exceptions-ready
Pi 5 frontier. The first live TCP packet-ingress prerequisite is now the
missing source-level RP1 Ethernet hardware frame-provider binding into the
accepted DriverPacketAdapter/smoltcp/listener/descriptor-delivery path, not a
generic kernel_main marker. v72 remains deferred, v60 remains blocked/deferred,
...
hardwareTestLock, the worker published the v76 route-entry archive, staged
selected tree 051125b2e9111036f7b5310634078a1e4e673bacbd53f4d05803f0882a460a70,
and captured a serialized Pi 5 run. Stable same-cursor TFTP before restore
retained 13 events, including two selected da591740/kernel_2712.img serves at
155,096 bytes, and final pre-restore identity still matched the selected tree.
Serial used the saturated-cursor direct-read fallback, retained 4,475 bytes
with two firmware NETWORK markers, and retained no kernel_main, provider
route-start, runtime-blocked, or runtime-ready marker. The first missing fact
is now before kernel_main after selected kernel TFTP service and final selected
identity. The lab restored to
phase12-ssh-v10-openssh-clean-pre-20260624T074100Z with tree
a0452458391d0e398b7e17e0f068bb652235f666bf277d004e0e214626128d10.
selected_next_task is
...
phase12-ssh-live-tcp-rp1-provider-route-entry-closeout-v77-20260702 accepts
provider-route-entry-frontier-blocked. No code implementation, lab publication,
boot snapshot mutation, Pi 5 power cycle, serial capture, TFTP capture,
packet-I/O proof, remote receipt, OpenSSH/generated-root retry, compatibility
claim, service success claim, ssh_ready=true, fake/kernel-backed command
expansion, broad shell work, or phase transition was performed. The closeout
reconciles v76 and v77: the source route has an accepted provider route-entry
marker and fail-closed provider runtime markers, but the selected Pi 5 run
served the v76 kernel and retained final selected identity without retaining
kernel_main, provider route-start, runtime-blocked, or runtime-ready markers.
The first missing fact is selected Pi 5 serial retention of kernel_main after
selected kernel TFTP service and final pre-restore selected identity. v72, v60,
and v53 remain deferred or blocked unless explicitly reselected with refreshed
...
remote receipt, OpenSSH/generated-root retry, compatibility claim, service
success claim, ssh_ready=true, fake/kernel-backed command expansion, broad shell
work, or phase transition was performed. The non-published candidate archive is
...
src/target/rpi5.rs now requires the source-owned RP1 DMA RX descriptor/ring
handoff and bounded packet-stimulus contract before emitting the runtime-ready
candidate marker. The archive review gate now requires descriptor/ring owner,
canonical redaction, bounded packet-stimulus contract, nonce strategy, timing
