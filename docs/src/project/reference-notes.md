...
- The reduced Cargo-linked assembly-only target also failed to print. With `TALOS_RPI5_CARGO_ASM_UART_PROOF=1`, the normal Cargo link still produced an 83,304-byte Image and `_start` wrote marker `CA` with the successful RP1 readback-flush UART sequence before parking, bypassing CPACR, BSS, stack, exceptions, and `rust_entry`. Hardware served `da591740/kernel_2712.img` twice but serial showed only NUL/newline. This moves the next useful comparison to the first executable page / link composition bytes of the Cargo-linked Image versus the standalone linker-layout assembly proof that printed `TL`.
