...
- Phase 11's first mapping contract records the general pcie2/RP1 translation as `cpu_phys = 0x1f_0000_0000 + (rp1_bus - 0xc0_4000_0000)` and selects the read-only RP1 UART0 PL011 flag register at CPU physical `0x1f_0003_0018` as the first stable register-read diagnostic target.
...
- A direct comparison between the Cargo/Rust image and the standalone linker-layout assembly proof showed matching PT_LOAD headers, segment offsets, load addresses, section start addresses, and final 83,304-byte Image size. The only expected layout difference was executable content: the Rust image's `.text.boot` was 336 bytes versus the linker-layout proof's 272 bytes, with Rust-owned code beginning at `0x81000`. Inlining the successful UART sequence directly at the normal Rust `_start` still did not print `BI`: hardware served the 83,304-byte Rust image twice, then serial showed only NUL/newline before the snapshot restore. This narrows the next useful experiment to a reduced Cargo-linked assembly-only target or deeper first-page/executable-content comparison, not another boot-tree, size, or section-address change.
- The reduced Cargo-linked assembly-only target was corrected to repeat its UART marker loop instead of writing a single marker and parking. With `TALOS_RPI5_CARGO_ASM_UART_PROOF=1`, the normal Cargo link still produced an 83,304-byte Image and `_start` wrote repeated `CL` markers with the successful RP1 readback-flush UART sequence before CPACR, BSS, stack, exceptions, or `rust_entry`. Hardware served `da591740/kernel_2712.img` twice and serial printed repeated `CL`/carriage-return-shaped marker output. This proves Cargo/Rust link composition and first executable page layout can reach the Pi 5 UART when the early marker loop is retained; the next boundary is the transition from early assembly marker to BSS/stack/Rust console output.
- A staged transition diagnostic with `S0` before RP1 pinmux, `S1` after pinmux, `S2` after CPACR, `S3` after BSS, and `S4` after stack currently reaches only `S0` on hardware. The first attempt accidentally looped in `S0` because the nested assembly helper did not preserve LR; after fixing LR preservation and making `S0` UART10-only before pinmux, hardware still served the image and printed `S0` only. This points to the staged diagnostic's pre-pinmux marker/pinmux transition, not Cargo link entry, as the next small boundary to simplify.
- Simplifying the transition diagnostic to one pre-pinmux `S0` write followed by the same RP1 pinmux/readback sequence and a repeated post-pinmux `S1` loop still did not reach `S1` on hardware. The image was served twice, but fresh serial advanced only by NUL plus a partial `S`. This suggests the custom transition marker path is still not matching the successful `CL` proof closely enough, so the next diagnostic should reuse the exact `rpi5_write_marker`/literal shape from the successful proof around the pinmux boundary before reintroducing staged helper calls.
