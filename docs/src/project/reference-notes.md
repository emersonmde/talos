...
- The `asm-uart-proof-firmware-address` variant uses the same no-`kernel_address` Image placement as the successful reset discriminator. The direct firmware-preserved RP1 UART0 form served the 128-byte image but produced no `TA` marker, so the current proof adds only the Linux-derived RP1 GPIO14/GPIO15 UART0 mux writes before direct UART writes at `0x1f00030000`.
