...
- The `asm-uart-proof-firmware-address` variant uses the same no-`kernel_address` Image placement as the successful reset discriminator. The direct firmware-preserved RP1 UART0 form served the 128-byte image but produced no `TA` marker. The follow-up minimal RP1 GPIO14/GPIO15 mux form served the 208-byte image and reached BL31 notices, but still produced no `TA`. The `asm-uart-then-reset-firmware-address` proof then wrote `TA` once through firmware-preserved RP1 UART0 and immediately invoked PSCI reset; repeated TFTP boot sequences proved execution continued past the UART stores. The current failure is therefore localized to UART visibility/routing/clock/reset/register semantics, not BL33 entry or a hard abort on the direct UART store.
