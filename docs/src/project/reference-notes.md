...
- The `asm-uart-proof-firmware-address` variant keeps the current assembly-only firmware-preserved RP1 UART0 marker proof but uses the same no-`kernel_address` Image placement as the successful reset discriminator. Its hardware run served the 128-byte image but produced no `TA` marker, so firmware-selected Image placement alone is not enough for UART first light.
