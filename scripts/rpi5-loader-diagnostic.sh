...
obj_file="$out_dir/rpi5_loader_diagnostic.o"
elf_file="$out_dir/rpi5_loader_diagnostic.elf"
bin_file="$out_dir/rpi5_loader_diagnostic.img"

mkdir -p "$out_dir"

clang -target aarch64-none-elf -ffreestanding -fno-stack-protector \
    -c src/arch/aarch64/rpi5_loader_diagnostic.S -o "$obj_file"
...
if [ ! -s "$bin_file" ]; then
    echo "raw loader diagnostic image is empty: $bin_file" >&2
    exit 1
fi
