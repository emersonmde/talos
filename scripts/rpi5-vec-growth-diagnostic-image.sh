...
for name in $(env | sed -n 's/^\(TALOS_RPI5_[A-Za-z0-9_]*\)=.*/\1/p'); do
    if [ "$name" = "TALOS_RPI5_VEC_GROWTH_DIAGNOSTIC_PAD_SIZE" ]; then
        continue
    fi
    unset "$name"
done

base_img="$(env TALOS_RPI5_VEC_GROWTH_DIAGNOSTIC=1 ./scripts/rpi5-image.sh)"
img_file="target/aarch64-talos-rpi5-bcm2712/debug/kernel_2712-vec-growth-diagnostic.img"
target_size="${TALOS_RPI5_VEC_GROWTH_DIAGNOSTIC_PAD_SIZE:-}"
