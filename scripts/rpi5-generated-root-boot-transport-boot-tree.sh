#!/bin/sh
set -eu

if [ "$#" -ne 2 ]; then
    echo "usage: $0 <pi-firmware-boot-source> <output-boot-tree>" >&2
    exit 2
fi

SOURCE_DIR="$1"
OUTPUT_DIR="$2"
SERIAL_PREFIX="da591740"
INITRAMFS_FILE="initramfs_2712"

./scripts/rpi5-boot-tree.sh "$SOURCE_DIR" "$OUTPUT_DIR" >/dev/null

KERNEL_IMG="$(./scripts/rpi5-generated-root-boot-transport-image.sh)"
cp "$KERNEL_IMG" "$OUTPUT_DIR/kernel_2712.img"
cp "$KERNEL_IMG" "$OUTPUT_DIR/kernel8.img"

artifact_tool="target/generated-root-artifact"
rustc tools/generated-root-artifact.rs -o "$artifact_tool"
"$artifact_tool" a "$OUTPUT_DIR/$INITRAMFS_FILE"

if ! grep -qx "initramfs $INITRAMFS_FILE followkernel" "$OUTPUT_DIR/config.txt"; then
    printf '%s\n' "initramfs $INITRAMFS_FILE followkernel" >>"$OUTPUT_DIR/config.txt"
fi

mkdir -p "$OUTPUT_DIR/$SERIAL_PREFIX"
for file in config.txt cmdline.txt bcm2712-rpi-5-b.dtb kernel_2712.img kernel8.img "$INITRAMFS_FILE"; do
    cp "$OUTPUT_DIR/$file" "$OUTPUT_DIR/$SERIAL_PREFIX/$file"
done

if [ -d "$OUTPUT_DIR/overlays" ]; then
    mkdir -p "$OUTPUT_DIR/$SERIAL_PREFIX/overlays"
    find "$OUTPUT_DIR/overlays" -maxdepth 1 -type f -exec cp {} "$OUTPUT_DIR/$SERIAL_PREFIX/overlays/" \;
fi

find "$OUTPUT_DIR" -type f | sort
