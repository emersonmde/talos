...
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-open path=/etc/banner.txt fd=3 object=regular-file reference=0 ok=true" "$LOG_FILE"
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-read path=/etc/banner.txt fd=3 offset-before=0 request=64 result=24 offset-after=24 data=\"Talos initramfs fixture\\n\" ok=true" "$LOG_FILE"
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-read path=/etc/banner.txt fd=3 offset-before=24 request=64 result=0 offset-after=24 eof=true ok=true" "$LOG_FILE"
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-open path=/bin/init fd=3 object=regular-file reference=0 ok=true" "$LOG_FILE"
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-read path=/bin/init fd=3 offset-before=0 request=4 result=4 offset-after=4 elf-magic=7f454c46 ok=true" "$LOG_FILE"
grep -Fq "qemu-readonly-initramfs-vfs-smoke: descriptor-read path=/empty fd=3 offset-before=0 request=64 result=0 offset-after=0 eof=true ok=true" "$LOG_FILE"
...
grep -Fq "qemu-readonly-initramfs-vfs-smoke: final participants=9 expected=9 errors=0 classification=qemu-readonly-initramfs-vfs-smoke-complete" "$LOG_FILE"
