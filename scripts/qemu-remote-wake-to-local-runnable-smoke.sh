...
LOG_FILE="target/qemu-remote-wake-to-local-runnable-smoke.log"

script_dir="$(CDPATH= cd "$(dirname "$0")" && pwd)"
. "$script_dir/qemu-nographic-smoke-lib.sh"

talos_qemu_prepare_image "qemu_remote_wake_to_local_runnable" "debug" "" "$@"
...
grep -q "qemu-remote-wake-to-local-runnable: local receiver=1 state-before=blocked state-after=runnable woke-task=201 local-runnable-len=1 duplicate-local-rejected=true ok=true" "$LOG_FILE"
grep -q "qemu-remote-wake-to-local-runnable: local receiver=2 state-before=blocked state-after=runnable woke-task=202 local-runnable-len=1 duplicate-local-rejected=true ok=true" "$LOG_FILE"
grep -q "qemu-remote-wake-to-local-runnable: local receiver=3 state-before=blocked state-after=runnable woke-task=203 local-runnable-len=1 duplicate-local-rejected=true ok=true" "$LOG_FILE"
...
grep -q "qemu-remote-wakeup-request: final participants=3 expected=3 errors=0 .* classification=qemu-remote-wake-to-local-runnable-complete" "$LOG_FILE"
