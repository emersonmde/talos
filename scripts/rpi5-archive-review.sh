...
if [ "$forced_kernel_address_allowed" = false ] && grep -q '^kernel_address=' "$extract_dir/config.txt"; then
    echo "config.txt should let Pi 5 firmware choose arm64 Image placement" >&2
    exit 1
fi
