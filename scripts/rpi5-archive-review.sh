...
if [ "$firmware_address_reset_proof" = false ] && [ "$firmware_address_uart_proof" = false ] && ! grep -qx 'kernel_address=0x80000' "$extract_dir/config.txt"; then
...
elif { grep -qx 'talos_loader_diagnostic=asm-uart-proof' "$extract_dir/config.txt" || grep -qx 'talos_loader_diagnostic=asm-uart-proof-firmware-address' "$extract_dir/config.txt" || grep -qx 'talos_loader_diagnostic=asm-entry-reset-proof' "$extract_dir/config.txt" || grep -qx 'talos_loader_diagnostic=asm-entry-reset-firmware-address' "$extract_dir/config.txt"; } && [ "$magic" = "ARMd" ]; then
    if [ "$header_image_size" != "$image_size" ]; then
