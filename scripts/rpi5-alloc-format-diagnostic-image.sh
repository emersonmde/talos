...
for name in $(env | sed -n 's/^\(TALOS_RPI5_[A-Za-z0-9_]*\)=.*/\1/p'); do
    if [ "$name" = "TALOS_RPI5_ALLOC_FORMAT_PAD_SIZE" ]; then
        continue
    fi
    unset "$name"
done

base_img="$(env TALOS_BOOT_SCENARIO=rpi5_alloc_format ./scripts/rpi5-image.sh)"
img_file="target/aarch64-talos-rpi5-bcm2712/debug/kernel_2712-alloc-format-diagnostic.img"
target_size="${TALOS_RPI5_ALLOC_FORMAT_PAD_SIZE:-}"
...
if [ -n "$target_size" ]; then
    ...
    if [ "$image_size" -gt "$target_size" ]; then
        echo "alloc format diagnostic image exceeds padded target: image=$image_size target=$target_size" >&2
        exit 1
    fi
