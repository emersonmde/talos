...
ARCHIVE="$1"
RESULT_MARKER="TALOS: rp1-endpoint-config-identity-result"

./scripts/rpi5-archive-review.sh "$ARCHIVE" >/dev/null

tmp_root="target/tmp"
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-rp1-endpoint-config-identity-read-review.XXXXXX")"
...
for required in \
    "rpi5-rp1-endpoint-config-identity-read: start" \
    ...
    "rp1-endpoint-config-id-inconclusive-capture"; do
    if ! grep -Fq -- "$required" "$kernel_strings"; then
        echo "kernel image missing endpoint config identity string: $required" >&2
        exit 1
    fi
done
...
for forbidden in \
    ...
    if grep -Fq -- "$forbidden" "$kernel_strings"; then
        echo "kernel image contains forbidden endpoint config identity string: $forbidden" >&2
        exit 1
    fi
done
