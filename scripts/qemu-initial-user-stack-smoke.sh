...
talos_qemu_prepare_image "qemu_initial_user_stack_smoke" "debug" "" "$@"
...
grep -Fq "qemu-initial-user-stack-smoke: success output=InitialUserStackPlan published=true stack-top=0x0000800000000000 initial-sp=0x00007fffffffffd0 sp-aligned-16=true ok=true" "$LOG_FILE"
...
grep -Fq "qemu-initial-user-stack-smoke: ownership usable-user-data=true stack-owned=true guard-has-frame=false guard-has-descriptor=false total-copied-bytes=42 total-zeroed-bytes=0x4000 ok=true" "$LOG_FILE"
grep -Fq "qemu-initial-user-stack-smoke: startup state=minimal-argc1-argv0-init-empty-envp argc=1 argv=nonnull argv0=/bin/init argv0-ptr=0x00007ffffffffff0 envp=null envp-state=empty-envp0 envp-entries=0 envp0-ptr=0x00007fffffffffe8 auxv=blocked-pending-startup-abi tls=blocked-pending-startup-abi copied-startup-bytes=42 ok=true" "$LOG_FILE"
grep -Fq "qemu-initial-user-stack-smoke: launch-binding user-sp-state=model-only-initial-user-stack-ready saved-frame-sp-el0=0x00007fffffffffd0 activation-state=blocked-no-ttbr-activation no-partial-launch=true ok=true" "$LOG_FILE"
