...
grep -Fq "qemu-initial-user-stack-smoke: ownership usable-user-data=true stack-owned=true guard-has-frame=false guard-has-descriptor=false total-copied-bytes=34 total-zeroed-bytes=0x4000 ok=true" "$LOG_FILE"
grep -Fq "qemu-initial-user-stack-smoke: startup state=minimal-argc1-argv0-init argc=1 argv=nonnull argv0=/bin/init argv0-ptr=0x00007fffffffffe8 envp=null auxv=blocked-pending-startup-abi tls=blocked-pending-startup-abi copied-startup-bytes=34 ok=true" "$LOG_FILE"
