...
tmp_root="target/tmp"
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-rp1-ethernet-macb-nsr-link-readonly-control-review.XXXXXX")"
...
for required in \
    "rpi5-rp1-ethernet-macb-nsr-link-readonly-control: start" \
    ...
    "observed-window-rp1-eth-base=withheld" \
    ...
    "macb-nsr-offset=withheld macb-nsr-link-bit=withheld" \
    "macb-nsr-raw=withheld macb-nsr-link=withheld" \
    "macb-nsr-result-valid=false" \
    ...
    "classification=no-mmio-no-ethernet-macb-nsr-link-control"; do
    if ! grep -Fq -- "$required" "$kernel_strings"; then
        echo "kernel image missing MACB NSR_LINK control string: $required" >&2
        exit 1
    fi
done
...
if [ -n "$CAPTURE_NONCE" ]; then
    if ! grep -Fq -- " capture-nonce=" "$kernel_strings"; then
        echo "kernel image missing MACB NSR_LINK control capture nonce label" >&2
        exit 1
    fi
    if ! grep -Fq -- "$CAPTURE_NONCE" "$kernel_strings"; then
        echo "kernel image missing MACB NSR_LINK control capture nonce: $CAPTURE_NONCE" >&2
        exit 1
    fi
fi
...
for forbidden in \
    ...
    "classification=macb-nsr-link-readonly-link-set" \
    "classification=macb-nsr-link-readonly-link-clear"; do
    if grep -Fq -- "$forbidden" "$kernel_strings"; then
        echo "kernel image contains forbidden MACB NSR_LINK control string: $forbidden" >&2
        exit 1
    fi
done
...
printf 'flags=%s\n' "$flags"
printf 'forbidden_macb_nsr_link_control_runtime_strings_absent=true\n'
