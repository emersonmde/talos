...
RESET_PROOF="$(./scripts/rpi5-entry-reset-proof.sh)"
cp "$RESET_PROOF" "$OUTPUT_DIR/kernel_2712.img"
cp "$RESET_PROOF" "$OUTPUT_DIR/kernel8.img"

sed -i '/^kernel_address=/d' "$OUTPUT_DIR/config.txt"
sed -i '/^talos_loader_diagnostic=/d' "$OUTPUT_DIR/config.txt"
printf '%s\n' 'talos_loader_diagnostic=asm-entry-reset-firmware-address' >> "$OUTPUT_DIR/config.txt"
