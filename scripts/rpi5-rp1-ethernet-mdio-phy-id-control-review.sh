...
ARCHIVE="$1"
MARKER="TALOS: rp1-ethernet-mdio-phy-id-control"
CAPTURE_NONCE=
...
tmp_root="target/tmp"
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-rp1-ethernet-mdio-phy-id-control-review.XXXXXX")"
...
for required in \
    "rpi5-rp1-ethernet-mdio-phy-id-control: start" \
    ...
    "classification=no-mdio-no-ethernet-rp1-ethernet-mdio-phy-id-control"; do
    if ! grep -Fq -- "$required" "$kernel_strings"; then
        echo "kernel image missing MDIO PHY-ID control string: $required" >&2
        exit 1
    fi
done
...
if [ -n "$CAPTURE_NONCE" ]; then
    if ! grep -Fq -- " capture-nonce=" "$kernel_strings"; then
        echo "kernel image missing MDIO PHY-ID control capture nonce label" >&2
        exit 1
    fi
    if ! grep -Fq -- "$CAPTURE_NONCE" "$kernel_strings"; then
        echo "kernel image missing MDIO PHY-ID control capture nonce: $CAPTURE_NONCE" >&2
        exit 1
    fi
fi
...
for forbidden in \
    ...
    if grep -Fq -- "$forbidden" "$kernel_strings"; then
        echo "kernel image contains forbidden MDIO PHY-ID control string: $forbidden" >&2
        exit 1
    fi
done
...
printf 'flags=%s\n' "$flags"
printf 'forbidden_mdio_phy_id_control_runtime_strings_absent=true\n'
