...
obj_file="$out_dir/rpi5_uart_proof.o"
elf_file="$out_dir/rpi5_uart_proof.elf"
bin_file="$out_dir/rpi5_uart_proof.img"

mkdir -p "$out_dir"

clang -target aarch64-none-elf -ffreestanding -fno-stack-protector \
    -c src/arch/aarch64/rpi5_uart_proof.S -o "$obj_file"
...
if [ ! -s "$bin_file" ]; then
    echo "UART proof image is empty: $bin_file" >&2
    exit 1
fi
