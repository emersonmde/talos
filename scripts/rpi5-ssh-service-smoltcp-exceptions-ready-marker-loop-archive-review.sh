...
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-ssh-exceptions-ready-review.XXXXXX")"
cleanup() {
    rm -rf "$work_dir"
}
trap cleanup EXIT
...
required_marker="TALOS: exceptions ready capture-nonce=$CAPTURE_NONCE"
for token in \
    "TALOS: asm_start" \
...
do
    if ! grep -Fq "$token" "$work_dir/kernel_2712.strings"; then
        echo "missing exceptions-ready marker loop token in kernel_2712.img: $token" >&2
        exit 1
    fi
done

printf 'exceptions_ready_marker_loop=ready\n'
