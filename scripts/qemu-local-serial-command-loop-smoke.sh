    ...
    if IFS= read -r -t 0.2 line <&3; then
        ...
        case "$line" in
            ...
            *"$LABEL: ready command=13"*)
                if [ "$sent" -eq 13 ] && [ "$SHELL_STDOUT_REGULAR_FILE_APPEND_REDIRECTION_SMOKE" -eq 1 ]; then
                    printf 'cat /etc/banner.txt\r' >&3
                    sent=14
...
if [ "$SHELL_GENERATED_USERLAND_MANIFEST_SMOKE" -eq 1 ]; then
    ...
    grep -q "talos: pipeline-lifecycle-status record=phase12-local-pipeline-dual-lifecycle-status-record-v1 .*producer-path=/bin/stderr .*consumer-path=/bin/stdin .*source=kernel-owned-pipeline-lifecycle-status-record" "$LOG_FILE"
    ...
    grep -q "talos: pipeline-lifecycle-status record=phase12-local-pipeline-dual-lifecycle-status-record-v1 .*producer-path=/bin/stdout .*consumer-path=/bin/stdin .*source=kernel-owned-pipeline-lifecycle-status-record" "$LOG_FILE"
