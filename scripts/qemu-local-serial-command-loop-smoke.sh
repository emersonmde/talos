...
SHELL_STDIO_SMOKE=0
if [ "$SHELL_STDOUT_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_EOF_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_READINESS_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ] || [ "$SHELL_STDERR_SMOKE" -eq 1 ]; then
    SHELL_STDIO_SMOKE=1
fi
...
while kill -0 "$qemu_pid" 2>/dev/null; do
    if IFS= read -r -t 0.2 line <&3; then
        printf '%s\n' "$line" >>"$LOG_FILE"
        if [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ] && [ "$delayed_stdin_sent" -eq 0 ]; then
            case "$line" in
        ...
        case "$line" in
            ...
            *"$LABEL: ready command=3"*)
                if [ "$sent" -eq 3 ]; then
                    ...
                    elif [ "$SHELL_STDIN_EOF_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_READINESS_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ]; then
            ...
            *"$LABEL: ready command=6"*)
                if [ "$sent" -eq 6 ]; then
                    ...
                    elif [ "$SHELL_STDIN_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_EOF_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_READINESS_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ]; then
                        printf 'exec stdout\r' >&3
                    elif [ "$SHELL_STDERR_SMOKE" -eq 1 ]; then
...
if [ "$PWD_COMMAND_SMOKE" -eq 1 ] || [ "$LS_ROOT_SMOKE" -eq 1 ] || [ "$LS_BIN_SMOKE" -eq 1 ] || [ "$CAT_BANNER_SMOKE" -eq 1 ] || [ "$CAT_CWD_SMOKE" -eq 1 ] || [ "$SHELL_VFS_EXEC_SMOKE" -eq 1 ] || [ "$SHELL_LITERAL_ARGV_SMOKE" -eq 1 ] || [ "$SHELL_PATH_LOOKUP_SMOKE" -eq 1 ] || [ "$SHELL_STDIO_SMOKE" -eq 1 ] || [ "$SHELL_WAITPID_SMOKE" -eq 1 ] || [ "$CD_FIXED_DIRS_SMOKE" -eq 1 ] || [ "$LS_CWD_SMOKE" -eq 1 ] || [ "$LITERAL_ECHO_SMOKE" -eq 1 ] || [ "$ECHO_COMMAND_SMOKE" -eq 1 ]; then
    grep -q "$LABEL: ready command=5" "$LOG_FILE"
fi
if [ "$PWD_COMMAND_SMOKE" -eq 1 ] || [ "$LS_BIN_SMOKE" -eq 1 ] || [ "$CAT_BANNER_SMOKE" -eq 1 ] || [ "$CAT_CWD_SMOKE" -eq 1 ] || [ "$SHELL_VFS_EXEC_SMOKE" -eq 1 ] || [ "$SHELL_LITERAL_ARGV_SMOKE" -eq 1 ] || [ "$SHELL_PATH_LOOKUP_SMOKE" -eq 1 ] || [ "$SHELL_STDIO_SMOKE" -eq 1 ] || [ "$SHELL_WAITPID_SMOKE" -eq 1 ] || [ "$CD_FIXED_DIRS_SMOKE" -eq 1 ] || [ "$LS_CWD_SMOKE" -eq 1 ]; then
...
if [ "$LINE_EDITING_SMOKE" -eq 1 ]; then
    ...
    grep -q "talos: exec-stderr fd=0x0000000000000002 bytes=0x000000000000001f return=0x000000000000001f source=userspace-talos-write" "$LOG_FILE"
    ...
    if [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ]; then
    ...
    if [ "$SHELL_STDIN_BOUNDED_WAIT_SMOKE" -eq 1 ]; then
        grep -Eq "talos: exec-stdin fd=0x0000000000000000 bytes=0x000000000000000e return=0x000000000000000e read-source=runtime-console0/local-input stdout-fd=0x0000000000000001 stdout-bytes=0x0000000000000064 stdout-return=0x0000000000000064 source=userspace-talos-read\\+userspace-talos-write read-result=bounded-wait/delayed-input readiness-observations=0x[0-9a-f]+" "$LOG_FILE"
    elif [ "$SHELL_STDIN_EOF_SMOKE" -eq 1 ] || [ "$SHELL_STDIN_READINESS_SMOKE" -eq 1 ]; then
    ...
    grep -q "$LABEL: dispatch command=4 status=handled responses=6" "$LOG_FILE"
    grep -q "talos: empty-command" "$LOG_FILE"
    grep -q "$LABEL: line command=5 hex=" "$LOG_FILE"
    grep -q "$LABEL: dispatch command=5 status=empty-command responses=1" "$LOG_FILE"
    grep -q "talos> bogus" "$LOG_FILE"
