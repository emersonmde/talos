...
if [ "$SHELL_GENERATED_USERLAND_MANIFEST_SMOKE" -eq 1 ]; then
    ...
    grep -q "$LABEL: dispatch command=9 status=handled responses=9" "$LOG_FILE"
    ...
    grep -q "$LABEL: dispatch command=12 status=handled responses=21" "$LOG_FILE"
    ...
    grep -q "$LABEL: dispatch command=5 status=handled responses=10" "$LOG_FILE"
