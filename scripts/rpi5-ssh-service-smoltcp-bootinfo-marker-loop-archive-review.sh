...
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-ssh-bootinfo-review.XXXXXX")"
cleanup() {
    rm -rf "$work_dir"
}
trap cleanup EXIT
...
required_marker="TALOS: boot info parsed capture-nonce=$CAPTURE_NONCE"
for token in \
    "TALOS: asm_start" \
    ...
    "capture-nonce=" \
    "$CAPTURE_NONCE" \
    "selected-normal-runtime-bootinfo=true" \
...
do
    if ! grep -Fq "$token" "$work_dir/kernel_2712.strings"; then
        echo "missing BootInfo marker loop token in kernel_2712.img: $token" >&2
        exit 1
    fi
done

printf 'bootinfo_marker_loop=ready\n'
