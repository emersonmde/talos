...
./scripts/rpi5-armstub-diagnostic-tree.sh "$SOURCE_DIR" "$OUTPUT_DIR"

prefix_dir="$OUTPUT_DIR/$SERIAL_PREFIX"
mkdir -p "$prefix_dir"

for path in config.txt cmdline.txt bcm2712-rpi-5-b.dtb kernel_2712.img kernel8.img armstub8-2712.bin; do
