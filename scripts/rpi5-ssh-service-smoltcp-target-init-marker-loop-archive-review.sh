...
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-ssh-target-init-review.XXXXXX")"
cleanup() {
    rm -rf "$work_dir"
}
trap cleanup EXIT
...
required_marker="TALOS: target init capture-nonce=$CAPTURE_NONCE"
for token in \
    "TALOS: asm_start" \
    ...
    "capture-nonce=" \
    "$CAPTURE_NONCE" \
    "selected-normal-runtime-target-init=true" \
...
do
    if ! grep -Fq "$token" "$work_dir/kernel_2712.strings"; then
        echo "missing target-init marker loop token in kernel_2712.img: $token" >&2
        exit 1
    fi
done

printf 'target_init_marker_loop=ready\n'
