...
grep -Eq "qemu-initial-userspace-process-launch-smoke: userspace-signal vector=lower-aarch64-sync esr=0x0000000054007a10 far=0x[0-9a-f]+ elr=0x0000000000010104 sp=0x0000800000000000 spsr=0x[0-9a-f]+ marker=0x7a10" "$LOG_FILE"
grep -Fq "qemu-initial-userspace-process-launch-smoke: frame available=true" "$LOG_FILE"
