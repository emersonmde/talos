...
ARCHIVE="$1"
MARKER="TALOS: rp1-ethernet-gem-mid-decode-discriminator-candidate"
CAPTURE_NONCE=
...
tmp_root="target/tmp"
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-rp1-ethernet-gem-mid-decode-discriminator-candidate-review.XXXXXX")"
...
for required in \
    "rpi5-rp1-ethernet-gem-mid-decode-discriminator-candidate: start" \
    ...
    if ! grep -Fq -- "$required" "$kernel_strings"; then
        echo "kernel image missing RP1 Ethernet GEM MID decode discriminator candidate string: $required" >&2
        exit 1
    fi
done
...
if [ -n "$CAPTURE_NONCE" ]; then
    if ! grep -Fq -- " capture-nonce=" "$kernel_strings"; then
        echo "kernel image missing RP1 Ethernet GEM MID decode discriminator candidate capture nonce label" >&2
        exit 1
    fi
    if ! grep -Fq -- "$CAPTURE_NONCE" "$kernel_strings"; then
        echo "kernel image missing RP1 Ethernet GEM MID decode discriminator candidate capture nonce: $CAPTURE_NONCE" >&2
        exit 1
    fi
fi

for forbidden in \
    "TALOS: rp1-ethernet-gem-mid-decode-discriminator-control" \
    ...
    if grep -Fq -- "$forbidden" "$kernel_strings"; then
        echo "kernel image contains forbidden RP1 Ethernet GEM MID decode discriminator candidate string: $forbidden" >&2
        exit 1
    fi
done
...
printf 'forbidden_rp1_ethernet_gem_mid_decode_discriminator_candidate_runtime_strings_absent=true\n'
