...
ARCHIVE="$1"
CONTROL_MARKER="TALOS: rp1-bridge-config-preflight-control"

./scripts/rpi5-archive-review.sh "$ARCHIVE" >/dev/null

tmp_root="target/tmp"
mkdir -p "$tmp_root"
work_dir="$(mktemp -d "$tmp_root/talos-rpi5-rp1-bridge-config-preflight-control-review.XXXXXX")"
...
for required in \
    "rpi5-rp1-bridge-config-preflight-control: start" \
    ...
    "classification=no-mmio-pcie2-bridge-preflight-control-visible"; do
    if ! grep -Fq -- "$required" "$kernel_strings"; then
        echo "kernel image missing bridge config preflight control string: $required" >&2
        exit 1
    fi
done

for forbidden in \
    "TALOS: rp1-bridge-config-preflight-result" \
    ...
    if grep -Fq -- "$forbidden" "$kernel_strings"; then
        echo "kernel image contains forbidden bridge config preflight control string: $forbidden" >&2
        exit 1
    fi
done
