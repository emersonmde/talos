        ...
        && (matches!(
            ...
            Some(LocalCommandExecRedirection::StdoutToTmpStdout(path))
                if path.is_exact_pipeline_combined_path()
                    ...
                    || path.as_bytes() == b"/tmp/talos-pipeline-output-alpha.txt"
        ) || matches!(
            request.consumer.redirection,
        ...
        && (matches!(
            ...
            Some(LocalCommandExecRedirection::StderrToTmpStderr(path))
                if path.is_exact_pipeline_combined_stderr_path()
                    || path.is_exact_pipeline_combined_stderr_append_path()
                    || path.as_bytes() == b"/tmp/talos-pipeline-error-beta.log"
        ) || matches!(
            request.consumer.redirection,
            Some(LocalCommandExecRedirection::StderrAppendTmpStderr(path))
                if path.is_exact_pipeline_combined_stderr_append_path()
                    || path.as_bytes() == b"/tmp/talos-pipeline-error-beta.log"
        ))
}
...
impl LocalCommandVolatilePath {
    ...
    fn is_exact_pipeline_combined_stderr_append_path(self) -> bool {
        self.as_bytes() == b"/tmp/pipeline-combined-stderr-append.txt"
    }
    ...
    O: ConsoleBackend,
{
    ...
    fn exec_vfs_pipeline(
        ...
        let pipe_source = match (
            ...
            (
                None,
                Some(LocalCommandExecRedirection::StderrAppendTmpStderr(_)),
                Some(LocalCommandExecRedirection::StdinFromEtcBanner),
                None,
            ) => "shell-pipe-producer-stdin-consumer-stderr-append-redirection",
            (
                None,
                None,
                Some(LocalCommandExecRedirection::StdinFromEtcBanner),
...
fn parse_bare_bin_pipeline_request(
    ...
    let producer_has_separated_redirection =
        has_separated_pipeline_stdin_redirection(producer_arguments);
    let consumer_has_separated_redirection =
        has_separated_pipeline_output_redirection(consumer_arguments.unwrap_or(""));
    ...
    if producer_has_separated_redirection || consumer_has_separated_redirection {
        if !(producer_has_separated_redirection
            && consumer_has_separated_redirection
            && (is_direct_pipeline_combined_stdin_stdout_redirection(&request)
                || is_direct_pipeline_combined_stdin_stderr_redirection(&request)))
        {
            return Err(LocalCommandExecError::InvalidPath);
...
fn parse_bare_bin_exec_request_with_arguments(
    ...
    let mut separated_redirection_started = false;
    if let Some(arguments) = arguments {
        let mut argument_tokens = arguments.as_bytes().split(|byte| is_space(*byte));
        while let Some(token) = argument_tokens.next() {
            ...
            if let Some(path) = token.strip_prefix(b">").and_then(|path| {
                ...
                } else if name.as_bytes() == b"stdout" {
                    LocalCommandVolatilePath::from_supported_stdout_path(path)
                } else {
...
fn parse_absolute_path_pipeline_request(
    ...
    let consumer_has_separated_redirection =
        has_separated_pipeline_output_redirection(consumer_arguments.unwrap_or(""));
    reject_unbounded_absolute_pipeline_producer_arguments(producer_arguments)?;
    reject_unbounded_absolute_pipeline_consumer_arguments(consumer_arguments.unwrap_or(""))?;
    let request = LocalCommandPipelineRequest {
...
fn parse_separated_pipeline_stderr_redirection(
    arguments: &str,
) -> Result<Option<LocalCommandExecRedirection>, LocalCommandExecError> {
    let mut tokens = arguments.as_bytes().split(|byte| is_space(*byte));
    let Some(operator) = next_non_empty_argument_token(&mut tokens) else {
    ...
    if let Some(arguments) = arguments {
        ...
        while let Some(token) = argument_tokens.next() {
            ...
            if let Some(path) = token.strip_prefix(b">").and_then(|path| {
                ...
                {
                    LocalCommandVolatilePath::from_exact_stdin_report_path(path)
                } else if path_text.as_bytes() == initramfs::PHASE10_STDOUT_PATH {
                    LocalCommandVolatilePath::from_supported_stdout_path(path)
                } else {
                    LocalCommandVolatilePath::from_exact_stdout_path(path)
                }
...
fn next_non_empty_argument_token<'a, I>(tokens: &mut I) -> Option<&'a [u8]>
where
...
fn has_separated_pipeline_stdin_redirection(arguments: &str) -> bool {
    arguments
        .as_bytes()
...
fn has_separated_pipeline_output_redirection(arguments: &str) -> bool {
    arguments
        .as_bytes()
        .split(|byte| is_space(*byte))
        .any(|token| token == b">" || token == b">>" || token == b"2>" || token == b"2>>")
}
...
fn reject_unbounded_pipeline_consumer_arguments(
    ...
    while let Some(token) = tokens.next() {
        ...
        if token == b">/tmp/pipeline-report.txt"
            ...
            || token == b"2>/tmp/pipeline-combined-stderr-append.txt"
            || token == b"2>>/tmp/pipeline-combined-stderr-append.txt"
            || token == b"2>/tmp/pipeline-stderr.txt"
            || token == b"2>>/tmp/pipeline-stderr.txt"
            || token == b">/tmp/talos-pipeline-output-alpha.txt"
            || token == b">>/tmp/talos-pipeline-output-alpha.txt"
            || token == b"2>/tmp/talos-pipeline-error-beta.log"
            || token == b"2>>/tmp/talos-pipeline-error-beta.log"
        {
            if count != 0 || saw_stdin_redirection {
                return Err(LocalCommandExecError::InvalidPath);
...
fn reject_unbounded_absolute_pipeline_consumer_arguments(
    arguments: &str,
) -> Result<(), LocalCommandExecError> {
    let mut tokens = arguments.as_bytes().split(|byte| is_space(*byte));
    let mut count = 0usize;
...
fn reject_unbounded_absolute_pipeline_producer_arguments(
    arguments: &str,
) -> Result<(), LocalCommandExecError> {
    let mut tokens = arguments.as_bytes().split(|byte| is_space(*byte));
    let mut count = 0usize;
    ...
    while let Some(token) = tokens.next() {
        ...
        if token == b"<" {
            let Some(source) = next_non_empty_argument_token(&mut tokens) else {
                return Err(LocalCommandExecError::InvalidPath);
            };
            if count != 0 || saw_stdin_redirection || source != b"/etc/banner.txt" {
                return Err(LocalCommandExecError::InvalidPath);
            }
            if next_non_empty_argument_token(&mut tokens).is_some() {
                return Err(LocalCommandExecError::InvalidPath);
...
talos> Talos initramfs fixture\n"
    ...
    fn local_command_loop_rejects_unaccepted_direct_stdout_output_redirection_forms() {
        let bytes = *b"/bin/stderr 2>/tmp/stdout.txt\r/bin/stdout >>/var/other.txt\r/bin/stderr 2>>/tmp/stdout.txt\r/bin/stdout | /bin/stdin >/tmp/stdout.txt\r/bin/stdin </etc/banner.txt >/tmp/stdout.txt\rcat /etc/banner.txt >/tmp/stdout.txt\rwaitpid\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stderr_file,
    ...
    fn local_command_loop_appends_direct_absolute_stdout_to_existing_volatile_regular_file() {
        let bytes = *b"/bin/stdout >/tmp/talos-output-alpha.txt\r/bin/stdout >>/tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\r/bin/stdout\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (created, appended, waited, observed, readback, normal) = {
            let mut io =
        ...
        assert_eq!(created.line(), b"/bin/stdout >/tmp/talos-output-alpha.txt");
        assert_eq!(created.status(), LocalCommandStatus::Handled);
        assert_eq!(
            appended.line(),
        ...
        assert_eq!(observed.line(), b"laststatus");
        assert_eq!(observed.status(), LocalCommandStatus::Handled);
        assert_eq!(readback.line(), b"cat /tmp/talos-output-alpha.txt");
        assert_eq!(readback.status(), LocalCommandStatus::Handled);
        assert_eq!(normal.line(), b"/bin/stdout");
        assert_eq!(normal.status(), LocalCommandStatus::Handled);
        assert_eq!(
    ...
    #[test_case]
    fn local_command_loop_redirects_direct_path_stdout_stderr_to_bounded_tmp_leaf_paths() {
        let bytes = *b"/bin/stdout >/tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\r/bin/stdout\r/bin/stderr 2>/tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\r/bin/stderr\r/bin/stdout >/var/out.txt\r/bin/stdout >/tmp/nested/out.txt\r/bin/stdout >/tmp/\r/bin/stdout >/tmp/../bad.txt\r/bin/stdout >/tmp/stderr.txt\r/bin/stderr 2>/var/err.txt\r/bin/stderr 2>/tmp/n/e\r/bin/stderr 2>/tmp/\r/bin/stderr 2>/tmp/../bad.txt\r/bin/stderr 2>/tmp/stdout.txt\r/bin/stderr >/tmp/misbound.err\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stdout_redirected,
            ...
            let mut io =
                DescriptorBackedLocalCommandIo::new_inherited_stdio(input, &mut backend).unwrap();
            let stdout_redirected = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            ...
            let stderr_redirected = run_one_descriptor_backed_serial_command(&mut io).unwrap();
        ...
        assert_eq!(
            stdout_redirected.line(),
            b"/bin/stdout >/tmp/talos-output-alpha.txt"
        );
        assert_eq!(stdout_redirected.status(), LocalCommandStatus::Handled);
        assert_eq!(stdout_redirected.response_lines(), 11);
        ...
        assert_eq!(normal_stdout.status(), LocalCommandStatus::Handled);
        assert_eq!(
            stderr_redirected.line(),
            b"/bin/stderr 2>/tmp/talos-error-beta.log"
        );
        assert_eq!(stderr_redirected.status(), LocalCommandStatus::Handled);
        assert_eq!(stderr_redirected.response_lines(), 11);
        ...
        assert!(output.contains(
            "talos: exec-stdout fd=0x0000000000000001 bytes=0x000000000000001f return=0x000000000000001f stream=regular-file route=volatile-vfs:/tmp/talos-output-alpha.txt source=userspace-talos-write\n"
        ));
        assert!(output.contains(
            "talos: cat path=/tmp/talos-output-alpha.txt bytes=0x000000000000001f source=volatile-vfs-descriptor-read\n"
        ));
        assert!(output.contains(
        ...
        assert!(output.contains(
            "talos: exec-stderr fd=0x0000000000000002 bytes=0x000000000000001f return=0x000000000000001f stream=regular-file route=volatile-vfs:/tmp/talos-error-beta.log source=userspace-talos-write\n"
        ));
        assert!(output.contains(
            "talos: cat path=/tmp/talos-error-beta.log bytes=0x000000000000001f source=volatile-vfs-descriptor-read\n"
        ));
        assert!(output.contains(
            "talos: exec-stdout fd=0x0000000000000001 bytes=0x000000000000001f return=0x000000000000001f stream=stdout route=runtime-console0/stdout source=userspace-talos-write\n"
        ));
    ...
    fn local_command_loop_accepts_direct_separated_redirection_tokens() {
        ...
        let (
            ...
            let mut io =
                DescriptorBackedLocalCommandIo::new_inherited_stdio(input, &mut backend).unwrap();
            let stdin_redirected = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdin_waited = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdin_observed = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdout_created = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdout_appended = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdout_waited = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stdout_observed = run_one_descriptor_backed_serial_command(&mut io).unwrap();
        ...
        assert_eq!(stdin_redirected.line(), b"/bin/stdin < /etc/banner.txt");
        assert_eq!(stdin_redirected.status(), LocalCommandStatus::Handled);
        assert_eq!(stdin_waited.status(), LocalCommandStatus::Handled);
        assert_eq!(stdin_observed.status(), LocalCommandStatus::Handled);
        assert_eq!(
            stdout_created.line(),
            b"/bin/stdout > /tmp/talos-output-alpha.txt"
        );
    ...
    #[test_case]
    fn local_command_loop_accepts_bare_name_explicit_fd_separated_redirection_tokens() {
        let bytes = *b"stdout 1 > /tmp/talos-output-alpha.txt\rstdout 1 >> /tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\rstderr 2 > /tmp/talos-error-beta.log\rstderr 2 >> /tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\r/bin/stdout 1 > /tmp/talos-output-alpha.txt\r/bin/stderr 2 > /tmp/talos-error-beta.log\rstdout\rstderr\rstdout 2 > /tmp/talos-output-alpha.txt\rstderr 1 > /tmp/talos-error-beta.log\rstdout 1 > /var/out.txt\rstdout 1 > /tmp/nested/out.txt\rstdout 1 >\rstdout 1 <> /tmp/talos-output-alpha.txt\rstdout 1 > /tmp/stderr.txt\rstdout 1 > /tmp/talos-output-alpha.txt extra\rstdout 1 2> /tmp/talos-output-alpha.txt\rstatus42 1 > /tmp/talos-output-alpha.txt\rstdout 1 > /tmp/talos-pipeline-output-alpha.txt | stdin\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stdout_created,
            ...
            stderr_observed,
            stderr_readback,
            direct_stdout_control,
            direct_stderr_control,
            normal_stdout,
            normal_stderr,
            bad_stdout_fd2,
            bad_stderr_fd1,
            ...
            let stderr_observed = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let stderr_readback = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let direct_stdout_control = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let direct_stderr_control = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let normal_stdout = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let normal_stderr = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let accepted_records = io.process_table_records();
            let bad_stdout_fd2 = run_one_descriptor_backed_serial_command(&mut io).unwrap();
        ...
        assert_eq!(
            stdout_created.line(),
            b"stdout 1 > /tmp/talos-output-alpha.txt"
        );
        assert_eq!(stdout_created.status(), LocalCommandStatus::Handled);
        assert_eq!(
            stdout_appended.line(),
        ...
        assert_eq!(stderr_readback.status(), LocalCommandStatus::Handled);
        assert_eq!(
            direct_stdout_control.line(),
            b"/bin/stdout 1 > /tmp/talos-output-alpha.txt"
        );
        assert_eq!(direct_stdout_control.status(), LocalCommandStatus::Handled);
        assert_eq!(
            direct_stderr_control.line(),
            b"/bin/stderr 2 > /tmp/talos-error-beta.log"
        );
        assert_eq!(direct_stderr_control.status(), LocalCommandStatus::Handled);
    ...
    #[test_case]
    fn local_command_loop_accepts_bare_name_separated_redirection_tokens() {
        let bytes = *b"stdin < /etc/banner.txt\rwaitpid\rlaststatus\rstdout > /tmp/talos-output-alpha.txt\rstdout >> /tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\rstderr 2> /tmp/talos-error-beta.log\rstderr 2>> /tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\r/bin/stdout > /tmp/talos-output-alpha.txt\rstdout\rstderr\rstdout > /var/out.txt\rstdout > /tmp/nested/out.txt\rstdout >\rstdout <> /tmp/talos-output-alpha.txt\rstdout 3 > /tmp/talos-output-alpha.txt\rstderr 2> /tmp/stdout.txt\rstderr 2>>\rstdin < /dev/null\rstatus42 > /tmp/talos-output-alpha.txt\rstdout | stdin > /tmp/talos-output-alpha.txt\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stdin_redirected,
        ...
        assert_eq!(stdin_redirected.line(), b"stdin < /etc/banner.txt");
        assert_eq!(stdin_redirected.status(), LocalCommandStatus::Handled);
        assert_eq!(stdin_waited.status(), LocalCommandStatus::Handled);
        assert_eq!(stdin_observed.status(), LocalCommandStatus::Handled);
        assert_eq!(
    ...
    fn local_command_loop_appends_bare_name_stdout_stderr_to_bounded_tmp_leaf_paths() {
        let bytes = *b"stdout >/tmp/talos-output-alpha.txt\rstdout >>/tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\rstdout\rstderr 2>/tmp/talos-error-beta.log\rstderr 2>>/tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\rstderr\r/bin/stdout >>/tmp/talos-output-alpha.txt\r/bin/stderr 2>>/tmp/talos-error-beta.log\rstdout >>/var/out.txt\rstdout >>/tmp/nested/out.txt\rstdout >>/tmp/\rstdout >>/tmp/../bad.txt\rstdout >>/tmp/stderr.txt\rstderr 2>>/var/err.txt\rstderr 2>>/tmp/n/e\rstderr 2>>/tmp/\rstderr 2>>/tmp/../bad.txt\rstderr 2>>/tmp/stdout.txt\rstderr >>/tmp/misbound.err\rstdout 3 >> /tmp/talos-output-alpha.txt\rstderr 3 >> /tmp/talos-error-beta.log\rstdout 3>>/tmp/talos-output-alpha.txt\rmissing >>/tmp/talos-output-alpha.txt\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stdout_created,
    ...
    #[test_case]
    fn local_command_loop_redirects_bare_name_stdout_stderr_to_bounded_tmp_leaf_paths() {
        let bytes = *b"stdout >/tmp/talos-output-alpha.txt\rwaitpid\rlaststatus\rcat /tmp/talos-output-alpha.txt\rstdout\rstderr 2>/tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\rstderr\rstdout >/var/out.txt\rstdout >/tmp/nested/out.txt\rstdout >/tmp/\rstdout >/tmp/../bad.txt\rstdout >/tmp/stderr.txt\rstderr 2>/var/err.txt\rstderr 2>/tmp/n/e\rstderr 2>/tmp/\rstderr 2>/tmp/../bad.txt\rstderr 2>/tmp/stdout.txt\rstderr >/tmp/misbound.err\rmissing >/tmp/talos-output-gamma.txt\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            stdout_redirected,
        ...
        assert_eq!(
            bad_command_name.line(),
            b"missing >/tmp/talos-output-gamma.txt"
        );
        assert_eq!(
            bad_command_name.status(),
            LocalCommandStatus::UnknownCommand
    ...
    fn local_command_loop_rejects_unaccepted_direct_stderr_output_redirection_forms() {
        let bytes = *b"/bin/stderr 2> /tmp/stdout.txt\r/bin/stderr 2>>/tmp/stdout.txt\rmissing 2>>/tmp/other.txt\r/bin/stderr 2>/var/err.txt\r/bin/stderr | /bin/stdin 2>/tmp/stderr.txt\r/bin/stdin </etc/banner.txt 2>/tmp/stderr.txt\rcat /etc/banner.txt 2>/tmp/stderr.txt\rwaitpid\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            separated,
    ...
    fn local_command_loop_appends_direct_child_stderr_to_existing_volatile_regular_file() {
        let bytes = *b"/bin/stderr 2>/tmp/talos-error-beta.log\r/bin/stderr 2>>/tmp/talos-error-beta.log\rwaitpid\rlaststatus\rcat /tmp/talos-error-beta.log\r/bin/stderr\rexec stdout\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        ...
        assert_eq!(created.line(), b"/bin/stderr 2>/tmp/talos-error-beta.log");
        assert_eq!(created.status(), LocalCommandStatus::Handled);
        assert_eq!(appended.line(), b"/bin/stderr 2>>/tmp/talos-error-beta.log");
        assert_eq!(appended.status(), LocalCommandStatus::Handled);
        assert_eq!(appended.response_lines(), 11);
        assert_eq!(waited.line(), b"waitpid");
        assert_eq!(waited.status(), LocalCommandStatus::Handled);
        assert_eq!(observed.line(), b"laststatus");
        assert_eq!(observed.status(), LocalCommandStatus::Handled);
        assert_eq!(readback.line(), b"cat /tmp/talos-error-beta.log");
        assert_eq!(readback.status(), LocalCommandStatus::Handled);
        assert_eq!(normal_stderr.line(), b"/bin/stderr");
        assert_eq!(normal_stderr.status(), LocalCommandStatus::Handled);
        assert_eq!(normal_stdout.line(), b"exec stdout");
...
talos> talos: exec-not-executable\n"
    ...
    fn local_command_loop_runs_direct_absolute_path_vfs_command_with_readonly_stdin_redirection() {
        let bytes = *b"/bin/stdin </etc/banner.txt\rwaitpid\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\r/bin/stdout </etc/banner.txt\r/bin/stdin </dev/null\r/bin/stdin < /dev/null\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
    ...
    #[test_case]
    fn local_command_loop_appends_direct_path_pipeline_combined_stdin_stderr_redirection() {
        let command = parse_local_command(
            b"/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt",
        )
        .unwrap();
        let request = parse_absolute_path_pipeline_request(command).unwrap();
        assert_eq!(request.producer.path(), initramfs::PHASE10_STDIN_PATH);
        ...
        assert_eq!(request.consumer.path(), initramfs::PHASE10_STDERR_PATH);
        assert!(matches!(
            request.consumer.redirection,
            Some(LocalCommandExecRedirection::StderrAppendTmpStderr(path))
                if path.is_exact_pipeline_combined_stderr_append_path()
        ...
        let bytes = *b"/bin/stdin </etc/banner.txt | /bin/stderr 2>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rwaitpid 0x100001\rwaitpid 0x100002\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rcat /tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt\r/bin/stderr\rstdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>/tmp/pipeline-stderr.txt\r/bin/stdout | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>> /tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 1>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>>/var/x\r/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt | /bin/stdin\rmissing </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | missing 2>>/tmp/pipeline-combined-stderr-append.txt\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let statuses = {
            let mut io =
                DescriptorBackedLocalCommandIo::new_inherited_stdio(input, &mut backend).unwrap();
            let mut statuses = [LocalCommandStatus::Handled; 21];
        ...
        assert_eq!(
            statuses[11..19]
                .iter()
                .filter(|status| **status == LocalCommandStatus::UnexpectedArgument)
                .count(),
            7
        );
        assert_eq!(
            statuses[11..19]
                .iter()
                .filter(|status| {
                    **status == LocalCommandStatus::InputError(PollingTtyRxOutcome::LineComplete)
                })
                .count(),
            1
        );
        assert_eq!(statuses[19], LocalCommandStatus::UnknownCommand);
        assert_eq!(statuses[20], LocalCommandStatus::UnexpectedArgument);
        assert!(output.contains("talos: exec path=/bin/stdin source=vfs-open-read\n"));
        assert!(output.contains("talos: exec path=/bin/stderr source=vfs-open-read\n"));
        assert!(output.contains(
            "talos: exec-descriptors owner=0x0000000000000001 inherited-count=0x0000000000000003 fd0=regular-file fd1=pipe-endpoint fd2=stdio-output loader-temp-fd=0x0000000000000003 loader-temp-open=false source=shell-process-descriptor-table\n"
        ...
        assert!(output.contains(
            "talos: exec-redirection op=sink source-fd=0x0000000000000002 target-path=/tmp/pipeline-combined-stderr-append.txt target-stream=regular-file target-route=volatile-vfs:/tmp/pipeline-combined-stderr-append.txt child-only=true shell-restored=true source=shell-redirection-stderr-tmp-stderr\n"
        ));
        assert!(output.contains(
            "talos: exec-redirection op=append source-fd=0x0000000000000002 target-path=/tmp/pipeline-combined-stderr-append.txt target-stream=regular-file target-route=volatile-vfs:/tmp/pipeline-combined-stderr-append.txt child-only=true shell-restored=true source=shell-redirection-stderr-tmp-stderr-append\n"
        ));
        assert!(output.contains(
            "talos: exec-stderr fd=0x0000000000000002 bytes=0x000000000000001f return=0x000000000000001f stream=regular-file route=volatile-vfs:/tmp/pipeline-combined-stderr-append.txt source=userspace-talos-write\n"
        ));
        assert!(output.contains(
            "talos: pipeline id=0x0000000000000001 producer-fd=0x0000000000000001 producer-path=/bin/stdin consumer-fd=0x0000000000000000 consumer-path=/bin/stderr bytes-written=0x000000000000003d bytes-read=0x0000000000000000 writer-closed=true reader-eof=false shell-restored=true source=shell-pipe-producer-stdin-consumer-stderr-redirection\n"
        ));
        ...
        assert!(output.contains(
            "talos: cat path=/tmp/pipeline-combined-stderr-append.txt bytes=0x000000000000003e source=volatile-vfs-descriptor-read\n"
        ));
        assert!(output.contains("Talos userspace stdin fixture read: Talos initramfs fixture\n"));
        assert_eq!(
            output.matches("Talos userspace stderr fixture\n").count(),
        ...
        assert_eq!(output.matches("talos: exec-invalid-path\n").count(), 8);
        assert_eq!(
            output.matches("talos: input-error line-complete\n").count(),
            1
        );
        assert_eq!(output.matches("talos: unknown-command").count(), 1);
    }
    ...
    #[test_case]
    fn local_command_loop_appends_direct_combined_pipeline_to_bounded_tmp_leaf_paths() {
        let stdout_command = parse_local_command(
            b"/bin/stdin < /etc/banner.txt | /bin/stdin >> /tmp/talos-pipeline-output-alpha.txt",
        )
        .unwrap();
    ...
    #[test_case]
    fn local_command_loop_accepts_bare_name_pipeline_separated_redirection_tokens() {
        let stdout_command = parse_local_command(
            b"stdin < /etc/banner.txt | stdin >> /tmp/talos-pipeline-output-alpha.txt",
        )
        .unwrap();
        ...
        let bytes = *b"stdin < /etc/banner.txt | stdin > /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | stdin >> /tmp/talos-pipeline-output-alpha.txt\rwaitpid 0x100001\rwaitpid 0x100002\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rcat /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | stderr 2> /tmp/talos-pipeline-error-beta.log\rstdin < /etc/banner.txt | stderr 2>> /tmp/talos-pipeline-error-beta.log\rwaitpid 0x100001\rwaitpid 0x100002\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rcat /tmp/talos-pipeline-error-beta.log\r/bin/stdin < /etc/banner.txt | /bin/stdin > /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt\rstderr\r/bin/stdin < /etc/banner.txt | stdin >> /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | /bin/stdin >> /tmp/talos-pipeline-output-alpha.txt\rstdin </etc/banner.txt | stdin >> /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | stdin >>/tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | stdin >> /var/out.txt\rstdin < /etc/banner.txt | stdin >> /tmp/nested/out.txt\rstdin < /etc/banner.txt | stdin >> /tmp/.\rstdin < /etc/banner.txt | stdin >> /tmp/../bad.txt\rstdin < /etc/banner.txt | stdin >> /tmp/stderr.txt\rstdin < /etc/banner.txt | stdin 1>> /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | stderr 2>> /var/err.txt\rstdin < /etc/banner.txt | stderr 2>> /tmp/nested/err.log\rstdin < /etc/banner.txt | stderr 2>> /tmp/\rstdin < /etc/banner.txt | stderr 2>> /tmp/stdout.txt\rstdin < /etc/banner.txt | stderr 2 >> /tmp/talos-pipeline-error-beta.log\rmissing < /etc/banner.txt | stdin >> /tmp/talos-pipeline-output-alpha.txt\rstdin < /etc/banner.txt | missing >> /tmp/talos-pipeline-output-alpha.txt\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let statuses = {
            let mut io =
                DescriptorBackedLocalCommandIo::new_inherited_stdio(input, &mut backend).unwrap();
            let mut statuses = [LocalCommandStatus::Handled; 38];
            let mut retained_records = None;
            let mut index = 0usize;
            while index < statuses.len() {
                let record = run_one_descriptor_backed_serial_command(&mut io).unwrap();
        ...
        for (index, status) in statuses[..21].iter().enumerate() {
            assert_eq!(
                *status,
                LocalCommandStatus::Handled,
                "accepted command index {index}"
        ...
        for (index, status) in statuses[21..36].iter().enumerate() {
            assert_eq!(
                *status,
                LocalCommandStatus::UnexpectedArgument,
                "rejected command index {}",
        ...
        assert_eq!(statuses[36], LocalCommandStatus::UnknownCommand);
        assert_eq!(statuses[37], LocalCommandStatus::UnexpectedArgument);
        assert!(output.contains(
            "talos: exec-descriptors owner=0x0000000000000001 inherited-count=0x0000000000000003 fd0=regular-file fd1=pipe-endpoint fd2=stdio-output loader-temp-fd=0x0000000000000003 loader-temp-open=false source=shell-process-descriptor-table\n"
        ));
        assert!(output.contains(
    ...
    #[test_case]
    fn local_command_loop_appends_bare_name_pipeline_combined_stdin_stderr_redirection() {
        let command = parse_local_command(
            b"stdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt",
        )
        .unwrap();
        ...
        let bytes = *b"stdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rwaitpid 0x100001\rwaitpid 0x100002\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rcat /tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt\rstderr\r/bin/stdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rmissing </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | missing 2>>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | /bin/stderr 2>>/tmp/pipeline-combined-stderr-append.txt\r/bin/stdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2>/tmp/pipeline-stderr.txt\rstdout | stderr 2>>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | stderr 2>> /tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | stderr 1>/tmp/pipeline-combined-stderr-append.txt\rstdin </etc/banner.txt | stderr 2>>/var/x\rstdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr-append.txt | stdin\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let statuses = {
            let mut io =
                DescriptorBackedLocalCommandIo::new_inherited_stdio(input, &mut backend).unwrap();
            let mut statuses = [LocalCommandStatus::Handled; 23];
            let mut retained_records = None;
            let mut index = 0usize;
            while index < statuses.len() {
                let record = run_one_descriptor_backed_serial_command(&mut io).unwrap();
        ...
        assert_eq!(statuses[12], LocalCommandStatus::UnknownCommand);
        for status in &statuses[13..] {
            assert_eq!(*status, LocalCommandStatus::UnexpectedArgument);
        }
        assert!(output.contains("talos: exec path=/bin/stdin source=vfs-open-read\n"));
        assert!(output.contains("talos: exec path=/bin/stderr source=vfs-open-read\n"));
    ...
    #[test_case]
    fn local_command_loop_runs_bare_name_pipeline_combined_stdin_stderr_redirection() {
        let command = parse_local_command(
            b"stdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr.txt",
        )
        .unwrap();
        let request = parse_bare_bin_pipeline_request(command).unwrap();
        assert_eq!(request.producer.path(), initramfs::PHASE10_STDIN_PATH);
        ...
        let bytes = *b"stdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr.txt\rwaitpid 0x100001\rwaitpid 0x100002\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rcat /tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt\r/bin/stdin </etc/banner.txt | /bin/stderr 2>/tmp/pipeline-combined-stderr.txt\r/bin/stderr\rmissing </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | missing 2>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2>>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2>/tmp/pipeline-stderr.txt\rstdout | stderr 2>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2> /tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 1>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2>/var/x\rstdin </etc/banner.txt | /bin/stderr 2>/tmp/pipeline-combined-stderr.txt\r/bin/stdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr.txt\rstdin </etc/banner.txt | stderr 2>/tmp/pipeline-combined-stderr.txt | stdin\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (
            pipeline,
            ...
            let direct_stderr = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            let retained_records = io.process_table_records();
            let missing_producer = run_one_descriptor_backed_serial_command(&mut io).unwrap();
            assert_eq!(io.process_table_records(), retained_records);
            let missing_consumer = run_one_descriptor_backed_serial_command(&mut io).unwrap();
    ...
    fn local_command_loop_runs_bare_name_vfs_command_with_readonly_stdin_redirection() {
        let bytes = *b"stdin </etc/banner.txt\rwaitpid\rlaststatus\rpipestatus\rcat /proc/talos/processes\rps\rstdout </etc/banner.txt\rstdin </dev/null\rstdin < /dev/null\r";
        let input = ScriptedInput::new(bytes, bytes.len());
        let mut backend = CaptureSink::new();
        let (exec, waited, observed, status, cat, ps, non_stdin, dev_null, spaced) = {
            let mut io =
