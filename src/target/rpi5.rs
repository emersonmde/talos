...
pub fn write_rp1_ethernet_bootinfo_report_serial_visibility_earliest_marker() {
    ...
    write_early_static(" marker-stage=earliest-entry");
    write_early_static(" marker=bootinfo-report-visibility-earliest-entry-marker");
    write_early_static(" emits-before-boot-info=true emits-after-boot-info=false");
    write_early_static(" post-bootinfo-report-path-marker-visible=false");
    write_early_static(" classification=bootinfo-report-serial-visibility-earliest-marker\n");
    wait_uart10_empty_early_phase();
}
...
pub fn run_rp1_ethernet_bootinfo_report_serial_visibility_earliest_only_control() -> ! {
    loop {
        write_early_static("TALOS: rp1-ethernet-bootinfo-report-serial-visibility-control");
        write_rp1_ethernet_bootinfo_report_serial_visibility_capture_nonce();
        write_rp1_ethernet_bootinfo_report_serial_visibility_common();
...
pub fn run_rp1_ethernet_bootinfo_report_serial_visibility_candidate() -> ! {
    loop {
        write_early_static("TALOS: rp1-ethernet-bootinfo-report-serial-visibility");
        write_rp1_ethernet_bootinfo_report_serial_visibility_capture_nonce();
        write_rp1_ethernet_bootinfo_report_serial_visibility_common();
        ...
        write_early_static(" post-bootinfo-report-path-marker-visible=true");
        write_early_static(
...
fn write_rp1_ethernet_bootinfo_report_serial_visibility_common() {
    write_early_static(
    ...
    write_early_static(" bootinfo-source=report_boot_identity");
    write_early_static(" target=none controller=none compatible=none phy-model=none");
    write_early_static(" selected-registers=none bcm54213pe-register-values=withheld");
    write_early_static(" ethernet-target-facts-constructed=false");
    write_early_static(" mdio-target-constructed=false man-frame-constructed=false");
    write_early_static(" macb-target-constructed=false gpio32-or-phy-target-constructed=false");
    write_early_static(" volatile-ethernet-access-performed=false");
    write_early_static(" selector-write-performed=false bmcr-write-performed=false");
    write_early_static(" broadcom-shadow-mmd-aux-access-performed=false");
    write_early_static(" interrupt-ownership=false phy-or-mac-configuration=false");
    write_early_static(" packet-io-performed=false networking-performed=false ssh-performed=false");
    write_early_static(
        " hardware-outcomes=no-selected-tftp,no-earliest-marker,earliest-marker-only,both-markers-observed,known-good-control-failed,staging-capture-inconclusive,restore-failed",
    );
    write_early_static(
        " rejected-claims=bcm54213pe-register-values,ethernet-readiness,link-readiness,gpio32-phy-reset-ownership,bmcr-write,broadcom-shadow-mmd-aux-access,interrupt-ownership,phy-mac-configuration,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
...
fn write_rp1_ethernet_bootinfo_report_serial_visibility_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_state_candidate")]
pub fn run_rp1_ethernet_gpio32_event_state_candidate() -> ! {
    write_early_static("rpi5-rp1-ethernet-gpio32-event-state-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-gpio32-event-state-candidate: before-gpio32-rio1-pad-read-only-volatile-loads-no-event-clear-no-mmio-writes\n",
    );
    wait_uart10_empty_early_phase();

    let status = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_STATUS);
    let ctrl = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_CTRL);
    let rio1_out = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OUT);
    let rio1_oe = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OE);
    let rio1_in = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_IN);
    let pad = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_PAD);
    let event_bits = status & 0x0ff0_0000;
    let classification = classify_rp1_ethernet_gpio32_event_state(status);

    loop {
        write_early_static("TALOS: rp1-ethernet-gpio32-event-state-candidate");
        write_rp1_ethernet_gpio32_event_state_capture_nonce();
        write_rp1_ethernet_gpio32_event_state_common("candidate");
        write_early_static(" target=rp1-ethernet-gpio32-event-state-readonly-discriminator");
        write_early_static(" gpio-controller=rp1_gpio gpio-line=32 signal=ETH_RST_N");
        write_early_static(" bank=bank1 bank-local-bit=4 active-low=true");
        write_early_static(" gpio32-status-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_STATUS as u64);
        write_early_static(" gpio32-ctrl-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_CTRL as u64);
        ...
        write_early_static(" gpio32-pad-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_PAD as u64);
        write_early_static(" status=");
        write_early_hex_u64(status as u64);
        write_early_static(" ctrl=");
        write_early_hex_u64(ctrl as u64);
        write_early_static(" rio1-out=");
        write_early_hex_u64(rio1_out as u64);
        write_early_static(" rio1-oe=");
        write_early_hex_u64(rio1_oe as u64);
        write_early_static(" rio1-in=");
        write_early_hex_u64(rio1_in as u64);
        write_early_static(" pad=");
        write_early_hex_u64(pad as u64);
        write_early_static(" status-event-mask=");
        write_early_hex_u64(0x0ff0_0000);
        write_early_static(" event-bits=");
        write_early_hex_u64(event_bits as u64);
        write_early_static(" source-decoding-status=source-backed-bits-20-27");
        write_early_static(
            " source-backed-event-bit-names=bit20-raw-falling,bit21-raw-rising,bit22-raw-low,bit23-raw-high,bit24-filtered-falling,bit25-filtered-rising,bit26-filtered-low,bit27-filtered-high",
        );
        write_early_static(
            " v2-proof-task-id=phase12-rp1-ethernet-gpio32-phy-reset-write-restore-v2-pi5-proof-20260610",
        );
        write_early_static(
            " v2-closeout-task-id=phase12-rp1-ethernet-gpio32-phy-reset-write-restore-v2-proof-closeout-20260610",
        );
        write_early_static(" v2-commit=0127984a1938cf050e2a6757f9f116f78976cf5e");
        write_early_static(
        ...
        write_rp1_ethernet_gpio32_event_state_rejections();
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_state_no_mmio_control")]
pub fn run_rp1_ethernet_gpio32_event_state_no_mmio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-gpio32-event-state-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-gpio32-event-state-control: no-gpio32-rio-pad-ethernet-no-mmio-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-gpio32-event-state-control");
        write_rp1_ethernet_gpio32_event_state_capture_nonce();
        write_rp1_ethernet_gpio32_event_state_common("no-gpio-no-ethernet-control");
        write_early_static(" target=none gpio-controller=none gpio-line=none signal=none");
        write_early_static(" bank=none bank-local-bit=none active-low=false");
        write_early_static(" gpio32-status-address=not-constructed");
        write_early_static(" gpio32-ctrl-address=not-constructed");
        write_early_static(" rio1-out-address=not-constructed");
        write_early_static(" rio1-oe-address=not-constructed");
        write_early_static(" rio1-in-address=not-constructed");
        write_early_static(" gpio32-pad-address=not-constructed");
        write_early_static(" status=withheld ctrl=withheld rio1-out=withheld rio1-oe=withheld");
        write_early_static(
            " rio1-in=withheld pad=withheld status-event-mask=withheld event-bits=withheld",
        );
        write_early_static(" source-decoding-status=capture-chain-inconclusive");
        write_early_static(" source-backed-event-bit-names=withheld");
        write_early_static(
            " v2-proof-task-id=phase12-rp1-ethernet-gpio32-phy-reset-write-restore-v2-pi5-proof-20260610",
        );
        write_early_static(
        ...
        write_rp1_ethernet_gpio32_event_state_rejections();
        write_early_static(
            " classification=no-gpio-no-ethernet-rp1-ethernet-gpio32-event-state-control\n",
        );
        wait_uart10_empty_early_phase();
    }
}
...
pub fn run_rp1_ethernet_gpio32_event_clear_candidate() -> ! {
    const EVENT_MASK: u32 = 0x0ff0_0000;
    const ACCEPTED_EVENT_BITS: u32 = 0x0ab0_0000;
    const IRQRESET_VALUE: u32 = 0x1000_0000;
    const CTRL_NON_IRQRESET_MASK: u32 = !IRQRESET_VALUE;

    write_early_static("rpi5-rp1-ethernet-gpio32-event-clear-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-gpio32-event-clear-candidate: before-guarded-gpio32-irqreset-write\n",
    );
    wait_uart10_empty_early_phase();

    let pre_status = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_STATUS);
    let pre_ctrl = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_CTRL);
    let pre_rio1_out = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OUT);
    let pre_rio1_oe = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OE);
    let pre_rio1_in = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_IN);
    let pre_pad = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_PAD);

    let pre_event_bits = pre_status & EVENT_MASK;
    let funcsel = pre_ctrl & 0x1f;
    let override_bits = pre_ctrl & ((0x3 << 12) | (0x3 << 14) | (0x3 << 16));
    let precondition = classify_rp1_ethernet_gpio32_event_clear_precondition(
        pre_status,
        pre_ctrl,
        pre_rio1_out,
        pre_rio1_oe,
    ...
    let mut post_status = pre_status;
    let mut post_ctrl = pre_ctrl;
    let mut post_rio1_out = pre_rio1_out;
    let mut post_rio1_oe = pre_rio1_oe;
    let mut post_rio1_in = pre_rio1_in;
    let mut post_pad = pre_pad;
    let mut event_clear_performed = false;
    let classification = if let Some(blocked) = precondition {
        blocked
    } else {
        write_rp1_reg_u32_ordered(RP1_GPIO32_OBSERVED_APERTURE_CTRL + 0x2000, IRQRESET_VALUE);
        post_status = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_STATUS);
        post_ctrl = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_CTRL);
        post_rio1_out = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OUT);
        post_rio1_oe = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_OE);
        post_rio1_in = read_rp1_reg_u32(RP1_RIO1_OBSERVED_APERTURE_IN);
        post_pad = read_rp1_reg_u32(RP1_GPIO32_OBSERVED_APERTURE_PAD);
        event_clear_performed = true;

        let invariants_ok = (post_ctrl & CTRL_NON_IRQRESET_MASK)
            == (pre_ctrl & CTRL_NON_IRQRESET_MASK)
            && post_rio1_out == pre_rio1_out
            && post_rio1_oe == pre_rio1_oe
            && post_rio1_in == pre_rio1_in
            && post_pad == pre_pad;
        let post_event_bits = post_status & EVENT_MASK;
        if !invariants_ok {
            "source-contract-violated-blocker"
        } else if post_event_bits == 0 {
            "event-clear-visible-with-invariants"
        } else {
    ...
    loop {
        write_early_static("TALOS: rp1-ethernet-gpio32-event-clear-candidate");
        write_rp1_ethernet_gpio32_event_clear_capture_nonce();
        write_rp1_ethernet_gpio32_event_clear_common("candidate");
        write_early_static(" target=rp1-ethernet-gpio32-event-clear-irqreset");
        write_early_static(" gpio-controller=rp1_gpio gpio-line=32 signal=ETH_RST_N");
        write_early_static(" bank=bank1 bank-local-bit=4 active-low=true");
        write_early_static(" gpio32-status-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_STATUS as u64);
        write_early_static(" gpio32-ctrl-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_CTRL as u64);
        write_early_static(" gpio32-ctrl-set-address=");
        write_early_hex_u64((RP1_GPIO32_OBSERVED_APERTURE_CTRL + 0x2000) as u64);
        write_early_static(" rio1-out-address=");
        write_early_hex_u64(RP1_RIO1_OBSERVED_APERTURE_OUT as u64);
        write_early_static(" rio1-oe-address=");
        write_early_hex_u64(RP1_RIO1_OBSERVED_APERTURE_OE as u64);
        ...
        write_early_static(" gpio32-pad-address=");
        write_early_hex_u64(RP1_GPIO32_OBSERVED_APERTURE_PAD as u64);
        write_early_static(" pre-status=");
        write_early_hex_u64(pre_status as u64);
        write_early_static(" pre-ctrl=");
        write_early_hex_u64(pre_ctrl as u64);
        write_early_static(" pre-rio1-out=");
        write_early_hex_u64(pre_rio1_out as u64);
        write_early_static(" pre-rio1-oe=");
        write_early_hex_u64(pre_rio1_oe as u64);
        write_early_static(" pre-rio1-in=");
        write_early_hex_u64(pre_rio1_in as u64);
        write_early_static(" pre-pad=");
        write_early_hex_u64(pre_pad as u64);
        write_early_static(" status-event-mask=");
        write_early_hex_u64(EVENT_MASK as u64);
        write_early_static(" accepted-event-bits=");
        write_early_hex_u64(ACCEPTED_EVENT_BITS as u64);
        write_early_static(" pre-event-bits=");
        write_early_hex_u64(pre_event_bits as u64);
        write_early_static(" funcsel=");
        write_early_dec_u64(funcsel as u64);
        write_early_static(" override-bits=");
        write_early_hex_u64(override_bits as u64);
        write_early_static(" event-clear-write-value=");
        write_early_hex_u64(IRQRESET_VALUE as u64);
        write_early_static(" event-clear-performed=");
        write_bool(event_clear_performed);
        write_early_static(" post-status=");
        write_early_hex_u64(post_status as u64);
        write_early_static(" post-ctrl=");
        write_early_hex_u64(post_ctrl as u64);
        write_early_static(" post-rio1-out=");
        write_early_hex_u64(post_rio1_out as u64);
        write_early_static(" post-rio1-oe=");
        write_early_hex_u64(post_rio1_oe as u64);
        write_early_static(" post-rio1-in=");
        write_early_hex_u64(post_rio1_in as u64);
        write_early_static(" post-pad=");
        write_early_hex_u64(post_pad as u64);
        write_early_static(" post-event-bits=");
        write_early_hex_u64(post_event_bits as u64);
        write_early_static(" ctrl-non-irqreset-preserved=");
        write_bool((post_ctrl & CTRL_NON_IRQRESET_MASK) == (pre_ctrl & CTRL_NON_IRQRESET_MASK));
        write_early_static(" rio1-out-preserved=");
        write_bool(post_rio1_out == pre_rio1_out);
        write_early_static(" rio1-oe-preserved=");
        write_bool(post_rio1_oe == pre_rio1_oe);
        write_early_static(" rio1-in-preserved=");
        write_bool(post_rio1_in == pre_rio1_in);
        write_early_static(" pad-preserved=");
        write_bool(post_pad == pre_pad);
        write_early_static(" touched-fields=GPIO32_CTRL_SET.IRQRESET");
        write_rp1_ethernet_gpio32_event_clear_rejections();
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_clear_no_mmio_control")]
pub fn run_rp1_ethernet_gpio32_event_clear_no_mmio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-gpio32-event-clear-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-gpio32-event-clear-control: no-gpio32-rio-pad-ethernet-no-mmio-target-construction-no-event-clear\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-gpio32-event-clear-control");
        write_rp1_ethernet_gpio32_event_clear_capture_nonce();
        write_rp1_ethernet_gpio32_event_clear_common("no-gpio-no-ethernet-control");
        write_early_static(" target=none gpio-controller=none gpio-line=none signal=none");
        write_early_static(" bank=none bank-local-bit=none active-low=false");
        write_early_static(" gpio32-status-address=not-constructed");
        write_early_static(" gpio32-ctrl-address=not-constructed");
        write_early_static(" gpio32-ctrl-set-address=not-constructed");
        write_early_static(" rio1-out-address=not-constructed");
        write_early_static(" rio1-oe-address=not-constructed");
        write_early_static(" rio1-in-address=not-constructed");
        write_early_static(" gpio32-pad-address=not-constructed");
        write_early_static(" pre-status=withheld pre-ctrl=withheld");
        write_early_static(" pre-rio1-out=withheld pre-rio1-oe=withheld");
        write_early_static(" pre-rio1-in=withheld pre-pad=withheld");
        write_early_static(" status-event-mask=withheld accepted-event-bits=withheld");
        write_early_static(" pre-event-bits=withheld funcsel=withheld override-bits=withheld");
        write_early_static(" event-clear-write-value=withheld event-clear-performed=false");
        write_early_static(" post-status=withheld post-ctrl=withheld");
        write_early_static(" post-rio1-out=withheld post-rio1-oe=withheld");
        write_early_static(" post-rio1-in=withheld post-pad=withheld post-event-bits=withheld");
        write_early_static(" ctrl-non-irqreset-preserved=true");
        write_early_static(" rio1-out-preserved=true rio1-oe-preserved=true");
        write_early_static(" rio1-in-preserved=true pad-preserved=true");
        write_early_static(" touched-fields=none");
        write_rp1_ethernet_gpio32_event_clear_rejections();
        write_early_static(
            " classification=no-gpio-no-ethernet-rp1-ethernet-gpio32-event-clear-control\n",
        );
        wait_uart10_empty_early_phase();
    }
}
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_phy_id_after_mpe_candidate")]
pub fn run_rp1_ethernet_mdio_phy_id_after_mpe_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    ...
    write_early_static("rpi5-rp1-ethernet-mdio-phy-id-after-mpe-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-phy-id-after-mpe-candidate: before-corrected-target-guarded-phy1-physid-man-transactions\n",
    );
    wait_uart10_empty_early_phase();
    ...
    let classification = if ncr_before & NCR_MPE == 0 {
    ...
    } else if !poll_rp1_ethernet_mdio_phy_id_after_mpe_idle(NSR, NSR_IDLE) {
        nsr_before_physid1 = read_rp1_reg_u32(NSR);
        ...
        write_rp1_reg_u32_ordered(MAN, PHYSID1_MAN_FRAME);
        man_writes_performed = true;
        if !poll_rp1_ethernet_mdio_phy_id_after_mpe_idle(NSR, NSR_IDLE) {
            nsr_after_physid1 = read_rp1_reg_u32(NSR);
            man_after_physid1 = read_rp1_reg_u32(MAN);
            "mdio-phy1-physid-after-mpe-timeout"
        } else {
    ...
    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-phy-id-after-mpe-candidate");
        write_rp1_ethernet_mdio_phy_id_after_mpe_capture_nonce();
        write_rp1_ethernet_mdio_phy_id_after_mpe_common("candidate");
        write_early_static(" target=rp1-ethernet-mdio-after-mpe-clause22-phy1-physid1-physid2");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        write_early_static(" phy-handle=phy1 phy-node=ethernet-phy@1 phy-address=1");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=");
        write_early_hex_u64(MACB_MID_CONTEXT as u64);
        write_early_static(" observed-window-macb-mid-context-raw=");
        write_early_hex_u64(macb_mid_context as u64);
        write_early_static(" corrected-window-comparator-cpu-physical-target=");
        write_early_hex_u64(0x1f_0010_00fc);
        write_early_static(" ncr-observed-target=");
        write_early_hex_u64(NCR as u64);
        write_early_static(" nsr-observed-target=");
        ...
        write_rp1_ethernet_mdio_phy_id_after_mpe_rejections(man_writes_performed);
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_phy_id_after_mpe_no_mdio_control")]
pub fn run_rp1_ethernet_mdio_phy_id_after_mpe_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-mdio-phy-id-after-mpe-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-phy-id-after-mpe-control: no-mdio-no-ethernet-no-mmio-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-phy-id-after-mpe-control");
        write_rp1_ethernet_mdio_phy_id_after_mpe_capture_nonce();
        write_rp1_ethernet_mdio_phy_id_after_mpe_common("no-mdio-no-ethernet-control");
        write_early_static(" target=none controller=none compatible=none");
        write_early_static(" phy-handle=none phy-node=none phy-address=none");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=not-constructed");
        write_early_static(" observed-window-macb-mid-context-raw=withheld");
        write_early_static(" corrected-window-comparator-cpu-physical-target=none");
        write_early_static(" ncr-observed-target=not-constructed");
        write_early_static(" nsr-observed-target=not-constructed");
        write_early_static(" man-observed-target=not-constructed");
        write_early_static(" ncr-mpe-bit=withheld nsr-idle-bit=withheld man-data-bits=withheld");
        ...
        write_early_static(" man-writes-performed=false");
        write_early_static(" man-restore-write-performed=false touched-fields=none");
        write_rp1_ethernet_mdio_phy_id_after_mpe_rejections(false);
        write_early_static(
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_mpe_enable_candidate")]
pub fn run_rp1_ethernet_mdio_mpe_enable_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const MPE_MASK: u32 = 1 << 4;

    write_early_static("rpi5-rp1-ethernet-mdio-mpe-enable-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-mpe-enable-candidate: before-ncr-mpe-set-readback-restore\n",
    );
    wait_uart10_empty_early_phase();

    let macb_mid_context = read_rp1_reg_u32(MACB_MID_CONTEXT);
    let pre_raw = read_rp1_reg_u32(NCR);
    let mut write_value = pre_raw | MPE_MASK;
    let mut post_raw = pre_raw;
    let mut restore_raw = pre_raw;
    let mut write_performed = false;
    let mut restore_performed = false;

    let classification = if pre_raw == 0xffff_ffff || pre_raw == 0xdead_dead {
        write_value = 0;
        "rp1-ethernet-mdio-mpe-enable-blocked-target-inconclusive"
    } else {
        write_rp1_reg_u32_ordered(NCR, write_value);
        write_performed = true;
        post_raw = read_rp1_reg_u32(NCR);
        write_rp1_reg_u32_ordered(NCR, pre_raw);
        restore_performed = true;
        restore_raw = read_rp1_reg_u32(NCR);

        if restore_raw != pre_raw {
            "rp1-ethernet-mdio-mpe-enable-restore-failed"
        } else if post_raw & MPE_MASK == 0 {
            "rp1-ethernet-mdio-mpe-enable-readback-mismatch-restored"
        } else if pre_raw & MPE_MASK != 0 {
    ...
    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-mpe-enable-candidate");
        write_rp1_ethernet_mdio_mpe_enable_capture_nonce();
        write_rp1_ethernet_mdio_mpe_enable_common("candidate");
        write_early_static(" target=rp1-ethernet-mdio-mpe-enable-set-readback-restore");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        write_early_static(
        ...
        write_early_static(" ncr-observed-target=");
        write_early_hex_u64(NCR as u64);
        write_early_static(" ncr-offset=0x0 ncr-mpe-bit=4 ncr-mpe-mask=0x00000010");
        write_early_static(" write-rule=pre_raw|0x00000010 restore-invariant=restore_raw==pre_raw");
        write_early_static(" pre-raw=");
        write_early_hex_u64(pre_raw as u64);
        write_early_static(" write-value=");
        write_early_hex_u64(write_value as u64);
        ...
        write_early_static(" pre-mpe-set=");
        write_bool(pre_raw & MPE_MASK != 0);
        write_early_static(" post-mpe-set=");
        write_bool(post_raw & MPE_MASK != 0);
        write_early_static(" restore-eq-pre=");
        write_bool(restore_raw == pre_raw);
        write_early_static(" ncr-mpe-write-performed=");
        write_bool(write_performed);
        write_early_static(" ncr-restore-write-performed=");
        write_bool(restore_performed);
        write_early_static(" man-writes-performed=false phy-id-reads-performed=false");
        write_early_static(" touched-fields=");
        if write_performed || restore_performed {
            write_early_static("MACB/GEM_NCR.MPE");
        } else {
            write_early_static("none");
        }
        write_rp1_ethernet_mdio_mpe_enable_rejections(write_performed);
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_mpe_enable_no_mdio_control")]
pub fn run_rp1_ethernet_mdio_mpe_enable_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-mdio-mpe-enable-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-mpe-enable-control: no-ncr-mpe-no-mdio-no-ethernet-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-mpe-enable-control");
        write_rp1_ethernet_mdio_mpe_enable_capture_nonce();
        write_rp1_ethernet_mdio_mpe_enable_common("no-mdio-no-ethernet-control");
        write_early_static(" target=none controller=none compatible=none");
        write_early_static(" register=none access=none");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=not-constructed");
        write_early_static(" observed-window-macb-mid-context-raw=withheld");
        write_early_static(" ncr-observed-target=not-constructed");
        write_early_static(" ncr-offset=withheld ncr-mpe-bit=withheld ncr-mpe-mask=withheld");
        write_early_static(" write-rule=withheld restore-invariant=withheld");
        write_early_static(
            " pre-raw=withheld write-value=withheld post-raw=withheld restore-raw=withheld",
        );
        write_early_static(" pre-mpe-set=false post-mpe-set=false restore-eq-pre=true");
        write_early_static(" ncr-mpe-write-performed=false ncr-restore-write-performed=false");
        write_early_static(" man-writes-performed=false phy-id-reads-performed=false");
        write_early_static(" touched-fields=none");
        write_rp1_ethernet_mdio_mpe_enable_rejections(false);
        write_early_static(
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_register_vector_candidate")]
pub fn run_rp1_ethernet_mdio_register_vector_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const NSR: usize = 0x1c_0010_0008;
    const MAN: usize = 0x1c_0010_0034;
    ...
    write_early_static("rpi5-rp1-ethernet-mdio-register-vector-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-register-vector-candidate: before-corrected-target-guarded-phy1-register-vector-man-transactions\n",
    );
    wait_uart10_empty_early_phase();

    let macb_mid_context = read_rp1_reg_u32(MACB_MID_CONTEXT);
    let ncr_before = read_rp1_reg_u32(NCR);
    let mut nsr_before = [0u32; 6];
    let mut nsr_after = [0u32; 6];
    let mut man_after = [0u32; 6];
    ...
    let classification = if ncr_before & NCR_MPE == 0 {
        ...
        while index < MAN_FRAMES.len() {
            if !poll_rp1_ethernet_mdio_register_vector_idle(NSR, NSR_IDLE) {
                nsr_before[index] = read_rp1_reg_u32(NSR);
                timed_out = true;
                break;
            }
            nsr_before[index] = read_rp1_reg_u32(NSR);
            write_rp1_reg_u32_ordered(MAN, MAN_FRAMES[index]);
            man_writes_performed = true;
            if !poll_rp1_ethernet_mdio_register_vector_idle(NSR, NSR_IDLE) {
                nsr_after[index] = read_rp1_reg_u32(NSR);
                man_after[index] = read_rp1_reg_u32(MAN);
                timed_out = true;
                break;
            }
            nsr_after[index] = read_rp1_reg_u32(NSR);
            man_after[index] = read_rp1_reg_u32(MAN);
            values[index] = man_after[index] & 0xffff;
            valid[index] = true;
            completed_count += 1;
            index += 1;
        }
        ...
        } else if values.iter().all(|value| *value == 0xffff) {
            "mdio-phy1-register-vector-global-all-ones-visible"
        } else if values[2] == 0xffff
            && values[3] == 0xffff
            && values
                .iter()
                .enumerate()
                .any(|(idx, value)| idx != 2 && idx != 3 && *value != 0xffff)
        {
    ...
    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-register-vector-candidate");
        write_rp1_ethernet_mdio_register_vector_capture_nonce();
        write_rp1_ethernet_mdio_register_vector_common("candidate");
        write_early_static(" target=rp1-ethernet-mdio-after-mpe-clause22-phy1-register-vector");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        write_early_static(" phy-handle=phy1 phy-node=ethernet-phy@1 phy-address=1");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=");
        write_early_hex_u64(MACB_MID_CONTEXT as u64);
        ...
        write_early_static(" bmcr-man-frame=");
        write_early_hex_u64(MAN_FRAMES[0] as u64);
        write_early_static(" bmsr-man-frame=");
        write_early_hex_u64(MAN_FRAMES[1] as u64);
        write_early_static(" physid1-man-frame=");
        write_early_hex_u64(MAN_FRAMES[2] as u64);
        write_early_static(" physid2-man-frame=");
        write_early_hex_u64(MAN_FRAMES[3] as u64);
        write_early_static(" anar-man-frame=");
        write_early_hex_u64(MAN_FRAMES[4] as u64);
        write_early_static(" anlpar-man-frame=");
        write_early_hex_u64(MAN_FRAMES[5] as u64);
        write_early_static(" bmcr=");
        write_early_hex_u64(values[0] as u64);
        write_early_static(" bmcr-valid=");
        write_bool(valid[0]);
        write_early_static(" bmsr=");
        write_early_hex_u64(values[1] as u64);
        write_early_static(" bmsr-valid=");
        write_bool(valid[1]);
        write_early_static(" physid1=");
        write_early_hex_u64(values[2] as u64);
        write_early_static(" physid1-valid=");
        write_bool(valid[2]);
        write_early_static(" physid2=");
        write_early_hex_u64(values[3] as u64);
        write_early_static(" physid2-valid=");
        write_bool(valid[3]);
        write_early_static(" anar=");
        write_early_hex_u64(values[4] as u64);
        write_early_static(" anar-valid=");
        write_bool(valid[4]);
        write_early_static(" anlpar=");
        write_early_hex_u64(values[5] as u64);
        write_early_static(" anlpar-valid=");
        write_bool(valid[5]);
        write_early_static(" completed-register-count=");
        write_early_dec_u64(completed_count as u64);
        write_early_static(" nsr-before-vector=");
        write_early_hex_u64(nsr_before[0] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_before[1] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_before[2] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_before[3] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_before[4] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_before[5] as u64);
        write_early_static(" nsr-after-vector=");
        write_early_hex_u64(nsr_after[0] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_after[1] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_after[2] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_after[3] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_after[4] as u64);
        write_early_static(",");
        write_early_hex_u64(nsr_after[5] as u64);
        write_early_static(" man-after-vector=");
        write_early_hex_u64(man_after[0] as u64);
        write_early_static(",");
        write_early_hex_u64(man_after[1] as u64);
        write_early_static(",");
        write_early_hex_u64(man_after[2] as u64);
        write_early_static(",");
        write_early_hex_u64(man_after[3] as u64);
        write_early_static(",");
        write_early_hex_u64(man_after[4] as u64);
        write_early_static(",");
        write_early_hex_u64(man_after[5] as u64);
        write_early_static(" man-writes-performed=");
        write_bool(man_writes_performed);
        write_early_static(" man-restore-write-performed=false touched-fields=");
        if man_writes_performed {
        ...
        write_rp1_ethernet_mdio_register_vector_rejections(man_writes_performed);
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_register_vector_no_mdio_control")]
pub fn run_rp1_ethernet_mdio_register_vector_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-mdio-register-vector-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-mdio-register-vector-control: no-mdio-no-ethernet-no-mmio-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-mdio-register-vector-control");
        write_rp1_ethernet_mdio_register_vector_capture_nonce();
        write_rp1_ethernet_mdio_register_vector_common("no-mdio-no-ethernet-control");
        write_early_static(" target=none controller=none compatible=none");
        write_early_static(" phy-handle=none phy-node=none phy-address=none");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=not-constructed");
        write_early_static(" observed-window-macb-mid-context-raw=withheld");
        ...
        write_early_static(" ncr-before=withheld ncr-mpe-precondition-met=false");
        write_early_static(" ncr-after=withheld ncr-mpe-write-performed=false");
        write_early_static(" register-vector=withheld");
        write_early_static(
        ...
        write_early_static(" bmcr=withheld bmcr-valid=false");
        write_early_static(" bmsr=withheld bmsr-valid=false");
        write_early_static(" physid1=withheld physid1-valid=false");
        write_early_static(" physid2=withheld physid2-valid=false");
        write_early_static(" anar=withheld anar-valid=false");
        write_early_static(" anlpar=withheld anlpar-valid=false");
        write_early_static(" completed-register-count=0");
        write_early_static(
            " nsr-before-vector=withheld nsr-after-vector=withheld man-after-vector=withheld",
        );
        write_early_static(" man-writes-performed=false");
        write_early_static(" man-restore-write-performed=false touched-fields=none");
        write_rp1_ethernet_mdio_register_vector_rejections(false);
        write_early_static(
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_status_diagnostic_candidate")]
pub fn run_rp1_ethernet_phy1_status_diagnostic_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const NSR: usize = 0x1c_0010_0008;
    const MAN: usize = 0x1c_0010_0034;
    ...
    write_early_static("rpi5-rp1-ethernet-phy1-status-diagnostic-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-phy1-status-diagnostic-candidate: before-readonly-corrected-target-phy1-status-vector\n",
    );
    wait_uart10_empty_early_phase();
    ...
    let classification = if ncr_before & NCR_MPE == 0 {
        ...
        let mut index = 0usize;
        while index < MAN_FRAMES.len() {
            if !poll_rp1_ethernet_phy1_status_diagnostic_idle(NSR, NSR_IDLE) {
                nsr_before[index] = read_rp1_reg_u32(NSR);
                timed_out = true;
                break;
            }
        ...
        if timed_out {
            ...
            "mdio-phy1-status-diagnostic-visible"
        }
    };
    let ncr_after = read_rp1_reg_u32(NCR);
    let decoded = crate::rp1_ethernet::decode_rp1_ethernet_phy1_status(
        crate::rp1_ethernet::Rp1EthernetPhy1StatusRawVector {
            bmcr: values[0] as u16,
            bmsr: values[1] as u16,
            physid1: values[2] as u16,
            physid2: values[3] as u16,
            anar: values[4] as u16,
            anlpar: values[5] as u16,
        },
    );

    loop {
        write_early_static("TALOS: rp1-ethernet-phy1-status-diagnostic-candidate");
        write_rp1_ethernet_phy1_status_diagnostic_capture_nonce();
        write_rp1_ethernet_phy1_status_diagnostic_common("candidate");
        write_early_static(" target=rp1-ethernet-phy1-status-decode-from-accepted-register-vector");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        write_early_static(" phy-handle=phy1 phy-node=ethernet-phy@1 phy-address=1");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=");
        write_early_hex_u64(MACB_MID_CONTEXT as u64);
        ...
        write_early_static(" ncr-after=");
        write_early_hex_u64(ncr_after as u64);
        write_early_static(" ncr-write-performed=false phy-config-write-performed=false");
        write_early_static(
            " register-vector=MII_BMCR:0x00,MII_BMSR:0x01,MII_PHYSID1:0x02,MII_PHYSID2:0x03,MII_ADVERTISE_ANAR:0x04,MII_LPA_ANLPAR:0x05",
        );
        write_early_static(" raw-bmcr=");
        write_early_hex_u64(values[0] as u64);
        write_early_static(" raw-bmsr=");
        write_early_hex_u64(values[1] as u64);
        write_early_static(" raw-physid1=");
        write_early_hex_u64(values[2] as u64);
        write_early_static(" raw-physid2=");
        write_early_hex_u64(values[3] as u64);
        write_early_static(" raw-anar=");
        write_early_hex_u64(values[4] as u64);
        write_early_static(" raw-anlpar=");
        write_early_hex_u64(values[5] as u64);
        write_early_static(" bmcr-valid=");
        write_bool(valid[0]);
        write_early_static(" bmsr-valid=");
        ...
        write_early_static(" anlpar-valid=");
        write_bool(valid[5]);
        write_rp1_ethernet_phy1_status_decoded_fields(decoded);
        write_early_static(" completed-register-count=");
        write_early_dec_u64(completed_count as u64);
        write_early_static(" nsr-before-vector=");
        write_hex_vector6(nsr_before);
        write_early_static(" nsr-after-vector=");
        write_hex_vector6(nsr_after);
        write_early_static(" man-after-vector=");
        write_hex_vector6(man_after);
        write_early_static(" man-writes-performed=");
        write_bool(man_writes_performed);
        write_early_static(" man-restore-write-performed=false touched-fields=");
        if man_writes_performed {
        ...
        write_rp1_ethernet_phy1_status_diagnostic_rejections(man_writes_performed);
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_status_diagnostic_no_mdio_control")]
pub fn run_rp1_ethernet_phy1_status_diagnostic_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-phy1-status-diagnostic-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-phy1-status-diagnostic-control: no-mdio-no-ethernet-no-mmio-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-phy1-status-diagnostic-control");
        write_rp1_ethernet_phy1_status_diagnostic_capture_nonce();
        write_rp1_ethernet_phy1_status_diagnostic_common("no-mdio-no-ethernet-control");
        write_early_static(" target=none controller=none compatible=none");
        write_early_static(" phy-handle=none phy-node=none phy-address=none");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=not-constructed");
        write_early_static(" observed-window-macb-mid-context-raw=withheld");
        ...
        write_early_static(" man-writes-performed=false");
        write_early_static(" man-restore-write-performed=false touched-fields=none");
        write_rp1_ethernet_phy1_status_diagnostic_rejections(false);
        write_early_static(
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_candidate")]
pub fn run_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const NSR: usize = 0x1c_0010_0008;
    const MAN: usize = 0x1c_0010_0034;
    const NCR_MPE: u32 = 1 << 4;
    const NSR_IDLE: u32 = 1 << 2;
    const BMCR_MAN_FRAME: u32 = 0x6082_0000;
    const BMSR_MAN_FRAME: u32 = 0x6086_0000;

    write_early_static(
        "rpi5-rp1-ethernet-phy1-bmsr-double-sample-link-readiness-candidate: start\n",
    );
    write_early_static(
        "rpi5-rp1-ethernet-phy1-bmsr-double-sample-link-readiness-candidate: before-readonly-corrected-target-bmcr-bmsr-double-sample\n",
    );
    wait_uart10_empty_early_phase();

    let macb_mid_context = read_rp1_reg_u32(MACB_MID_CONTEXT);
    let ncr_before = read_rp1_reg_u32(NCR);
    let mut nsr_before_bmcr = 0u32;
    let mut nsr_after_bmcr = 0u32;
    let mut nsr_before_bmsr_first = 0u32;
    let mut nsr_after_bmsr_first = 0u32;
    let mut nsr_before_bmsr_second = 0u32;
    let mut nsr_after_bmsr_second = 0u32;
    let mut man_after_bmcr = 0u32;
    let mut man_after_bmsr_first = 0u32;
    let mut man_after_bmsr_second = 0u32;
    let mut bmcr_raw = 0u16;
    let mut bmsr_first_raw = 0u16;
    let mut bmsr_second_raw = 0u16;
    let mut bmcr_valid = false;
    let mut bmsr_first_valid = false;
    let mut bmsr_second_valid = false;
    let mut man_writes_performed = false;

    let classification = if ncr_before & NCR_MPE == 0 {
        "mdio-phy1-bmsr-double-sample-bmcr-precondition-blocker"
    } else if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(NSR, NSR_IDLE) {
        nsr_before_bmcr = read_rp1_reg_u32(NSR);
        "mdio-phy1-bmsr-double-sample-timeout"
    } else {
        nsr_before_bmcr = read_rp1_reg_u32(NSR);
        write_rp1_reg_u32_ordered(MAN, BMCR_MAN_FRAME);
        man_writes_performed = true;

        if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(NSR, NSR_IDLE) {
            nsr_after_bmcr = read_rp1_reg_u32(NSR);
            man_after_bmcr = read_rp1_reg_u32(MAN);
            "mdio-phy1-bmsr-double-sample-timeout"
        } else {
            nsr_after_bmcr = read_rp1_reg_u32(NSR);
            man_after_bmcr = read_rp1_reg_u32(MAN);
            bmcr_raw = (man_after_bmcr & 0xffff) as u16;
            bmcr_valid = true;
            let bmcr = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmcr(bmcr_raw);
            if bmcr.reset || bmcr.loopback || bmcr.restart_autoneg {
                "mdio-phy1-bmsr-double-sample-bmcr-precondition-blocker"
            } else if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(NSR, NSR_IDLE)
            {
                nsr_before_bmsr_first = read_rp1_reg_u32(NSR);
                "mdio-phy1-bmsr-double-sample-timeout"
            } else {
                nsr_before_bmsr_first = read_rp1_reg_u32(NSR);
                write_rp1_reg_u32_ordered(MAN, BMSR_MAN_FRAME);
                if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(NSR, NSR_IDLE) {
                    nsr_after_bmsr_first = read_rp1_reg_u32(NSR);
                    man_after_bmsr_first = read_rp1_reg_u32(MAN);
                    "mdio-phy1-bmsr-double-sample-timeout"
                } else {
                    nsr_after_bmsr_first = read_rp1_reg_u32(NSR);
                    man_after_bmsr_first = read_rp1_reg_u32(MAN);
                    bmsr_first_raw = (man_after_bmsr_first & 0xffff) as u16;
                    bmsr_first_valid = true;

                    if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(NSR, NSR_IDLE)
                    {
                        nsr_before_bmsr_second = read_rp1_reg_u32(NSR);
                        "mdio-phy1-bmsr-double-sample-timeout"
                    } else {
                        nsr_before_bmsr_second = read_rp1_reg_u32(NSR);
                        write_rp1_reg_u32_ordered(MAN, BMSR_MAN_FRAME);
                        if !poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(
                            NSR, NSR_IDLE,
                        ) {
                            nsr_after_bmsr_second = read_rp1_reg_u32(NSR);
                            man_after_bmsr_second = read_rp1_reg_u32(MAN);
                            "mdio-phy1-bmsr-double-sample-timeout"
                        } else {
                            nsr_after_bmsr_second = read_rp1_reg_u32(NSR);
                            man_after_bmsr_second = read_rp1_reg_u32(MAN);
                            bmsr_second_raw = (man_after_bmsr_second & 0xffff) as u16;
                            bmsr_second_valid = true;
                            let bmsr_second =
                                crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmsr(bmsr_second_raw);
                            if bmsr_second.link_status && bmsr_second.autoneg_complete {
                                "mdio-phy1-bmsr-double-sample-link-ready"
                            } else {
    ...
    let ncr_after = read_rp1_reg_u32(NCR);
    let bmcr = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmcr(bmcr_raw);
    let bmsr_first = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmsr(bmsr_first_raw);
    let bmsr_second = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmsr(bmsr_second_raw);

    loop {
        write_early_static("TALOS: rp1-ethernet-phy1-bmsr-double-sample-link-readiness-candidate");
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_capture_nonce();
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_common("candidate");
        write_early_static(" target=rp1-ethernet-phy1-bmsr-latch-low-double-sample-link-readiness");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        write_early_static(" phy-handle=phy1 phy-node=ethernet-phy@1 phy-address=1");
        write_early_static(" observed-window-macb-mid-context-cpu-physical-target=");
        write_early_hex_u64(MACB_MID_CONTEXT as u64);
        ...
        write_early_static(" raw-bmsr-first=");
        write_early_hex_u64(bmsr_first_raw as u64);
        write_early_static(" raw-bmsr-second=");
        write_early_hex_u64(bmsr_second_raw as u64);
        write_early_static(" bmcr-valid=");
        write_bool(bmcr_valid);
        write_early_static(" bmsr-first-valid=");
        write_bool(bmsr_first_valid);
        write_early_static(" bmsr-second-valid=");
        write_bool(bmsr_second_valid);
        write_early_static(" bmcr-reset=");
        write_bool(bmcr.reset);
        write_early_static(" bmcr-loopback=");
        write_bool(bmcr.loopback);
        write_early_static(" bmcr-restart-autoneg=");
        write_bool(bmcr.restart_autoneg);
        write_early_static(" bmcr-autoneg-enable=");
        write_bool(bmcr.autoneg_enable);
        write_early_static(" bmcr-speed=");
        write_early_static(bmcr.speed_label);
        write_early_static(" bmcr-preconditions-clear=");
        write_bool(bmcr_valid && !bmcr.reset && !bmcr.loopback && !bmcr.restart_autoneg);
        write_early_static(" bmsr-first-link-status=");
        write_bool(bmsr_first.link_status);
        write_early_static(" bmsr-first-autoneg-complete=");
        write_bool(bmsr_first.autoneg_complete);
        write_early_static(" bmsr-second-link-status=");
        write_bool(bmsr_second.link_status);
        write_early_static(" bmsr-second-autoneg-complete=");
        write_bool(bmsr_second.autoneg_complete);
        write_early_static(" bmsr-second-autoneg-ability=");
        write_bool(bmsr_second.autoneg_ability);
        write_early_static(" latch-low-sample-policy=classify-from-second-bmsr-sample");
        write_early_static(" link-readiness-result-valid=");
        write_bool(bmcr_valid && bmsr_first_valid && bmsr_second_valid);
        write_early_static(" nsr-before-bmcr=");
        write_early_hex_u64(nsr_before_bmcr as u64);
        write_early_static(" nsr-after-bmcr=");
        write_early_hex_u64(nsr_after_bmcr as u64);
        write_early_static(" nsr-before-bmsr-first=");
        write_early_hex_u64(nsr_before_bmsr_first as u64);
        write_early_static(" nsr-after-bmsr-first=");
        write_early_hex_u64(nsr_after_bmsr_first as u64);
        write_early_static(" nsr-before-bmsr-second=");
        write_early_hex_u64(nsr_before_bmsr_second as u64);
        write_early_static(" nsr-after-bmsr-second=");
        write_early_hex_u64(nsr_after_bmsr_second as u64);
        write_early_static(" man-after-bmcr=");
        write_early_hex_u64(man_after_bmcr as u64);
        write_early_static(" man-after-bmsr-first=");
        write_early_hex_u64(man_after_bmsr_first as u64);
        write_early_static(" man-after-bmsr-second=");
        write_early_hex_u64(man_after_bmsr_second as u64);
        write_early_static(" man-writes-performed=");
        write_bool(man_writes_performed);
        write_early_static(" man-restore-write-performed=false touched-fields=");
        if man_writes_performed {
        ...
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_rejections(man_writes_performed);
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
        wait_uart10_empty_early_phase();
...
pub fn run_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-phy1-bmsr-double-sample-link-readiness-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-phy1-bmsr-double-sample-link-readiness-control: no-mdio-no-ethernet-no-mmio-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-phy1-bmsr-double-sample-link-readiness-control");
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_capture_nonce();
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_common(
        ...
        write_early_static(" raw-bmcr=withheld raw-bmsr-first=withheld raw-bmsr-second=withheld");
        write_early_static(" bmcr-valid=false bmsr-first-valid=false bmsr-second-valid=false");
        write_early_static(
            " decoded-bmcr=withheld decoded-bmsr-first=withheld decoded-bmsr-second=withheld",
        );
        write_early_static(" bmcr-preconditions-clear=false");
        write_early_static(" latch-low-sample-policy=withheld");
        write_early_static(" link-readiness-result-valid=false");
        write_early_static(
            " nsr-before-bmcr=withheld nsr-after-bmcr=withheld nsr-before-bmsr-first=withheld nsr-after-bmsr-first=withheld nsr-before-bmsr-second=withheld nsr-after-bmsr-second=withheld",
        );
        write_early_static(
        ...
        write_early_static(" man-writes-performed=false");
        write_early_static(" man-restore-write-performed=false touched-fields=none");
        write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_rejections(false);
        write_early_static(
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_post_physical_link_status_candidate")]
pub fn run_rp1_ethernet_post_physical_link_status_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const NSR: usize = 0x1c_0010_0008;
    const MAN: usize = 0x1c_0010_0034;
    ...
    write_early_static("rpi5-rp1-ethernet-post-physical-link-status-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-post-physical-link-status-candidate: before-readonly-phy1-and-macb-link-status-sample\n",
    );
    wait_uart10_empty_early_phase();
    ...
    let mut bmsr_first_raw = 0u16;
    let mut bmsr_second_raw = 0u16;
    let mut anar_raw = 0u16;
    let mut anlpar_raw = 0u16;
    let mut bmcr_valid = false;
    let mut bmsr_first_valid = false;
    let mut bmsr_second_valid = false;
    let mut anar_valid = false;
    let mut anlpar_valid = false;
    let mut mdio_read_count = 0u32;

    let classification = if ncr_before & NCR_MPE == 0 {
        "post-physical-link-status-source-precondition-blocker"
    } else if let Some(value) = read_rp1_ethernet_post_physical_link_status_phy1_register(
        NSR,
        MAN,
        NSR_IDLE,
        BMCR_READ_MAN_FRAME,
        ...
        bmcr_valid = true;
        mdio_read_count += 1;
        if let Some(value) = read_rp1_ethernet_post_physical_link_status_phy1_register(
            NSR,
            MAN,
            NSR_IDLE,
            BMSR_READ_MAN_FRAME,
        ) {
            bmsr_first_raw = value;
            bmsr_first_valid = true;
            mdio_read_count += 1;
            if let Some(value) = read_rp1_ethernet_post_physical_link_status_phy1_register(
                NSR,
                ...
                if let Some(value) = read_rp1_ethernet_post_physical_link_status_phy1_register(
                    ...
                    if let Some(value) = read_rp1_ethernet_post_physical_link_status_phy1_register(
                        ...
                        anlpar_valid = true;
                        mdio_read_count += 1;
                        "post-physical-link-status-sampled"
                    } else {
                        "post-physical-link-status-source-precondition-blocker"
                    }
                } else {
    ...
    let bmsr_first = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmsr(bmsr_first_raw);
    let bmsr_second = crate::rp1_ethernet::decode_rp1_ethernet_phy1_bmsr(bmsr_second_raw);
    let anar = crate::rp1_ethernet::decode_rp1_ethernet_phy1_advertisement(anar_raw);
    let anlpar = crate::rp1_ethernet::decode_rp1_ethernet_phy1_advertisement(anlpar_raw);
    let all_phy_reads_valid =
        bmcr_valid && bmsr_first_valid && bmsr_second_valid && anar_valid && anlpar_valid;
    let phy_ready = bmsr_second.link_status && bmsr_second.autoneg_complete && anlpar_raw != 0;
    let final_classification = if classification != "post-physical-link-status-sampled" {
        classification
    } else if phy_ready && macb_nsr_link {
        "post-physical-link-status-link-ready"
    } else if !phy_ready {
        "post-physical-link-status-phy-not-ready"
    } else if !macb_nsr_link {
    ...
    loop {
        ...
        write_early_static(" accepted-v2-bmcr-write-value=0x1200 accepted-v2-post-bmcr=0x1000");
        write_early_static(
        ...
        write_early_hex_u64(bmcr_raw as u64);
        write_early_static(" bmsr-first-raw=");
        write_early_hex_u64(bmsr_first_raw as u64);
        write_early_static(" bmsr-second-raw=");
        write_early_hex_u64(bmsr_second_raw as u64);
        write_early_static(" anar-raw=");
        write_early_hex_u64(anar_raw as u64);
        write_early_static(" anlpar-raw=");
        write_early_hex_u64(anlpar_raw as u64);
        write_early_static(" macb-nsr-raw=");
        write_early_hex_u64(macb_nsr_raw as u64);
        write_early_static(" bmcr-valid=");
        write_bool(bmcr_valid);
        ...
        write_bool(bmsr_second_valid);
        write_early_static(" anar-valid=");
        write_bool(anar_valid);
        write_early_static(" anlpar-valid=");
        write_bool(anlpar_valid);
        write_early_static(" all-phy-reads-valid=");
        write_bool(all_phy_reads_valid);
        write_early_static(" bmcr-reset=");
        write_bool(bmcr.reset);
        write_early_static(" bmcr-loopback=");
        write_bool(bmcr.loopback);
        write_early_static(" bmcr-autoneg-enable=");
        write_bool(bmcr.autoneg_enable);
        write_early_static(" bmcr-autoneg-restart=");
        write_bool(bmcr.restart_autoneg);
        write_early_static(" bmcr-isolate=");
        write_bool(bmcr.isolate);
        write_early_static(" bmcr-speed=");
        write_early_static(bmcr.speed_label);
        write_early_static(" bmsr-first-link-status=");
        write_bool(bmsr_first.link_status);
        ...
        write_early_static(" bmsr-second-autoneg-ability=");
        write_bool(bmsr_second.autoneg_ability);
        write_early_static(" anar-selector=");
        write_early_dec_u64(anar.selector as u64);
        write_early_static(" anar-10hd=");
        write_bool(anar.capable_10_half);
        write_early_static(" anar-10fd=");
        write_bool(anar.capable_10_full);
        write_early_static(" anar-100tx-hd=");
        write_bool(anar.capable_100_tx_half);
        write_early_static(" anar-100tx-fd=");
        write_bool(anar.capable_100_tx_full);
        write_early_static(" anlpar-selector=");
        write_early_dec_u64(anlpar.selector as u64);
        write_early_static(" anlpar-10hd=");
        write_bool(anlpar.capable_10_half);
        write_early_static(" anlpar-10fd=");
        write_bool(anlpar.capable_10_full);
        write_early_static(" anlpar-100tx-hd=");
        write_bool(anlpar.capable_100_tx_half);
        write_early_static(" anlpar-100tx-fd=");
        write_bool(anlpar.capable_100_tx_full);
        write_early_static(" anlpar-acknowledge=");
        write_bool(anlpar.acknowledge);
        write_early_static(" anlpar-nonzero=");
        write_bool(anlpar_raw != 0);
        write_early_static(" macb-nsr-link=");
        write_bool(macb_nsr_link);
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_post_physical_link_status_no_mdio_macb_control")]
pub fn run_rp1_ethernet_post_physical_link_status_no_mdio_macb_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-post-physical-link-status-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-post-physical-link-status-control: no-mdio-no-macb-no-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-post-physical-link-status-control");
        write_rp1_ethernet_post_physical_link_status_capture_nonce();
        write_rp1_ethernet_post_physical_link_status_common("no-mdio-no-macb-control");
        write_early_static(" target=none controller=none compatible=none");
        write_early_static(" phy-handle=none phy-node=none phy-address=none");
        write_early_static(" physical-link-precondition=confirmed");
        write_early_static(
        ...
        write_early_static(
            " bmcr-valid=false bmsr-first-valid=false bmsr-second-valid=false anar-valid=false anlpar-valid=false all-phy-reads-valid=false",
        );
        write_early_static(
            " bmcr-reset=withheld bmcr-loopback=withheld bmcr-autoneg-enable=withheld bmcr-autoneg-restart=withheld bmcr-isolate=withheld bmcr-speed=withheld",
        );
        write_early_static(" bmsr-first-link-status=withheld bmsr-first-autoneg-complete=withheld");
        write_early_static(
            " bmsr-second-link-status=withheld bmsr-second-autoneg-complete=withheld bmsr-second-autoneg-ability=withheld",
        );
        write_early_static(
            " anar-selector=withheld anar-10hd=withheld anar-10fd=withheld anar-100tx-hd=withheld anar-100tx-fd=withheld",
        );
        write_early_static(
            " anlpar-selector=withheld anlpar-10hd=withheld anlpar-10fd=withheld anlpar-100tx-hd=withheld anlpar-100tx-fd=withheld anlpar-acknowledge=withheld",
        );
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_bcm54213pe_readonly_preflight_candidate")]
pub fn run_rp1_ethernet_bcm54213pe_readonly_preflight_candidate() -> ! {
    const MACB_MID_CONTEXT: usize = 0x1c_0010_00fc;
    const NCR: usize = 0x1c_0010_0000;
    const NSR: usize = 0x1c_0010_0008;
    const MAN: usize = 0x1c_0010_0034;
    const NCR_MPE: u32 = 1 << 4;
    const NSR_IDLE: u32 = 1 << 2;
    const CTRL1000_READ_MAN_FRAME: u32 = 0x60a6_0000;
    const STAT1000_READ_MAN_FRAME: u32 = 0x60aa_0000;

    write_early_static("rpi5-rp1-ethernet-bcm54213pe-readonly-preflight-candidate: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-bcm54213pe-readonly-preflight-candidate: before-phy1-mii-ctrl1000-stat1000-readonly-sample\n",
    );
    wait_uart10_empty_early_phase();

    let macb_mid_context = read_rp1_reg_u32(MACB_MID_CONTEXT);
    let ncr_before = read_rp1_reg_u32(NCR);
    let mut ctrl1000_raw = 0u16;
    let mut stat1000_raw = 0u16;
    let mut ctrl1000_valid = false;
    let mut stat1000_valid = false;
    let mut mdio_read_count = 0u32;

    let classification = if ncr_before & NCR_MPE == 0 {
        "bcm54213pe-readonly-preflight-precondition-blocker"
    } else if let Some(value) = read_rp1_ethernet_bcm54213pe_readonly_preflight_phy1_register(
        NSR,
        MAN,
        NSR_IDLE,
        ...
        if let Some(value) = read_rp1_ethernet_bcm54213pe_readonly_preflight_phy1_register(
            ...
            stat1000_valid = true;
            mdio_read_count += 1;
            "bcm54213pe-readonly-preflight-sampled"
        } else {
            "bcm54213pe-readonly-preflight-precondition-blocker"
        }
    } else {
    ...
    let ncr_after = read_rp1_reg_u32(NCR);
    let decoded = crate::rp1_ethernet::decode_rp1_ethernet_phy1_gigabit_preflight(
        crate::rp1_ethernet::Rp1EthernetPhy1GigabitPreflightRawVector {
            ctrl1000: ctrl1000_raw,
            stat1000: stat1000_raw,
        },
    );
    let man_read_command_writes = mdio_read_count > 0;

    loop {
        write_early_static("TALOS: rp1-ethernet-bcm54213pe-readonly-preflight-candidate");
        write_rp1_ethernet_bcm54213pe_readonly_preflight_capture_nonce();
        write_rp1_ethernet_bcm54213pe_readonly_preflight_common("candidate");
        write_early_static(" target=phy1-mii-ctrl1000-stat1000-readonly-preflight");
        write_early_static(" controller=rp1_eth compatible=raspberrypi,rp1-gem,cdns,macb");
        ...
        write_early_static(" ncr-after=");
        write_early_hex_u64(ncr_after as u64);
        write_early_static(" selected-registers=MII_CTRL1000:0x09,MII_STAT1000:0x0a");
        write_early_static(" ctrl1000-man-frame=");
        write_early_hex_u64(CTRL1000_READ_MAN_FRAME as u64);
        write_early_static(" stat1000-man-frame=");
        write_early_hex_u64(STAT1000_READ_MAN_FRAME as u64);
        write_early_static(" ctrl1000-raw=");
        write_early_hex_u64(ctrl1000_raw as u64);
        write_early_static(" ctrl1000-valid=");
        write_bool(ctrl1000_valid);
        write_early_static(" stat1000-raw=");
        write_early_hex_u64(stat1000_raw as u64);
        write_early_static(" stat1000-valid=");
        write_bool(stat1000_valid);
        write_early_static(" ctrl1000-advertise-1000-half=");
        write_bool(decoded.ctrl1000.advertise_1000_half);
        write_early_static(" ctrl1000-advertise-1000-full=");
        write_bool(decoded.ctrl1000.advertise_1000_full);
        write_early_static(" ctrl1000-prefer-master=");
        write_bool(decoded.ctrl1000.prefer_master);
        write_early_static(" ctrl1000-force-master=");
        write_bool(decoded.ctrl1000.force_master);
        write_early_static(" ctrl1000-master-manual-config-enable=");
        write_bool(decoded.ctrl1000.master_manual_config_enable);
        write_early_static(" stat1000-master-slave-resolution-failure=");
        write_bool(decoded.stat1000.master_slave_resolution_failure);
        write_early_static(" stat1000-master-slave-resolution-master=");
        write_bool(decoded.stat1000.master_slave_resolution_master);
        write_early_static(" stat1000-local-receiver-ok=");
        write_bool(decoded.stat1000.local_receiver_ok);
        write_early_static(" stat1000-remote-receiver-ok=");
        write_bool(decoded.stat1000.remote_receiver_ok);
        write_early_static(" stat1000-link-partner-1000-full=");
        write_bool(decoded.stat1000.link_partner_1000_full);
        write_early_static(" stat1000-link-partner-1000-half=");
        write_bool(decoded.stat1000.link_partner_1000_half);
        write_early_static(" completed-register-count=");
        write_early_hex_u64(mdio_read_count as u64);
        write_early_static(" mdio-target-constructed=true man-frame-constructed=true");
        write_early_static(" macb-target-constructed=false gpio-target-constructed=false");
        write_early_static(" phy-register-write-performed=false bmcr-write-performed=false");
        write_early_static(
            " broadcom-selector-write-performed=false phy-reset-or-gpio32-action=false",
        );
        write_early_static(" phy-or-mac-configuration-performed=false packet-io-performed=false");
        write_rp1_ethernet_bcm54213pe_readonly_preflight_rejections(
            mdio_read_count == 2,
            man_read_command_writes,
        );
        write_early_static(" classification=");
        write_early_static(classification);
        write_early_static("\n");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_bcm54213pe_readonly_preflight_no_mdio_control")]
pub fn run_rp1_ethernet_bcm54213pe_readonly_preflight_no_mdio_control() -> ! {
    write_early_static("rpi5-rp1-ethernet-bcm54213pe-readonly-preflight-control: start\n");
    write_early_static(
        "rpi5-rp1-ethernet-bcm54213pe-readonly-preflight-control: no-mdio-no-ethernet-no-target-construction\n",
    );
    wait_uart10_empty_early_phase();

    loop {
        write_early_static("TALOS: rp1-ethernet-bcm54213pe-readonly-preflight-control");
        write_rp1_ethernet_bcm54213pe_readonly_preflight_capture_nonce();
        write_rp1_ethernet_bcm54213pe_readonly_preflight_common("no-mdio-no-ethernet-control");
        write_early_static(" target=none controller=none compatible=none");
        ...
        write_early_static(" selected-registers=withheld");
        write_early_static(" ctrl1000-man-frame=withheld stat1000-man-frame=withheld");
        write_early_static(" ctrl1000-raw=withheld ctrl1000-valid=false");
        write_early_static(" stat1000-raw=withheld stat1000-valid=false");
        write_early_static(
            " ctrl1000-advertise-1000-half=false ctrl1000-advertise-1000-full=false",
        );
        write_early_static(
            " ctrl1000-prefer-master=false ctrl1000-force-master=false ctrl1000-master-manual-config-enable=false",
        );
        write_early_static(
            " stat1000-master-slave-resolution-failure=false stat1000-master-slave-resolution-master=false",
        );
        write_early_static(" stat1000-local-receiver-ok=false stat1000-remote-receiver-ok=false");
        write_early_static(
            " stat1000-link-partner-1000-full=false stat1000-link-partner-1000-half=false",
        );
        write_early_static(" completed-register-count=0");
        write_early_static(" mdio-target-constructed=false man-frame-constructed=false");
        write_early_static(" macb-target-constructed=false gpio-target-constructed=false");
        ...
        write_early_static(" phy-or-mac-configuration-performed=false packet-io-performed=false");
        write_rp1_ethernet_bcm54213pe_readonly_preflight_rejections(false, false);
        write_early_static(
            " classification=no-mdio-no-ethernet-bcm54213pe-readonly-preflight-control\n",
        );
        wait_uart10_empty_early_phase();
    }
}
...
fn write_rp1_ethernet_gpio32_event_state_common(report_kind: &str) {
    write_early_static(
        " event-state-report-contract-id=phase12-rp1-ethernet-gpio32-event-state-readonly-discriminator-report-v1",
    );
    write_early_static(
        " event-state-contract-id=phase12-rp1-ethernet-gpio32-event-state-source-contract-v1",
    );
    write_early_static(
        " source-task-id=phase12-rp1-ethernet-gpio32-event-state-source-contract-20260611",
    );
    write_early_static(" report-kind=");
    write_early_static(report_kind);
    write_early_static(
        " hardware-proof-boundary-classification=hardware-proof-limited-to-gpio32-readonly-event-state-control-output",
    );
}
...
fn write_rp1_ethernet_gpio32_event_state_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_gpio32_event_state_rejections() {
    write_early_static(
        " allowed-classifications=rp1-ethernet-gpio32-event-state-clear-precondition,rp1-ethernet-gpio32-event-state-blocked-event-state,rp1-ethernet-gpio32-event-state-source-unresolved-event-state,rp1-ethernet-gpio32-event-state-inconclusive-capture,no-gpio-no-ethernet-rp1-ethernet-gpio32-event-state-control,staging/build-blocker",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=event-clearing,gpio-rio-pad-or-mmio-writes,gpio32-ownership,phy-reset-assertion-or-deassertion,gpio32-write-restore-retry-or-success,mdio-transactions-or-phy-ownership,ethernet-driver-readiness,interrupt-delivery-handler-ownership-or-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=event-state-discrimination-does-not-authorize-clear-or-write,gpio32-ownership-not-proven,no-mdio-phy-ownership,no-ethernet-driver-readiness,no-packet-io,no-network-stack",
    );
    write_early_static(" claims-event-clear=false claims-gpio-rio-pad-mmio-write=false");
    write_early_static(" claims-gpio32-ownership=false claims-phy-reset-assertion=false");
    write_early_static(
        " claims-phy-reset-deassertion=false claims-gpio32-write-restore-retry=false",
    );
    write_early_static(" claims-mdio-transactions=false claims-phy-ownership=false");
    write_early_static(" claims-ethernet-driver-ready=false claims-interrupt-ownership=false");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_state_candidate")]
fn classify_rp1_ethernet_gpio32_event_state(status: u32) -> &'static str {
    if status & 0x0ff0_0000 == 0 {
...
fn write_rp1_ethernet_gpio32_event_clear_common(report_kind: &str) {
    write_early_static(
        " event-clear-report-contract-id=phase12-rp1-ethernet-gpio32-event-clear-guard-report-contract-v1",
    );
    write_early_static(
        " source-contract-id=phase12-rp1-ethernet-gpio32-event-clear-source-contract-v1",
    );
    write_early_static(
        " source-task-id=phase12-rp1-ethernet-gpio32-event-clear-source-contract-20260611",
    );
    write_early_static(" report-kind=");
    write_early_static(report_kind);
    write_early_static(
...
fn write_rp1_ethernet_gpio32_event_clear_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_gpio32_event_clear_rejections() {
    write_early_static(
        " allowed-classifications=event-clear-visible-with-invariants,event-clear-persistent-or-firmware-owned-blocker,source-contract-violated-blocker,precise-staging-capture-blocker,no-gpio-no-ethernet-rp1-ethernet-gpio32-event-clear-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=ctrl-rw-clr-xor-write,rio-pad-function-mutation,gpio32-ownership,phy-reset-assertion-or-deassertion,gpio32-write-restore-retry-or-success,mdio-transactions-or-phy-ownership,ethernet-driver-readiness,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=level-event-bits-may-reassert,firmware-or-hardware-may-repopulate-event-bits,gpio32-ownership-not-proven,no-mdio-phy-ownership,no-ethernet-driver-readiness,no-packet-io,no-network-stack",
    );
    #[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_clear_candidate")]
    write_early_static(" claims-event-clear-executed=true");
    #[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_clear_no_mmio_control")]
    write_early_static(" claims-event-clear-executed=false");
    write_early_static(" claims-write-outside-irqreset=false claims-ctrl-rw-clr-xor-write=false");
    write_early_static(" claims-rio-pad-function-mutation=false claims-gpio32-ownership=false");
    write_early_static(" claims-phy-reset-assertion=false claims-phy-reset-deassertion=false");
    write_early_static(" claims-gpio32-write-restore-retry=false claims-mdio-transactions=false");
    write_early_static(" claims-phy-ownership=false claims-ethernet-driver-ready=false");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_mdio_phy_id_after_mpe_candidate")]
fn poll_rp1_ethernet_mdio_phy_id_after_mpe_idle(nsr: usize, idle_bit: u32) -> bool {
    let mut remaining = 1_000_000u32;
    while remaining != 0 {
        if read_rp1_reg_u32(nsr) & idle_bit != 0 {
            return true;
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_status_diagnostic_candidate")]
fn poll_rp1_ethernet_phy1_status_diagnostic_idle(nsr: usize, idle_bit: u32) -> bool {
    poll_rp1_ethernet_mdio_register_vector_idle(nsr, idle_bit)
}

#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_candidate")]
fn poll_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_idle(
    nsr: usize,
    idle_bit: u32,
) -> bool {
    poll_rp1_ethernet_mdio_register_vector_idle(nsr, idle_bit)
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_post_physical_link_status_candidate")]
fn poll_rp1_ethernet_post_physical_link_status_idle(nsr: usize, idle_bit: u32) -> bool {
    poll_rp1_ethernet_mdio_register_vector_idle(nsr, idle_bit)
}
...
fn write_rp1_ethernet_mdio_phy_id_after_mpe_common(report_kind: &str) {
    write_early_static(
        " mdio-phy-id-report-contract-id=phase12-rp1-ethernet-mdio-phy-id-after-mpe-guard-report-contract-v1",
    );
    write_early_static(
        " source-contract-id=phase12-rp1-ethernet-mdio-phy-id-after-mpe-source-contract-v1",
    );
    write_early_static(
        " source-task-id=phase12-rp1-ethernet-mdio-phy-id-after-mpe-source-contract-20260611",
    );
    write_early_static(
...
fn write_rp1_ethernet_mdio_phy_id_after_mpe_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_mdio_phy_id_after_mpe_rejections(runtime_mdio_transaction: bool) {
    write_early_static(
        " allowed-classifications=mdio-phy1-physid-after-mpe-visible,mdio-phy1-physid-after-mpe-timeout,mdio-phy1-physid-after-mpe-precondition-blocker,precise-staging-capture-blocker,no-mdio-no-ethernet-rp1-ethernet-mdio-phy-id-after-mpe-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=ethernet-driver-readiness,broad-mdio-or-phy-ownership,ncr-write-permission-or-execution,man-write-without-corrected-ncr-mpe-precondition,phy-reset-ownership,gpio32-ownership-event-clearing-write-restore-or-reset-success,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=corrected-ncr-mpe-may-be-clear,gpio32-and-eth-rst-n-remain-unowned,visible-phy-id-proves-only-selected-management-transaction,no-link-mac-dma-interrupt-packet-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-runtime-mdio-transaction=");
    write_bool(runtime_mdio_transaction);
    write_early_static(" claims-mdio-phy-ownership=false claims-mpe-write-permission=false");
    write_early_static(
        " claims-ncr-write-permission=false claims-gpio32-phy-reset-ownership=false",
    );
    write_early_static(" claims-ethernet-ready=false claims-broad-mmio-ready=false");
    write_early_static(" claims-interrupt-completion=false claims-dma-descriptor-ownership=false");
    write_early_static(" claims-packet-io=false claims-networking=false claims-sockets=false");
...
fn write_rp1_ethernet_mdio_mpe_enable_common(report_kind: &str) {
    write_early_static(
        " mdio-mpe-enable-report-contract-id=phase12-rp1-ethernet-mdio-mpe-enable-guard-report-contract-v1",
    );
    write_early_static(
        " source-contract-id=phase12-rp1-ethernet-mdio-mpe-enable-source-contract-v1",
    );
    write_early_static(
        " source-task-id=phase12-rp1-ethernet-mdio-mpe-enable-source-contract-20260611",
    );
    write_early_static(" report-kind=");
    write_early_static(report_kind);
    write_early_static(
...
fn write_rp1_ethernet_mdio_mpe_enable_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_mdio_mpe_enable_rejections(mpe_write_performed: bool) {
    write_early_static(
        " allowed-classifications=rp1-ethernet-mdio-mpe-enable-set-readback-restored,rp1-ethernet-mdio-mpe-enable-already-set-restored,rp1-ethernet-mdio-mpe-enable-readback-mismatch-restored,rp1-ethernet-mdio-mpe-enable-restore-failed,rp1-ethernet-mdio-mpe-enable-blocked-target-inconclusive,precise-staging-capture-blocker,no-mdio-no-ethernet-rp1-ethernet-mdio-mpe-enable-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=man-writes,phy-id-reads,broad-mdio-or-phy-ownership,phy-reset-or-gpio32-ownership,ethernet-driver-readiness,broad-ethernet-mmio-readiness,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=mpe-enable-is-necessary-not-sufficient-for-visible-mdio,gpio32-and-eth-rst-n-remain-unowned,no-man-phy-id-link-mac-dma-interrupt-packet-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-ncr-mpe-write-readback-restore-boundary=");
    write_bool(mpe_write_performed);
    write_early_static(" claims-man-writes=false claims-phy-id-reads=false");
    write_early_static(" claims-mdio-phy-ownership=false claims-gpio32-phy-reset-ownership=false");
    write_early_static(" claims-ethernet-ready=false claims-broad-mmio-ready=false");
    write_early_static(" claims-interrupt-completion=false claims-dma-descriptor-ownership=false");
    write_early_static(" claims-packet-io=false claims-networking=false claims-sockets=false");
    write_early_static(" claims-ssh=false claims-phase-12-2=false claims-phase-transition=false");
...
fn write_rp1_ethernet_mdio_register_vector_common(report_kind: &str) {
    write_early_static(
        " mdio-register-vector-report-contract-id=phase12-rp1-ethernet-mdio-register-vector-guard-report-contract-v1",
    );
    write_early_static(
        " source-contract-id=phase12-rp1-ethernet-mdio-register-vector-source-contract-v1",
    );
    write_early_static(
        " source-task-id=phase12-rp1-ethernet-mdio-register-vector-source-contract-20260611",
    );
    write_early_static(
...
fn write_rp1_ethernet_mdio_register_vector_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_mdio_register_vector_rejections(runtime_mdio_transaction: bool) {
    write_early_static(
        " allowed-classifications=mdio-phy1-register-vector-visible,mdio-phy1-register-vector-global-all-ones-visible,mdio-phy1-register-vector-physid-only-all-ones-mixed-visible,mdio-phy1-register-vector-timeout,mdio-phy1-register-vector-precondition-blocker,precise-staging-capture-blocker,no-mdio-no-ethernet-rp1-ethernet-mdio-register-vector-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=ethernet-driver-readiness,broad-mdio-or-phy-ownership,ncr-write-permission-or-execution,man-write-without-corrected-ncr-mpe-precondition,phy-absence-from-all-ones,phy-reset-ownership,gpio32-ownership-event-clearing-write-restore-or-reset-success,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=corrected-ncr-mpe-may-be-clear,all-ones-vector-is-not-phy-absence-proof,gpio32-and-eth-rst-n-remain-unowned,no-link-mac-dma-interrupt-packet-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-runtime-mdio-transaction=");
    write_bool(runtime_mdio_transaction);
    write_early_static(" claims-mdio-phy-ownership=false claims-mpe-write-permission=false");
    write_early_static(" claims-ncr-write-permission=false claims-ncr-write-executed=false");
    write_early_static(" claims-phy-absence-from-all-ones=false");
...
fn write_rp1_ethernet_phy1_status_diagnostic_common(report_kind: &str) {
    write_early_static(
        " phy1-status-diagnostic-report-contract-id=phase12-rp1-ethernet-phy1-status-diagnostic-report-contract-v1",
    );
    write_early_static(" task-id=phase12-rp1-ethernet-phy1-status-diagnostic-pi5-proof-20260614");
    write_early_static(
        " source-contract-id=phase12-rp1-ethernet-mdio-register-vector-source-contract-v1",
    );
    write_early_static(
...
fn write_rp1_ethernet_phy1_status_diagnostic_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_status_diagnostic_candidate")]
fn write_hex_vector6(values: [u32; 6]) {
    write_early_hex_u64(values[0] as u64);
    write_early_static(",");
    write_early_hex_u64(values[1] as u64);
    write_early_static(",");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_phy1_status_diagnostic_candidate")]
fn write_rp1_ethernet_phy1_status_decoded_fields(
    decoded: crate::rp1_ethernet::Rp1EthernetPhy1StatusDecode,
) {
    write_early_static(" bmcr-reset=");
    write_bool(decoded.bmcr.reset);
    write_early_static(" bmcr-loopback=");
    write_bool(decoded.bmcr.loopback);
    write_early_static(" bmcr-speed-select-lsb-100=");
    write_bool(decoded.bmcr.speed_select_lsb_100);
    write_early_static(" bmcr-speed-select-msb-1000=");
    write_bool(decoded.bmcr.speed_select_msb_1000);
    write_early_static(" bmcr-speed=");
    write_early_static(decoded.bmcr.speed_label);
    write_early_static(" bmcr-autoneg-enable=");
    write_bool(decoded.bmcr.autoneg_enable);
    write_early_static(" bmcr-power-down=");
    write_bool(decoded.bmcr.power_down);
    write_early_static(" bmcr-isolate=");
    write_bool(decoded.bmcr.isolate);
    write_early_static(" bmcr-restart-autoneg=");
    write_bool(decoded.bmcr.restart_autoneg);
    write_early_static(" bmcr-duplex-full=");
    write_bool(decoded.bmcr.duplex_full);
    write_early_static(" bmsr-link-status=");
    write_bool(decoded.bmsr.link_status);
    write_early_static(" bmsr-autoneg-complete=");
    write_bool(decoded.bmsr.autoneg_complete);
    write_early_static(" bmsr-autoneg-ability=");
    write_bool(decoded.bmsr.autoneg_ability);
    write_early_static(" bmsr-extended-status=");
    write_bool(decoded.bmsr.extended_status);
    write_early_static(" bmsr-extended-capability=");
    write_bool(decoded.bmsr.extended_capability);
    write_early_static(" bmsr-capabilities=");
    write_bool(decoded.bmsr.capable_10_half);
    write_early_static(",");
    write_bool(decoded.bmsr.capable_10_full);
    write_early_static(",");
    write_bool(decoded.bmsr.capable_100base_x_half);
    write_early_static(",");
    write_bool(decoded.bmsr.capable_100base_x_full);
    write_early_static(",");
    write_bool(decoded.bmsr.capable_100base_t4);
    write_early_static(" phy-id-oui=");
    write_early_hex_u64(decoded.phy_id.oui as u64);
    write_early_static(" phy-id-model=");
    write_early_hex_u64(decoded.phy_id.model as u64);
    write_early_static(" phy-id-revision=");
    write_early_hex_u64(decoded.phy_id.revision as u64);
    write_early_static(" anar-selector=");
    write_early_dec_u64(decoded.anar.selector as u64);
    write_early_static(" anar-10hd=");
    write_bool(decoded.anar.capable_10_half);
    write_early_static(" anar-10fd=");
    write_bool(decoded.anar.capable_10_full);
    write_early_static(" anar-100tx-hd=");
    write_bool(decoded.anar.capable_100_tx_half);
    write_early_static(" anar-100tx-fd=");
    write_bool(decoded.anar.capable_100_tx_full);
    write_early_static(" anar-100t4=");
    write_bool(decoded.anar.capable_100_t4);
    write_early_static(" anar-pause=");
    write_bool(decoded.anar.pause);
    write_early_static(" anar-asym-pause=");
    write_bool(decoded.anar.asym_pause);
    write_early_static(" anlpar-selector=");
    write_early_dec_u64(decoded.anlpar.selector as u64);
    write_early_static(" anlpar-10hd=");
    write_bool(decoded.anlpar.capable_10_half);
    write_early_static(" anlpar-10fd=");
    write_bool(decoded.anlpar.capable_10_full);
    write_early_static(" anlpar-100tx-hd=");
    write_bool(decoded.anlpar.capable_100_tx_half);
    write_early_static(" anlpar-100tx-fd=");
    write_bool(decoded.anlpar.capable_100_tx_full);
    write_early_static(" anlpar-100t4=");
    write_bool(decoded.anlpar.capable_100_t4);
    write_early_static(" anlpar-pause=");
    write_bool(decoded.anlpar.pause);
    write_early_static(" anlpar-asym-pause=");
    write_bool(decoded.anlpar.asym_pause);
    write_early_static(" anlpar-acknowledge=");
    write_bool(decoded.anlpar.acknowledge);
}
...
fn write_rp1_ethernet_phy1_status_diagnostic_rejections(runtime_mdio_transaction: bool) {
    write_early_static(
        " allowed-classifications=mdio-phy1-status-diagnostic-visible,mdio-phy1-status-diagnostic-timeout,mdio-phy1-status-diagnostic-precondition-blocker,precise-staging-capture-blocker,no-mdio-no-ethernet-rp1-ethernet-phy1-status-diagnostic-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=ncr-write-permission-or-execution,phy-configuration-write,phy-reset-or-gpio32-action,autonegotiation-restart,link-forcing,broad-mdio-or-phy-ownership,ethernet-driver-readiness,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=decoded-link-and-autoneg-bits-are-register-state-only,gpio32-and-eth-rst-n-remain-unowned,no-phy-config-packet-dma-interrupt-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-runtime-mdio-transaction=");
    write_bool(runtime_mdio_transaction);
    write_early_static(" claims-mdio-phy-ownership=false claims-phy-config-write=false");
    write_early_static(" claims-ncr-write-permission=false claims-ncr-write-executed=false");
    write_early_static(" claims-phy-reset-or-gpio32-action=false");
    write_early_static(" claims-autoneg-restart=false claims-link-forcing=false");
    write_early_static(" claims-ethernet-ready=false claims-broad-mmio-ready=false");
    write_early_static(" claims-interrupt-completion=false claims-dma-descriptor-ownership=false");
    write_early_static(" claims-packet-io=false claims-networking=false claims-sockets=false");
    write_early_static(" claims-ssh=false claims-phase-12-2=false claims-phase-transition=false");
...
fn write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_common(report_kind: &str) {
    write_early_static(
        " phy1-bmsr-double-sample-link-readiness-contract-id=phase12-rp1-ethernet-phy1-bmsr-latch-low-double-sample-link-readiness-contract-v1",
    );
    write_early_static(
    ...
    write_early_static(" accepted-frontier=rp1-ethernet-phy1-status-diagnostic-frontier-closed");
    write_early_static(
...
fn write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_phy1_bmsr_double_sample_link_readiness_rejections(
    runtime_mdio_transaction: bool,
) {
    write_early_static(
        " allowed-classifications=mdio-phy1-bmsr-double-sample-link-ready,mdio-phy1-bmsr-double-sample-link-not-ready,mdio-phy1-bmsr-double-sample-bmcr-precondition-blocker,mdio-phy1-bmsr-double-sample-timeout,precise-staging-capture-blocker,no-mdio-no-ethernet-rp1-ethernet-phy1-link-readiness-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=ncr-write-permission-or-execution,phy-configuration-write,phy-reset-or-gpio32-action,autonegotiation-restart,link-forcing,broad-mdio-or-phy-ownership,ethernet-driver-readiness,interrupt-completion,dma-descriptor-rings-channel-ownership-or-transfer-completion,packet-io,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=link-readiness-is-limited-to-second-bmsr-sample-register-state,gpio32-and-eth-rst-n-remain-unowned,no-phy-config-packet-dma-interrupt-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-runtime-mdio-transaction=");
    write_bool(runtime_mdio_transaction);
    write_early_static(" claims-mdio-phy-ownership=false claims-phy-config-write=false");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_post_physical_link_status_candidate")]
fn read_rp1_ethernet_post_physical_link_status_phy1_register(
    nsr: usize,
    man: usize,
    idle_bit: u32,
    read_frame: u32,
) -> Option<u16> {
    if !poll_rp1_ethernet_post_physical_link_status_idle(nsr, idle_bit) {
        return None;
    }
    write_rp1_reg_u32_ordered(man, read_frame);
    if !poll_rp1_ethernet_post_physical_link_status_idle(nsr, idle_bit) {
...
fn read_rp1_ethernet_bcm54213pe_readonly_preflight_phy1_register(
    ...
    if !poll_rp1_ethernet_bcm54213pe_readonly_preflight_idle(nsr, idle_bit) {
        return None;
    }
    write_rp1_reg_u32_ordered(man, read_frame);
    if !poll_rp1_ethernet_bcm54213pe_readonly_preflight_idle(nsr, idle_bit) {
...
fn write_rp1_ethernet_post_physical_link_status_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_bcm54213pe_readonly_preflight_common(report_kind: &str) {
    write_early_static(
        " bcm54213pe-readonly-preflight-hw-proof-contract-id=phase12-rp1-ethernet-bcm54213pe-readonly-preflight-hw-proof-contract-v1",
    );
    write_early_static(
        " task-id=phase12-rp1-ethernet-bcm54213pe-readonly-preflight-pi5-proof-20260616",
    );
    write_early_static(
        " proof-core-task-id=phase12-rp1-ethernet-bcm54213pe-readonly-preflight-hw-proof-core-20260616",
    );
    write_early_static(
        " report-core-task-id=phase12-rp1-ethernet-bcm54213pe-readonly-preflight-report-core-20260616",
    );
    write_early_static(
        " source-contract-task-id=phase12-rp1-ethernet-bcm54213pe-readonly-preflight-source-contract-20260616",
    );
    write_early_static(" accepted-frontier=bcm54213pe-readonly-preflight-frontier-closed");
    write_early_static(
        " selected-discriminator=bcm54213pe-phy1-mii-ctrl1000-stat1000-readonly-preflight",
    );
    write_early_static(" report-kind=");
    write_early_static(report_kind);
    write_early_static(
        " hardware-proof-boundary-classification=hardware-proof-limited-to-phy1-mii-ctrl1000-stat1000-control-output",
    );
}
...
fn write_rp1_ethernet_bcm54213pe_readonly_preflight_capture_nonce() {
    if let Some(nonce) = option_env!("TALOS_CAPTURE_NONCE") {
        if !nonce.is_empty() {
            write_early_static(" capture-nonce=");
            write_early_static(nonce);
...
fn write_rp1_ethernet_bcm54213pe_readonly_preflight_rejections(
    runtime_mdio_reads: bool,
    runtime_man_read_command_writes: bool,
) {
    write_early_static(
        " bounded-runtime-hardware-claims=clause22-phy1-mii-ctrl1000-stat1000-man-read-command-writes",
    );
    write_early_static(
        " allowed-classifications=bcm54213pe-readonly-preflight-sampled,bcm54213pe-readonly-preflight-precondition-blocker,bcm54213pe-readonly-preflight-capture-blocker,no-mdio-no-ethernet-bcm54213pe-readonly-preflight-control",
    );
    write_early_static(
        " rejected-runtime-hardware-claims=phy-register-write,selector-write-surfaces,gpio32-reset-action,bmcr-autoneg-restart,broadcom-shadow-mmd-aux-access,interrupt-ownership,phy-mac-configuration,link-readiness,packet-io,dma-descriptors,networking,sockets,ssh,phase-12-2,phase-transition",
    );
    write_early_static(
        " retained-risks=ctrl1000-stat1000-sample-is-only-selected-register-state,gpio32-reset-and-broadcom-selector-surfaces-remain-unowned,no-link-readiness-packet-dma-interrupt-socket-ssh-or-phase-12-2-readiness",
    );
    write_early_static(" claims-runtime-mdio-reads=");
    write_bool(runtime_mdio_reads);
    write_early_static(" claims-runtime-man-read-command-writes=");
    write_bool(runtime_man_read_command_writes);
    write_early_static(" claims-phy-register-write=false claims-bmcr-write=false");
    write_early_static(
        " claims-selector-write-surface=false claims-broadcom-shadow-mmd-aux-access=false",
    );
    write_early_static(" claims-gpio32-reset-action=false claims-interrupt-ownership=false");
    write_early_static(" claims-phy-or-mac-configuration=false claims-link-readiness=false");
...
#[cfg(talos_boot_scenario = "rpi5_rp1_ethernet_gpio32_event_clear_candidate")]
fn classify_rp1_ethernet_gpio32_event_clear_precondition(
    pre_status: u32,
    pre_ctrl: u32,
    pre_rio1_out: u32,
    pre_rio1_oe: u32,
    ...
    if pre_status == 0xffff_ffff
        || pre_status == 0xdead_dead
        || pre_ctrl == 0xffff_ffff
        || pre_ctrl == 0xdead_dead
        || pre_rio1_out == 0xffff_ffff
        || pre_rio1_out == 0xdead_dead
        || pre_rio1_oe == 0xffff_ffff
        || pre_rio1_oe == 0xdead_dead
        || pre_rio1_in == 0xffff_ffff
        || pre_rio1_in == 0xdead_dead
        || pre_pad == 0xffff_ffff
        || pre_pad == 0xdead_dead
    {
        return Some("precise-staging-capture-blocker");
    }
    if pre_event_bits != 0x0ab0_0000 || funcsel != 5 || override_bits != 0 {
        return Some("source-contract-violated-blocker");
    }
    None
}
